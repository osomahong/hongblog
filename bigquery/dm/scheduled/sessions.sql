-- sessions 증분 적재. 상류가 비어 있으면 DELETE 없이 실패시켜 데이터 유실을 막는다.
...
SET upstream_rows = (SELECT COUNT(*) FROM `hong-project2511.analytics_dm.events` WHERE date BETWEEN target_start AND target_end);

IF upstream_rows = 0 THEN
  SELECT ERROR(FORMAT(
    'analytics_dm.events에 %t ~ %t 구간 데이터가 없어 중단합니다. 대상 테이블은 그대로 둡니다.',
    target_start, target_end));
END IF;

DELETE FROM `hong-project2511.analytics_dm.sessions` WHERE date BETWEEN target_start AND target_end;

INSERT INTO `hong-project2511.analytics_dm.sessions`
