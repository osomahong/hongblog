-- search_content_daily 증분 적재. 상류가 비어 있으면 DELETE 없이 실패시켜 데이터 유실을 막는다.
DECLARE target_start DATE DEFAULT DATE_SUB(@run_date, INTERVAL 10 DAY);
DECLARE target_end   DATE DEFAULT DATE_SUB(@run_date, INTERVAL 1 DAY);
DECLARE upstream_rows INT64;

SET upstream_rows = (SELECT COUNT(*) FROM `hong-project2511.analytics_dm.content_daily` WHERE date BETWEEN target_start AND target_end);

IF upstream_rows = 0 THEN
  SELECT ERROR(FORMAT(
    'analytics_dm.content_daily에 %t ~ %t 구간 데이터가 없어 중단합니다. 대상 테이블은 그대로 둡니다.',
    target_start, target_end));
END IF;

DELETE FROM `hong-project2511.analytics_dm.search_content_daily` WHERE date BETWEEN target_start AND target_end;

INSERT INTO `hong-project2511.analytics_dm.search_content_daily`
...
WITH gsc_raw AS (
  ...
  WHERE data_date BETWEEN target_start AND target_end
    AND url IS NOT NULL
),
gsc_page AS (
  SELECT
...
ga AS (
  ...
  WHERE date BETWEEN target_start AND target_end
)
SELECT
