...
이 글에서는 2026년 9월 기준 공식 발표에서 확인되는 사양과, 로컬 AI 에이전트가 돌아가는 조건이 무엇인지, 이미 있는 맥 스튜디오나 RTX 5090 데스크톱과 무엇이 다른지를 정리하겠습니다. 실행체를 집에 두고 폰으로 보는 구성 자체는 [테일스케일 사용법](/insights/tailscale-usage-ai-remote-work)과 [맥미니 tmux 설정](/insights/mac-tmux-ssh-tailscale-remote-ai-setup) 글에서 다뤘으므로, 여기서는 그 실행체가 될 컴퓨터 쪽만 봅니다.
