...
title: 'GPT-6 아스트라(Astra) 해설, 다른 모델 비교와 AI 시대 전환기의 중장기 전망'
excerpt: >-
  GPT-6 아스트라는 OpenAI가 2026년 9월 3일 공개한, 컴퓨터를 사람처럼 직접 조작하는 데 초점을 둔 모델입니다. 이번 출시가
  왜 예전 모델 발표와 다른 반응을 얻었는지, 클로드 페이블 5.1과 제미나이 3.8 플래시와 견주면 어디에 서는지, ARC-AGI-3
...
highlights:
  ...
  - >-
    ...
    보입니다.
metaTitle: 'GPT-6 아스트라(Astra) 해설, 모델 비교와 AI 전환기 중장기 전망 (2026)'
metaDescription: >-
  GPT-6 아스트라는 OpenAI가 2026년 9월 3일 공개한, 컴퓨터를 사람처럼 직접 조작하는 데 초점을 둔 모델입니다. 클로드
  페이블 5.1과 제미나이 3.8 플래시와의 비교, ARC-AGI-3 99.9%의 조건, SNS 반응, 사무직과 AI 전환기의 중장기 전망을
  정리했습니다.
ogTitle: 'GPT-6 아스트라 해설, 다른 모델 비교와 AI 전환기의 중장기 전망'
ogDescription: >-
...
summary3:
  - >-
    GPT-6 아스트라는 OpenAI가 2026년 9월 3일 공개한 모델로 컴퓨터 조작에 초점을 두었고 OSWorld 2.0에서
    작업당 약 40분에 72.6%를 기록해 GPT-5.6 Sol의 75분과 65.7%를 앞섰습니다.
  - >-
    ARC-AGI-3 점수는 표준 하네스에서 62.7%, 추론 상태를 넘겨주는 하네스에서 99.9%로 달라졌고 독립 평가의 종합 지능
    지수에서는 클로드 페이블 5.1보다 5점 낮은 61점을 받았습니다.
  - >-
    모델 점수 경쟁이 하네스와 작업당 비용, 신뢰 경쟁으로 옮겨 가는 전환기로 보이며 사무직 일자리는 단기 실업률보다 업무 단위의
    재편으로 먼저 나타날 가능성이 커 보입니다.
---
...
국내 반응은 유튜브에서 먼저 나왔습니다. 한 경제 유튜버의 해설 영상은 올라온 지 7시간 만에 조회 19만 회와 댓글 313개를 모았습니다. 영상의 결론은 제목 그대로 아스트라의 진짜 목표가 사무직이라는 것이었는데, 그 결론에 댓글이 어떻게 반응했는지가 더 눈에 들어옵니다.

![국내 경제 유튜브 채널의 GPT-6 아스트라 해설 영상 페이지. 아스트라의 진짜 목표가 사무직이라는 제목 아래 조회수 19만 회와 좋아요 2천 7백 건, 댓글 313개가 표시되어 있다](/images/insights/gpt-6-astra-ai-transition-era/10-youtube-dante-gpt6-office-workers.png)

- **가장 많은 공감을 받은 댓글**은 몸 관리를 해 두고 언제든 현장 노동에 투입될 준비를 하자는 농담이었습니다
- **두 번째**는 사무직에서 무직으로 간다는 짧은 댓글이었습니다
- **그 아래**로 두 달마다 발전 폭이 커진다는 반응과 토큰 비용 걱정이 이어졌습니다

같은 날 올라온 한 개발자 유튜버의 쇼츠는 AGI라는 표현에 조금 더 거리를 두었습니다. 여기서 가장 많은 공감을 받은 댓글은 AGI 선언은 늘 하던 호들갑일 것이라는 내용이었고, 추론 도중에 사용자에게 되묻는 기능이 제일 마음에 든다는 댓글이 그다음이었습니다. 벤치마크 점수를 다 믿지 못하겠다며 실제로 써 봐야 안다는 반응도 있었습니다.

![국내 개발자 유튜브 채널의 GPT-6 아스트라 쇼츠 화면. 진짜 AGI인지 묻는 제목 아래 OpenAI 발표 페이지의 은하 이미지가 보이고 AGI의 출현이라고도 말했다는 자막이 붙어 있다](/images/insights/gpt-6-astra-ai-transition-era/11-youtube-zerocho-shorts-agi.png)
...
출시 과정 자체도 반응을 키웠습니다. 첫날은 사이버보안 프로그램 데이브레이크에 들어간 기업 고객에게만 열렸고 유료 구독자는 며칠을 기다리게 됐습니다. 샘 올트먼은 몇 시간 뒤 어수선한 출시였다고 사과했고, OpenAI는 아스트라를 못 쓰는 날마다 사용량 초기화권을 하나씩 적립해 주기로 했습니다. 쇼츠에서 매일 리셋권을 준다고 한 것이 이 조치입니다.

> 이번 반응의 크기는 점수표보다 시연 영상에서 나온 것으로 보입니다. 키보드를 아무도 치지 않는데 그림판과 3D 도구와 쇼핑 화면이 차례로 움직이는 장면이 사무직 일자리라는 단어와 곧바로 이어졌습니다.
...
앞서 언급한 경제 유튜버는 이 초점을 코딩 다음 단계로 읽었습니다. 클로드 코드와 Codex가 나오면서 코딩은 상당 부분 정복됐는데 컴퓨터를 대신 써 달라는 요청은 아직 잘 안 됐고, 그래서 코딩 직군보다 사무직 대체가 느렸다는 진단입니다. 영상에는 개인 사례도 하나 나옵니다. 국내에서 만든 AI 브라우저 어사이드에 클로드 오퍼스 5를 붙여 스마트링 구독 해지를 시켰더니 로그인용 일회용 비밀번호 수신부터 해지 확정까지 2분 45초 만에 끝났다고 합니다. [어사이드 브라우저 에이전트 비교](/insights/aside-vs-cdp-playwright-browser-agent)에서 다룬 그 도구입니다. 모델 한 단계 향상과 좋아진 하네스가 합쳐지는 지점을 실제로 겪은 사례로 볼 수 있습니다.
...
유튜브 영상의 결론과 댓글 반응은 사무직 위기라는 말로 모였습니다. 그 걱정이 근거 없는 것은 아닙니다. 다만 출시 며칠의 반응으로 앞날을 읽으면 두 가지를 놓치기 쉽습니다.
...
다음은 일하는 방식의 변화입니다. 와튼스쿨의 이선 몰릭은 6월 글에서 AI 사용법이 함께 일하는 방식에서 일을 맡기는 방식으로 옮겨 가고 있다고 적었습니다. OpenAI 안에서는 직원 4분의 1이 매주 에이전트 네 개 이상을 동시에 돌리고, 법무와 인사 같은 비기술 직군도 거의 같은 속도로 에이전트를 받아들였다는 내부 조사가 근거였습니다. 아스트라를 먼저 써 본 뒤에는 며칠 동안 중단 없이 의미 있는 작업을 이어 간다고 했고, 이메일과 글과 일정을 읽게 한 뒤 개인 위키를 닷새 가까이 만들게 한 사례가 유튜브 해설에 소개됐습니다.
...
아스트라는 토큰 가격이 전작의 2.5배인데 코딩 작업당 비용은 페이블 5의 절반입니다. 초당 33토큰과 최대 출력 단가를 곱하면 시간당 6달러 아래의 자동화 엔지니어를 고용하는 셈이라는 계산도 나왔습니다. 물론 병렬로 여러 에이전트를 돌리면 그 몇 배가 나가고, 유튜브 댓글에도 토큰이 녹겠다는 걱정이 상위에 있었습니다. 앞으로 모델을 고를 때는 100만 토큰당 가격표보다 우리 작업 하나에 얼마가 드는지가 판단 기준이 될 것으로 보입니다.
