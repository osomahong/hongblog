...
ogDescription: >-
  ...
  정리합니다.
summary3:
  - 'GA4의 not provided는 설정 오류가 아니라 검색엔진이 검색어를 웹사이트에 넘기지 않아서 생기는 정상 동작입니다.'
  - '구글은 2011년 검색어 암호화를 시작해 2013년 오가닉 검색어 전달을 전면 차단했고 네이버도 2024년 11월 검색 리퍼러 정보를 제한했습니다.'
  - '2026년 기준 대부분의 사이트에서 오가닉 검색어의 80에서 95퍼센트가 이 값이라 검색어는 서치 콘솔에서 보고 GA4에서는 랜딩 페이지로 대신 봅니다.'
---
...
| 시점 | 변화 |
|------|------|
| **2018년** | GDPR 시행. 유럽발 개인정보 보호 규제가 글로벌 표준으로 확산. 검색어 데이터를 수집할 법적 근거가 더욱 축소 |
...
(not provided)를 두고 가장 보편적인 대안은 **Google Search Console(GSC)을 GA4에 연결**하는 것입니다. GSC를 연결하면 GA4 내에서 "Google 오가닉 검색어" 보고서에서 키워드 데이터를 확인할 수 있습니다.

하지만 여기서 반드시 알아야 할 것이 있습니다. **GSC의 데이터와 GA4의 데이터는 측정 방식이 근본적으로 다릅니다.**

| 구분 | Google Search Console | GA4 |
|------|----------------------|-----|
| **관점** | 쿼리 중심 (어떤 검색어로 노출/클릭됐는지) | 사용자 중심 (방문 후 어떤 행동을 했는지) |
...
- 검색어별 노출 수, 클릭 수, 평균 게재순위, [CTR](/class/digital-marketing-terms/what-is-ctr) 확인
- 노출은 많지만 클릭이 적은 키워드 발견 → 제목/메타 디스크립션 최적화 기회 포착

**GSC 연결로 할 수 없는 것:**
- 특정 사용자가 어떤 검색어로 들어와서 [전환](/class/digital-marketing-terms/what-is-conversion)했는지 추적
- 검색어별 [전환율](/class/digital-marketing-terms/what-is-cvr), 이탈률, 체류시간 등 행동 데이터 직접 연결
...
<div>"어떤 페이지에 도착했고, 거기서 무엇을 했는지"</div>
...
GSC 연결만으로는 과거의 "검색어 → 사용자 행동 → 전환" 전체 흐름을 복원할 수 없습니다. 그렇다면 사고방식 자체를 바꿔야 합니다. **"어떤 검색어로 들어왔는지"에서 "어떤 페이지에 도착했고, 거기서 무엇을 했는지"로 분석의 축을 이동**하는 것입니다.
...
이 방식으로 "이 페이지에 오가닉으로 유입되는 사용자는 대체로 어떤 의도를 가지고 있고, 그 의도가 전환으로 이어지고 있는지"를 파악할 수 있습니다. 개별 사용자 단위의 검색어 추적은 불가능하지만, 페이지 단위의 의도 분석은 충분히 가능합니다.

### 글로벌 사례: Zapier의 의도 기반 페이지 전략

검색어 데이터 없이도 성공적인 오가닉 전략을 운영하는 대표적인 사례가 Zapier입니다. Zapier는 프로그래매틱 SEO로 50,000개 이상의 랜딩 페이지를 자동 생성했습니다. "Slack + Trello 자동화", "Gmail + Sheets 연동" 같은 조합별 페이지를 만들어, 각 페이지가 특정 검색 의도에 정확히 대응하도록 설계했습니다.
