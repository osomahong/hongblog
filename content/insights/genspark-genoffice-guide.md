...
title: '젠스파크 GenOffice 정리: 무료 오픈소스 AI 오피스'
excerpt: >-
...
summary3:
  ...
  - >-
    문서와 스프레드시트, 프레젠테이션, PDF를 다루는 네 앱으로 이뤄지고 원본 XML 조각을 붙여 두는 방식이라 저장해도 서식이 덜
    깨집니다.
  - >-
    Apache-2.0이지만 ee 디렉터리와 상표는 빠져 있고 알파 단계에 macOS는 애플 실리콘 전용이라 업무용 중요 문서에 쓰기는 아직
    이릅니다.
---
...
### 오픈소스니까 사내에 그대로 배포해도 되나요?

본체만 그렇습니다. 본체는 아파치 2.0이라 상업적 사용과 수정, 재배포가 되지만 `ee` 디렉터리에는 별도의 엔터프라이즈 라이선스가 걸려 있고 GenOffice와 Genspark 이름과 로고는 상표라서 함께 오지 않습니다. 포크해서 배포하려면 자체 브랜딩을 붙여야 합니다.

### 편집한 문서 내용이 외부로 나가나요?

AI 기능을 쓰는 순간에는 나갑니다. 저장소 설명에 따르면 앱은 젠스파크 계정으로 로그인해 모델 호출을 젠스파크 서버로 보내고 모델 API 키는 로컬에 저장하지 않습니다. 문서 편집 자체는 내 컴퓨터에서 이뤄지고 계정 없이도 되므로, 밖으로 나가면 안 되는 파일은 AI 패널을 쓰지 않는 선에서 다루는 편이 안전합니다.

### 인텔 맥에서도 설치할 수 있나요?

지금 배포되는 설치 파일로는 되지 않습니다. 2026년 8월 4일 기준 macOS용은 애플 실리콘 전용 dmg 하나이고 윈도우는 x64 설치 파일이 있습니다. 소스를 직접 빌드하는 방법이 남아 있지만 스프레드시트 앱의 xlsx 처리가 러스트로 만들어져 있어 러스트 도구 모음을 따로 갖춰야 합니다.
