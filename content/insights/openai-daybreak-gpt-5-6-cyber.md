...
title: OpenAI Daybreak 확장과 GPT-5.6-Cyber 공개 정리
excerpt: >-
...
summary3:
  ...
  - >-
    GPT-5.6-Cyber는 더 엄격한 Red 티어 뒤에 있고 2026년 9월 1일부터 모든 개별 Daybreak 계정에 하드웨어 보안
    키가 의무화됩니다.
---
...
GPT-5.6-Cyber는 [GPT-5.6 Sol](/insights/gpt-5-6-sol-terra-luna-comparison)을 기반으로, 이중용도 보안 작업에서 거부(refusal)를 줄이도록 훈련한 모델입니다. 듀얼 유즈(dual-use)는 방어에도 공격에도 쓸 수 있는 기술을 뜻합니다. 익스플로잇 체인 개발이나 권한 상승 같은 요청은 일반 모델이라면 안전을 이유로 답변을 거절하지만, 이 모델은 인가된 환경에서 그 작업을 수행하도록 만들어졌습니다.
...
OpenAI는 자사의 Preparedness Framework 기준으로 GPT-5.6-Cyber의 위험 등급을 "High"로 평가했고, 최고 단계인 "Critical"에는 이르지 않았다고 밝혔습니다. 다만 개발 중인 신형 [Astra 모델](/insights/openai-astra-model-comparison)은 Critical에 도달할 것으로 본다고 덧붙였습니다.

## 인가된 방어팀 관점의 두 가지 선택지

실무 관점에서 정리하면, 사내 보안팀이나 침투 테스트를 수행하는 조직에게 이번 발표는 두 가지 선택지를 만듭니다.
...
### Daybreak는 누구나 신청할 수 있나요?

심사를 통과한 방어팀만 쓸 수 있습니다. 개인과 조직이 각각 신원 인증과 계정 보안 요건, 모니터링 동의, 승인된 용도 안에서만 쓰겠다는 서약을 거쳐야 합니다. 2026년 9월 1일부터는 모든 개별 계정에 하드웨어 보안 키도 의무로 붙습니다.

### Blue와 Red 가운데 어느 쪽을 신청해야 하나요?

하는 일이 방어 쪽인지 공격적 점검 쪽인지에 따라 나뉩니다. 악성코드 분석이나 사건 대응, 패치 검증이 주 업무라면 Blue로 충분하고, 자사 시스템을 대상으로 한 취약점 연구나 레드팀 훈련이 필요하면 Red를 신청합니다. Red는 심사가 더 엄격하고 하드웨어 보안 키와 모니터링까지 함께 받아들여야 합니다.

### 다른 회사 시스템 점검에도 GPT-5.6-Cyber를 쓸 수 있나요?

허가받지 않은 대상에는 쓸 수 없습니다. Red 티어의 작업은 조직이 소유하거나 운영하는 시스템, 또는 테스트 허가를 받은 시스템으로 한정됩니다. 권한이 없는 대상에 쓰면 그 순간부터 방어가 아니라 공격에 해당합니다.
