...
summary3:
  - 'tmux는 터미널 하나에서 여러 프로그램을 띄우고 창을 닫아도 프로그램이 꺼지지 않게 유지하는 도구입니다.'
  - '구조는 작업 전체를 담는 세션과 탭에 해당하는 윈도우, 화면을 나눈 페인 세 단위로 되어 있습니다.'
  - '에이전트 하나로 충분한 동안은 도구를 늘리지 말고 macOS에서 여러 개를 동시에 관리한다면 cmux 같은 전용 앱도 검토할 만합니다.'
---
...
전통적으로 tmux가 가장 많이 쓰이는 상황은 원격 서버 작업입니다. 여기서 [SSH](/insights/tailscale-ssh-remote-access-guide)라는 용어가 낯설 수 있는데, 쉽게 말하면 내 컴퓨터의 터미널에서 멀리 있는 다른 컴퓨터(서버)에 접속해 명령을 내리는 표준 원격 접속 방식입니다. 접속해 있는 동안에는 그 서버의 터미널을 내 화면에서 그대로 쓰는 셈입니다. 예를 들어 서버에 SSH로 접속해 1시간짜리 데이터 처리 스크립트를 돌린다고 하면, 흐름은 이렇습니다.
...
[Claude Code](/class/claude-code-for-everyone/what-is-claude-code) 같은 AI 코딩 에이전트는 일을 시키면 수십 분씩 혼자 작업하기도 합니다. 여기서 자연스럽게 질문이 떠오릅니다. 에이전트가 일하는 동안 우리는 기다려야만 할까요? 그래서 나온 방식이 **에이전트 여러 개를 동시에 돌리는 병렬 실행**입니다. 하나는 기능 개발, 하나는 테스트 작성, 하나는 문서 정리를 맡기는 식입니다.
