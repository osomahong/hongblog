...
excerpt: >-
  AI 모델 증류는 큰 모델이 만든 답과 확신의 분포를 작은 모델에 옮겨 학습시키는 방법입니다. 위스키 증류와 어디까지 닮았는지, 실제로 어떤
  단계를 거치는지, 왜 모델 성능이 점점 빠르게 좋아지는지, 로컬 LLM은 2027년에 어디까지 갈지 정리했습니다.
category: AI_TECH
tags:
  - AI
  - API
...
summary3:
  - >-
    AI 모델 증류는 큰 교사 모델이 만든 출력과 확신의 분포를 작은 학생 모델에 옮겨 학습시키는 방법이고, 2015년 제프리 힌턴 연구진의
    논문에서 지금의 형태가 정리됐습니다.
  - >-
    DeepSeek는 671B 모델이 만든 추론 과정을 Qwen2.5와 Llama3 계열 여섯 모델에 옮겨 1.5B부터 70B까지 공개했고,
    작은 모델이 직접 학습해 얻는 것보다 물려받는 쪽이 성능이 높다고 밝혔습니다.
  - >-
    같은 성능을 내는 데 드는 추론 가격은 벤치마크에 따라 해마다 9배에서 900배까지 떨어졌고, 이 하락이 로컬에서 돌릴 수 있는 모델의
    수준을 끌어올리고 있습니다.
highlights:
  - >-
    로컬 모델을 고를 때 이름에 Distill이 붙어 있으면 교사 모델과 학생 아키텍처를 함께 확인합니다. 같은 7B라도 어느 모델에서
    무엇을 물려받았는지에 따라 잘하는 작업이 다릅니다.
  - >-
    증류 모델을 상업적으로 쓰기 전에 교사 모델 쪽 약관을 확인합니다. 주요 사업자는 자사 출력으로 경쟁 모델을 만드는 것을 약관에서
    금지하고 있습니다.
metaTitle: 'AI 모델 증류 뜻과 과정: 로컬 LLM이 좋아지는 이유'
metaDescription: >-
  AI 모델 증류는 큰 모델이 만든 답과 확신의 분포를 작은 모델에 옮겨 학습시키는 방법입니다. 위스키 증류와 무엇이 다른지, 어떤 단계를
  거치는지, 로컬 LLM 성능이 왜 빠르게 오르는지 정리했습니다.
ogDescription: 술을 내리는 증류와 이름은 같지만 과정은 다릅니다. 교사 모델의 확신까지 물려받는 구조와 로컬 LLM 전망을 함께 정리했습니다.
ogImage: /og/ai-model-distillation.png
quiz:
  - question: 모델 증류에서 학생 모델이 교사 모델로부터 추가로 얻는 정보는 무엇일까요?
    options:
      - 정답 하나만이 아니라 오답 후보들에 매긴 확신의 분포까지 함께 얻는다
      - 교사 모델의 파라미터를 그대로 복사해서 얻는다
      - 교사 모델이 학습에 쓴 원본 데이터를 그대로 넘겨받는다
      - 교사 모델의 하드웨어 사양과 실행 설정을 넘겨받는다
    correctIndex: 0
    explanation: >-
      증류의 핵심은 정답 하나가 아니라 확률 분포를 물려주는 데 있습니다. 고양이 사진을 두고 교사가 고양이 90%, 개 7%, 자동차
      0.1%라고 답하면 학생은 고양이가 자동차보다 개에 가깝다는 관계까지 배웁니다. 힌턴 연구진은 이렇게 분포에 담긴 정보를 다크
      놀리지라고 불렀습니다. 파라미터나 원본 데이터가 그대로 넘어가지는 않습니다.
---
...
1. **교사 모델에 데이터를 넣어 출력을 받습니다.** 이때 정답 하나만 받는 것이 아니라 모든 후보에 매긴 확률을 함께 받습니다.
...
3. **학생 모델을 그 분포에 맞춰 학습시킵니다.** 학습이 끝나면 온도를 원래대로 되돌려 씁니다.
...
두 번째 쪽이 훨씬 많은 것을 알려 줍니다. 힌턴 연구진은 이렇게 분포 안에 숨어 있는 정보를 다크 놀리지(dark knowledge)라고 불렀습니다. 쉽게 말하면 **교사가 답을 고르면서 무엇과 무엇을 헷갈렸는지에 담긴 정보**입니다. 정답표만 외운 학생과 채점자의 고민까지 들은 학생의 차이입니다. 앞에서 견습생이 장인의 망설임까지 본다고 한 것이 이 대목입니다.
