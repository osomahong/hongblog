...
highlights:
  ...
  - >-
    ...
    스파크를 봅니다.
  - >-
    긴 입력을 자주 넣는 에이전트 작업이라면 대역폭보다 계산 성능을 먼저 확인합니다. 프리필 단계가 먼저 걸립니다.
...
## 프리필과 디코드, 에이전트에서 드러나는 세 번째 축

앞 절에서 대역폭이 응답 속도를 정한다고 적었는데, 정확히는 응답의 한쪽만 정합니다. 모델이 답을 만드는 과정은 두 단계로 나뉘고 각 단계를 제한하는 것이 서로 다릅니다.

- **프리필(prefill)**: 입력한 내용 전체를 한 번에 읽어 내부 상태를 만드는 단계입니다. 연산량이 많아 계산 성능이 한계를 정합니다.
- **디코드(decode)**: 토큰을 하나씩 만들어 내는 단계입니다. 토큰마다 저장해 둔 상태를 읽어야 해서 메모리 대역폭이 한계를 정합니다.

두 단계의 비중은 무엇을 시키느냐에 따라 크게 달라집니다. 짧게 묻고 길게 답을 받는 대화라면 디코드가 대부분입니다. 반대로 저장소 전체를 읽히거나 긴 문서를 통째로 넣는 에이전트 작업이라면 프리필이 시간을 잡아먹습니다. 입력이 길어질수록 부담이 빠르게 커져서, 프롬프트를 두 배로 늘리면 어텐션 연산은 네 배 가까이 늘어납니다.

이 구분이 장비 선택을 뒤집는 경우가 있습니다. 대역폭만 보면 통합 메모리를 크게 단 맥 계열이 DGX 스파크보다 세 배가량 앞서고, 애플도 M3 울트라 맥 스튜디오에서 600B가 넘는 모델을 돌릴 수 있다고 안내합니다. 그런데 메모리가 512GB인 맥 스튜디오를 에이전트 서빙에 쓰다가 프리필이 느려 처분하고 DGX 스파크로 옮겼다는 홈랩 구성 사례가 있습니다. 긴 입력을 자주 넣는 작업에서 대역폭이 아니라 연산 성능이 먼저 걸린 것으로 보입니다.

| 작업 성격 | 주로 걸리는 단계 | 먼저 볼 값 |
|---|---|---|
| 짧은 질문과 대화 | 디코드 | 메모리 대역폭 |
| 긴 문서 요약, 저장소 읽기 | 프리필 | 계산 성능 |
| 에이전트가 여러 요청을 동시에 처리 | 프리필과 배칭 | 계산 성능과 실행 엔진의 배칭 지원 |
| 큰 모델을 올리는 것 자체 | 해당 없음 | 메모리 용량 |

DGX 스파크가 대역폭 273GB/s인데도 에이전트 용도로 언급되는 이유가 여기에 있습니다. 1페타플롭 FP4로 표기된 계산 성능이 프리필 쪽을 맡기 때문입니다. 커뮤니티 보고 가운데는 DGX 스파크 두 대로 오픈 모델을 서빙해 프리필 초당 1,000에서 2,000토큰, 디코드 초당 50토큰 안팎이 나왔다는 기록이 있습니다. 공식 벤치마크가 아니므로 참고 수준으로 보는 편이 맞습니다.

장비를 고르기 전에 확인할 것은 하나 더 늘어납니다. 돌릴 모델이 올라가는지(용량), 답이 빨리 나오는지(대역폭)에 더해, 평소 넣는 입력이 얼마나 긴지(연산 성능)를 같이 봐야 합니다.
...
- **96GB와 128GB 경계**: GLM-4.5 Air와 Qwen3.5 122B가 이 구간입니다. RTX PRO 6000 한 장이나 128GB 통합 메모리 장비에서 실행됩니다.
- **256GB 이상**: 300B를 넘는 모델은 DGX 스파크 두 대를 묶거나 DGX 스테이션에서 실행해야 합니다. GLM-5급 700B대는 DGX 스파크 네 대나 DGX 스테이션 범위이고, Qwen3.8 2.4T는 이 글의 어느 장비에서도 실행되지 않습니다.
...
## 여러 대를 상시 운영할 때 더 필요한 것

한 대로 모델을 띄워 보는 것과 여러 대를 24시간 돌리는 것은 준비물이 다릅니다. 장비를 늘리면 모델을 어디에 올릴지 정하는 문제 말고도 몇 가지가 따라옵니다.

- **모델 서빙**: 가중치를 올리고 요청을 받는 부분입니다. 여러 대에 나눠 올리거나 요청을 묶어 처리하려면 vLLM처럼 배칭을 지원하는 실행 엔진이 필요합니다.
- **진입점 하나로 묶기**: 장비가 늘면 주소도 늘어납니다. 여러 모델을 OpenAI 호환 주소 하나로 모아 두면 도구 쪽에서는 주소를 바꾸지 않아도 됩니다. [무료 LLM API를 묶는 방식](/insights/free-llm-api-router-limits-and-data-cost)과 구조가 같고, 대상이 외부 업체가 아니라 집 안의 장비라는 점만 다릅니다.
- **화면**: 터미널로만 쓰지 않는다면 웹 화면이 따로 필요합니다. Open WebUI처럼 여러 모델을 골라 쓸 수 있는 화면을 붙이는 구성이 흔합니다.
- **관측**: 어느 장비가 얼마나 바쁜지, 언제 메모리가 모자랐는지 남지 않으면 문제가 생겨도 원인을 찾기 어렵습니다.
- **외부 접근**: 밖에서 쓰려면 통로가 필요합니다. 준이아빠블로그에서 쓰는 방식은 [테일스케일로 연결하는 구성](/insights/tailscale-usage-ai-remote-work)에 정리해 두었습니다.

실제 구성 사례에는 이 요소들이 모두 들어 있습니다. DGX 스파크 세 대를 200Gbps로 직결하고, 서빙은 vLLM에 맡기고, 여러 모델의 진입점을 게이트웨이 하나로 모으고, 컨테이너 운영과 관측 도구는 별도 저전력 장비에 올린 홈랩 구성이 공유된 적도 있었습니다.

여기서 눈여겨볼 것은 장비를 고르기까지의 과정입니다. 그 사례는 메모리 용량과 대역폭, 프리필 연산량, 비용 네 가지를 기준으로 후보를 다시 살폈다고 밝혔습니다. 그래픽카드 두 장 구성은 용량이 모자라고 발열과 전력도 상시 가동에 부담이라 제외했습니다. 통합 메모리를 크게 단 맥은 프리필이 느려 에이전트에 쓰지 못했고, 카드 네 장 구성은 성능이 충분했지만 비용에서 걸렸습니다.

개인 사례이므로 그대로 따를 필요는 없습니다. 다만 장비를 고르는 기준이 용량 하나가 아니라 넷이었다는 점, 그리고 실제로 써 본 뒤에야 맞지 않는다는 것을 알았다는 점은 참고할 만합니다. 처음부터 크게 사기보다 쓰려는 작업으로 한 대를 먼저 재 보는 편이 안전합니다.

## DGX 스파크 두 대를 묶으면 무엇이 달라지나요?

메모리가 256GB로 늘어나 300B대 모델을 실행할 수 있습니다. 엔비디아는 두 대 연결 시 405B까지 다룰 수 있다고 안내하고, 연결은 본체의 ConnectX-7 포트를 200Gbps 케이블로 직접 잇는 방식입니다. 다만 토큰 속도가 두 배가 되는 것은 아닙니다. 두 대에 나뉜 가중치를 토큰마다 네트워크로 주고받아야 하므로, 두 대의 의미는 속도보다 한 대로 실행하지 못하던 모델을 다룬다는 데 있습니다.
