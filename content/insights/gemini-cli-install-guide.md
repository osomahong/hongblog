...
구글의 별도 지원 문서도 같은 날짜를 적었습니다. 제미나이 코드 어시스트 개인 등급과 구글 AI 프로, 울트라에서 제미나이 CLI와 구글 계정 로그인이 중단됐고, 스탠다드와 엔터프라이즈 구독은 그대로라는 안내입니다.
...
설정을 다시 만들 필요는 없다는 뜻입니다. 다만 무료 할당량은 줄어듭니다. 안티그래비티는 요금제에 따라 할당량이 다르고, 프로와 울트라가 아닌 사용자는 주 단위로 갱신되는 몫을 받습니다.
