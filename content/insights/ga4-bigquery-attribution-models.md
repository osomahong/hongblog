...
title: 'GA4 빅쿼리 어트리뷰션 분석: 3가지 모델 SQL 쿼리 정리'
excerpt: >-
...
quiz:
  - options:
    ...
    correctIndex: 3
metaTitle: 'GA4 빅쿼리 어트리뷰션 분석: 3가지 모델 SQL 쿼리 정리'
metaDescription: >-
  GA4 기본 보고서의 채널별 전환은 라스트클릭 한 관점이라 앞 단계에서 일한 채널이 낮게 잡힙니다. 빅쿼리 로우데이터로 세션 데이터마트와 전환
  경로 테이블을 만들고 라스트클릭, 퍼스트터치, 선형 모델을 SQL로 구현하는 방법을 정리했습니다.
ogDescription: >-
...
summary3:
  ...
  - >-
    빅쿼리로 내보낸 로우데이터는 user_pseudo_id와 ga_session_id를 묶어 세션 단위 데이터마트로 만드는 것이 첫
    단계입니다.
  - >-
    라스트클릭과 퍼스트터치, 선형 모델은 각각 LAST_VALUE와 FIRST_VALUE, 1을 접점 수로 나누는 방식으로 같은 데이터에
    동시에 적용할 수 있습니다.
---
...
문제는 실제 고객 여정이 단순하지 않다는 점입니다. 블로그 검색으로 브랜드를 알게 되고, 소셜 미디어 광고를 보고 관심이 높아진 뒤, 이메일 프로모션을 보고 최종 구매하는 경우를 생각해보겠습니다. 라스트클릭 모델에서는 이메일이 전환의 100%를 가져갑니다. 블로그 SEO와 소셜 광고의 기여는 보이지 않습니다.
...
어트리뷰션 분석의 출발점은 **세션 단위 데이터마트**입니다. GA4 빅쿼리 테이블(`events_*`)은 이벤트 단위 로우데이터이므로, 이를 세션 단위로 집계해야 "어떤 소스/매체로 들어왔는지"를 분석할 수 있습니다.
...
라스트클릭 모델은 **전환 직전 마지막 터치포인트에 100% 기여를 부여**하는 방식입니다. GA4 기본 보고서에서 사용하는 방식과 가장 가깝습니다. 구현이 단순하고 직관적이지만, 상단 [퍼널](/class/digital-marketing-terms/what-is-funnel)(인지, 관심 단계)의 채널을 과소평가하는 경향이 있습니다.
...
퍼스트터치 모델은 라스트클릭과 정반대입니다. **사용자가 처음 유입된 채널에 100% 기여를 부여**합니다. "이 사용자를 처음 데려온 채널이 어디인지"에 초점을 맞추므로, 신규 사용자 획득 채널의 가치를 평가하는 데 유용합니다.
...
전환 시점에서 30일 이전까지 포함하는 설정이 일반적입니다. 다만 제품 성격에 따라 달라져서 즉석 구매 상품은 7일이면 충분하고 B2B 솔루션은 90일까지 넓혀야 하는 경우도 있습니다. 기간을 제한하지 않으면 전환에서 멀리 떨어진 접점까지 경로에 들어옵니다.

### direct / (none) 세션은 빼고 분석해야 하나요?

어느 한쪽이 정답은 아닙니다. 포함한 채 따로 표시해 두면 direct 비중 자체가 UTM 관리 상태를 보여 주는 신호가 되고, source 조건으로 빼면 마케팅 채널의 기여만 비교할 수 있습니다. 빼는 쪽을 고를 때는 접점이 하나뿐인 전환이 통째로 누락된다는 점을 함께 봐야 합니다.

### 모델 하나만 골라서 써도 되나요?

한 모델만 보면 결론이 한쪽으로 기웁니다. 라스트클릭은 전환을 마무리한 채널을, 퍼스트터치는 신규 유입을 만든 채널을 크게 보여 주기 때문에 같은 데이터에 세 모델을 함께 돌려 비교하는 편이 낫습니다. 채널별로 라스트클릭에서 퍼스트터치를 뺀 값이 양수면 하단 퍼널, 음수면 상단 퍼널에서 일하는 채널로 읽습니다.
...
3. **세 가지 모델을 비교해야 편향을 줄일 수 있습니다.** 채널별 `lc_vs_ft_diff` 값으로 각 채널이 퍼널의 어느 단계에서 역할을 하는지 파악하고, 이를 기반으로 예산 배분 의사결정을 내릴 수 있습니다.
