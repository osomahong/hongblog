...
**대조군(Control Group)을 반드시 두세요.** 대조군이란 메시지를 아예 보내지 않는 그룹입니다. "메시지를 보낸 그룹이 안 보낸 그룹보다 얼마나 더 구매했는지"를 비교해야, CRM 메시지가 실제로 매출에 기여했는지 증명할 수 있습니다. 이걸 **증분 효과(Incrementality)**라고 합니다. 메시지를 안 보내도 어차피 살 사람이었다면, 그 매출은 CRM의 성과가 아니니까요.
