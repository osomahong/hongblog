...
excerpt: 바이브코딩과 AI 에이전트로 빠르게 서비스를 만들 수 있는 시대입니다. 하지만 인증 구조를 이해하지 못하면 비밀번호가 코드에 그대로 노출되는 사고가 일어납니다. 인증의 기본 개념부터 OAuth 2.0까지, 비개발자도 이해할 수 있도록 정리했습니다.
category: AI_TECH
tags:
  - API
  - 자동화
...
quiz:
  - options:
    ...
    explanation: Client Secret은 서버에서만 사용해야 하는 비밀 값입니다. AI가 생성한 코드에서 이 값이 프론트엔드(브라우저)에 노출되면, 누구나 여러분의 앱을 사칭할 수 있습니다. 바이브코딩 도구가 서버/클라이언트 구분 없이 코드를 생성하는 경우가 있으므로, 이 부분을 가장 먼저 확인해야 합니다.
    correctIndex: 0
metaTitle: OAuth 2.0이란? 초보자를 위한 인증 가이드 | 비유로 쉽게 이해하기
metaDescription: 바이브코딩과 AI 코딩 시대에 반드시 알아야 할 인증의 기본 개념과 OAuth 2.0의 동작 원리를 호텔 카드키, 발레파킹 비유로 쉽게 설명합니다.
