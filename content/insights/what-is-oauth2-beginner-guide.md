...
title: 'OAuth 2.0 뜻: 초보자를 위한 인증 이해하기'
excerpt: >-
...
quiz:
  - options:
    ...
    explanation: >-
      ...
      가장 먼저 확인해야 합니다.
    correctIndex: 0
metaTitle: 'OAuth 2.0 뜻: 초보자를 위한 인증 가이드, 비유로 쉽게 이해하기'
metaDescription: >-
  OAuth 2.0은 비밀번호를 넘기지 않고 필요한 권한만 기한을 정해 빌려주는 인가 방식입니다. 인증과 인가의 차이, API 키와 세션,
  JWT를 거친 발전 과정, 호텔 카드키 비유로 본 동작 원리, AI가 만든 코드에서 확인할 항목을 정리했습니다.
...
summary3:
  - 인증은 접속한 사람이 누구인지 확인하는 일이고 인가는 그 사람에게 어떤 권한을 줄지 정하는 일입니다.
  - 비밀번호 직접 전달에서 API 키와 세션을 거쳐 OAuth 2.0으로 온 이유는 앞 방식마다 권한 구분과 만료가 없었기 때문입니다.
  - OAuth 2.0은 비밀번호를 넘기지 않고 필요한 범위의 권한만 기한을 정해 빌려주는 구조입니다.
---
...
웹 서비스에서의 인증도 같은 원리입니다. "이 요청을 보낸 사람이 정말 권한이 있는 사람인지"를 확인하는 것입니다. 로그인 화면에서 이메일과 비밀번호를 입력하는 것이 가장 기본적인 인증 방식입니다.

여기서 한 가지 더 알아야 할 개념이 있습니다. **인가(Authorization)**입니다.

- **인증(Authentication)**: 누가 접속했는지 확인하는 것: "이 사람이 홍길동이 맞는지"
- **인가(Authorization)**: 확인된 사람에게 어떤 권한을 줄지 결정하는 것: "홍길동은 관리자 페이지에 접근할 수 있는지"
...
### API Key 방식은 이제 쓰면 안 되나요?

지금도 쓰입니다. 권한을 나눌 수 없고 만료가 없다는 한계 때문에 외부 서비스와 권한을 주고받을 때는 맞지 않지만, 내부 서비스끼리 통신하는 구간에서는 여전히 널리 쓰입니다. 다만 키를 코드 파일이 아니라 환경 변수에 두어야 한다는 조건은 OAuth 2.0과 똑같이 적용됩니다.

### 액세스 토큰이 만료되면 사용자가 다시 로그인해야 하나요?

리프레시 토큰이 있으면 다시 로그인하지 않아도 됩니다. 액세스 토큰이 만료되면 리프레시 토큰으로 새 액세스 토큰을 받아 그대로 이어서 쓰기 때문입니다. 리프레시 토큰까지 만료되거나 사용자가 권한을 취소하면 그때는 인증 절차를 처음부터 다시 밟습니다.

### Client Secret을 .env 파일에 넣으면 안전한가요?

그 파일을 Git에 올리지 않는다는 조건에서만 안전합니다. `.env`가 저장소에 함께 올라가면 코드에 직접 적어 둔 것과 결과가 같아서, `.gitignore`에 `.env`가 들어 있는지 같이 확인해야 합니다. 바이브코딩으로 처음부터 만든 프로젝트에는 `.gitignore`가 아예 없는 경우가 있습니다.
