...
title: 비개발자의 크롬 확장 프로그램 배포 기록 A to Z
excerpt: >-
...
quiz:
  - options:
    ...
    explanation: >-
      ...
      의미합니다. API로 텍스트 데이터(JSON)만 주고받는 것은 원격 코드에 해당하지 않습니다. 여기서 Yes를 선택하면 심사가 매우
      까다로워지거나 반려될 수 있습니다.
    correctIndex: 1
thumbnailUrl: >-
  https://avqz4vnz10fk0ilw.public.blob.vercel-storage.com/uploads/1769565798268-0ijdg6.png
metaTitle: 비개발자의 크롬 확장 프로그램 배포 기록 A to Z
metaDescription: >-
  AI 기반 SEO 분석 확장 프로그램 개발 및 Chrome 웹 스토어 배포 과정을 비개발자 관점에서 상세히 다룹니다. 권한 요청 소명, 원격
  코드 사용 여부, 개인정보 처리방침 등 배포 과정의 핵심 팁과 해결 전략을 공유하여, 비개발자도 크롬 확장 프로그램을 배포할 수 있도록
  돕습니다.
ogDescription: >-
  AI로 만든 크롬 확장 프로그램을 웹 스토어에 올리면서 막혔던 권한 소명과 원격 코드 항목, 개인정보 처리방침 URL을 어떻게 넘겼는지
  적었습니다.
...
가장 먼저 해야 하는 것은 확장 프로그램이 사용하는 권한의 '정당성'을 서술하는 것이었습니다. 특히 제가 만든 도구 특성상 사이트 코드에 접근해야 하는데, 구글은 이를 '보안 위험'으로 간주할 수 있으니 상세한 소명을 해야 했습니다.
...
*   **정답:** **No (아니요)**
*   **이유:** 구글이 말하는 '원격 코드'란 외부 서버에서 실행 가능한 스크립트 파일(.js)을 다운로드하여 실행(`eval`)하는 보안 위협을 뜻합니다. API로 텍스트 데이터(JSON)만 주고받는 것은 원격 코드가 아닙니다. 여기서 **Yes를 선택하면 심사가 매우 까다로워지거나 반려**될 수 있습니다.
...
### API를 호출하면 원격 코드를 쓰는 것으로 봐야 하나요?

아닙니다. 구글이 말하는 원격 코드는 외부 서버에서 실행 가능한 스크립트 파일을 내려받아 실행하는 경우를 가리킵니다. API로 JSON 같은 텍스트 데이터만 주고받는다면 아니요로 답하면 되고, 여기서 예를 고르면 심사가 크게 까다로워집니다.

### 개인정보를 수집하지 않아도 처리방침 URL이 필요한가요?

스토어 등록 단계에서는 URL 입력란이 필수입니다. 별도 웹사이트가 없으면 구글 사이트 도구로 빈 페이지를 만들어 서버에 데이터를 저장하지 않고 로컬에서만 처리한다는 내용을 올려 두는 방법이 있습니다. 개인정보를 다룰 일이 없는 도구였지만 저는 제출해 두고 넘어가는 쪽을 택했습니다.

### 권한 소명은 어떤 식으로 적어야 하나요?

사용자가 원할 때만 작동한다는 점을 권한마다 풀어서 적습니다. activeTab은 아이콘을 클릭했을 때만 현재 탭 정보를 읽는다고, scripting은 페이지를 바꾸는 것이 아니라 분석을 위해 읽기만 한다고, storage는 설정을 로컬에 저장하는 용도라고 썼습니다. 권한 이름을 나열하는 것보다 언제 무엇을 읽는지가 드러나야 합니다.
