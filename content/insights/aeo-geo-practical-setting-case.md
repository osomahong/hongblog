...
AEO를 잘한다는 것은 콘텐츠를 잘 쓴다는 뜻이 아닙니다. 실제 작동은 **소스코드 뒤에 숨은 기능들**에서 결정됩니다. `<head>`의 메타 태그, `schema.org` 기반 JSON-LD 구조화 데이터, `sitemap.xml`, `robots.txt`, `llms.txt`, `canonical URL` 같은 요소들을 AI가 먼저 읽습니다. 대부분의 작성자가 본문만 손본 채 "AEO가 안 걸린다"고 말하는 이유는 여기에 있습니다.
...
AEO(Answer Engine Optimization)는 질문에 **직접 답변**을 주도록 콘텐츠를 구조화하는 작업입니다. ChatGPT, Perplexity, 음성 검색처럼 "답을 돌려주는" 엔진이 콘텐츠를 인용하거나 요약할 수 있도록 만드는 것이 목표입니다. 핵심 신호는 질문형 제목, Q&A, FAQ 구조화 데이터입니다.
...
두 가지 개념은 겹치지만 초점이 다릅니다. AEO는 답변 형식에, GEO는 인용 가능성에 집중합니다. 실무에서는 하나의 파이프라인으로 같이 처리하는 것이 효율적입니다.
...
이 개념이 아직 낯설 수 있습니다. 코드를 직접 들여다본 적이 없는 분들도 따라올 수 있도록, 준이아빠블로그가 실제로 내보내고 있는 코드를 그대로 펼쳐 보여드리겠습니다.
...
## `<head>` 태그에 들어가는 메타 태그 내용들은 어떤 역할을 할까요?
...
준이아빠블로그의 핵심 자동화는 여기에 있습니다. `src/app/insights/[slug]/page.tsx:83-128`의 `extractFaqPairs()` 함수는 빌드 타임에 본문을 파싱하여 **물음표로 끝나는 H2**를 질문으로, 다음 단락을 답변으로 묶습니다. 2개 이상 Q&A가 추출되면 FAQPage JSON-LD가 자동으로 발행됩니다.
...
이 글에는 지금 물음표로 끝나는 H2가 9개 있고, 자주 묻는 질문 섹션의 H3가 3개 더 있습니다. 따라서 12개 Q&A가 FAQPage JSON-LD로 자동 발행됩니다. **작성자는 H2 종결 톤만 통일하면 구조화 데이터가 따라오는 구조**입니다.
...
llms.txt는 AI 검색 시대에 새로 등장한 개념이라 초보자 입장에서 낯설 수 있습니다. 준이아빠블로그에 실제 배포된 파일을 발췌해, 어떤 형식이고 무엇을 담고 있는지부터 먼저 확인해 보시겠습니다.
...
상승은 일회성 이벤트가 아니라 계단식 구조입니다. 2월 말 FAQPage 자동 스키마 도입 직후 1차 상승, 3월 말 일시 하락 이후 sitemap priority 조정과 llms.txt 배포가 반영된 4월 둘째 주부터 재상승. **세팅을 한 번에 몰아서 한 것이 아니라 구간마다 기능을 하나씩 추가했고, 추가할 때마다 곡선이 계단을 밟듯 올라왔습니다**. 같은 본문을 쓰더라도 보이지 않는 기능들이 붙느냐 붙지 않느냐에 따라 결과가 이렇게 달라집니다.
...
1. **H2를 질문형으로 바꾸세요.** 가장 비용이 낮고 효과가 즉각적입니다. 기존 글의 H2만 고쳐도 1~2주 안에 서치콘솔 노출이 움직이기 시작합니다.
...
이 다섯 단계는 누적입니다. 하나씩 쌓을수록 다음 단계의 효과가 더 크게 나타납니다. 본문을 잘 쓰려는 노력은 그 다음 순서에 옵니다. **보이지 않는 기능들부터 맞춰 두면, AI는 같은 본문을 다른 신호로 받아들입니다**.

## 자주 묻는 질문

### SEO가 부실한데 AEO부터 해도 될까요?

순서를 바꾸면 효과가 나오지 않습니다. AI가 인용하는 출처는 결국 검색 엔진에 색인된 문서라서, 색인되지 않은 페이지는 답변 후보 목록에 오르지도 못합니다. 메타 태그와 canonical, sitemap을 먼저 맞춰 두고 그 위에 질문형 헤딩과 구조화 데이터를 얹는 편이 빠릅니다.

### metaTitle과 og:title을 같은 문구로 써도 되나요?

동작에는 문제가 없지만 채널 하나를 포기하는 선택입니다. 검색 결과에 걸리는 제목은 키워드가 앞에 와야 하고, 공유 카드에 걸리는 제목은 열어 볼 이유가 한 줄에 담겨야 해서 필요한 문구가 서로 다릅니다. 두 값을 나눠 적으면 검색과 공유 양쪽의 클릭률을 각각 관리할 수 있습니다.

### llms.txt를 올리면 AI가 반드시 읽어 가나요?

보장되지는 않습니다. llms.txt는 표준 규격이 아니라 2024년 말부터 퍼진 비공식 규약이고, 일부 AI 업체가 존중하는 것으로 알려져 있는 정도입니다. 다만 이 파일이 없으면 크롤러는 사이트 주제를 파악하려고 본문을 수백 개 읽어야 하므로, 들이는 시간 대비 얻는 것이 큰 쪽에 속합니다.
