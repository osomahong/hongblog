...
metaDescription: >-
  클로드 업무 활용은 같은 자료와 지시사항을 프로젝트에 묶어 두고 반복 업무를 맡기는 방식입니다. 반복 보고서, 문서 정리, 회의록 정리,
  팀 공유 4가지 업무 활용 사례와 설정 순서를 공식 문서 기준으로 정리했습니다.
ogDescription: >-
...
클로드 업무 활용은 같은 자료와 지시사항을 [프로젝트](/class/claude-in-practice/what-are-claude-projects)에 묶어 두고 반복 업무를 맡기는 방식입니다. 클로드를 업무에 쓰기 시작하면 대부분 여기서 달라집니다. 매번 새 대화를 열고 처음부터 상황을 설명하는 사람과, 한 번 정리해 둔 자리 위에서 바로 일을 시작하는 사람으로 나뉩니다.

이 글에서는 클로드 웹과 앱에서 쓰는 프로젝트 기능을 기준으로, 업무에서 자주 나오는 활용 사례 4가지와 설정 순서를 정리합니다. 데스크톱에서 파일을 직접 다루는 [코워크](/insights/claude-cowork-what-you-can-do) 이야기는 다루지 않습니다.
...
프로젝트 하나에는 200K [컨텍스트 윈도우](/class/claude-fundamentals/context-window-explained)가 주어집니다. 앤트로픽은 이를 500쪽짜리 책 한 권 분량으로 설명합니다. 유료 플랜에서는 이 한도에 가까워지면 검색 증강 생성(RAG) 방식이 작동해 처리 용량을 최대 10배까지 넓힙니다.
...
### 무료 계정에서도 클로드 프로젝트를 쓸 수 있나요?

쓸 수 있습니다. 무료 계정도 프로젝트를 만들 수 있고 개수만 최대 5개로 묶여 있습니다. 다만 프로젝트를 조직 구성원과 공유하는 기능은 Team과 Enterprise 플랜에서 열리고, 컨텍스트 한도에 가까워졌을 때 처리 용량을 넓히는 검색 증강 생성도 유료 플랜에서 작동합니다.

### 프로젝트 하나에 여러 업무를 함께 넣어도 되나요?

목적별로 나누는 쪽이 안전합니다. 보고서용과 자료 조사용, 글쓰기용을 한자리에 두면 지시사항이 서로 부딪히고, 성격이 다른 문서가 섞이면 클로드가 어느 쪽을 기준으로 답할지 모호해집니다. 지금 쓰지 않는 지난 양식을 함께 두는 것도 같은 문제를 만듭니다.

### 일반 대화창에서 나눈 이야기가 프로젝트로 이어지나요?

이어지지 않습니다. 클로드는 프로젝트마다 메모리를 따로 만들기 때문에 일반 대화창의 맥락이 프로젝트 안으로 넘어오지 않고 프로젝트끼리도 서로 넘어가지 않습니다. 한 주제를 계속 다룰 생각이라면 처음부터 프로젝트 안에서 대화를 여는 편이 낫습니다.
