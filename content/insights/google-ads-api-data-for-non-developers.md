...
metaDescription: >-
  구글 Ads API는 광고 계정의 구조와 성과, 분류 데이터를 가져오는 통로입니다. 마케터가 보는 지표와 API 필드 매핑, GAQL 조회
  과정, 마이크로 단위 변환, PMax와 디멘드젠의 소재 성과 한계를 정리했습니다.
...
### 전환수가 0.3이나 0.7처럼 소수로 나오는데 잘못 뽑은 값인가요?

정상 값입니다. 데이터 기반 기여 모델에서는 전환 하나가 여러 터치포인트에 나뉘어 배분되므로 계정 화면과 API 모두 소수가 나옵니다. 정수로 맞춰야 하는 리포트라면 `metrics.all_conversions`를 쓰거나 전환 액션별로 따로 집계합니다.

### PMax 캠페인의 소재별 클릭수와 전환수도 API로 가져오나요?

가져올 수 없습니다. 개별 에셋은 BEST, GOOD, LOW 같은 성과 등급만 제공되고 숫자 지표는 에셋 그룹 레벨까지만 옵니다. 그래서 여러 매체를 합친 리포트에서 소재별 비교를 할 때는 구글 쪽을 에셋 그룹 단위로 맞추는 편이 현실적입니다.

### 오늘 데이터가 광고 계정 화면과 맞지 않는데 왜 그런가요?

당일 데이터가 실시간이 아니기 때문입니다. 보통 3시간 정도 지연이 있고 전환은 전환 기간 설정에 따라 최대 90일까지 소급해서 반영됩니다. 여기에 날짜가 계정 타임존 기준으로 끊기므로 다른 매체 데이터와 합칠 때는 타임존을 먼저 맞춰야 합니다.
