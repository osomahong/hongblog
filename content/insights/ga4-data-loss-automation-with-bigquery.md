...
metaTitle: 'GA4 데이터 유실, BigQuery 자동 보정 파이프라인'
metaDescription: >-
  GA4 데이터에서 Page Title 누락 문제를 BigQuery 기반 자동화 파이프라인으로 해결했습니다. 수동 스프레드시트 관리에 따른
  문제점을 분석하고, URL 정규화, STRUCT 구조 수정, 정규표현식 활용 등 기술적 난관을 극복한 과정을 소개합니다.
ogDescription: >-
  스프레드시트로 관리하던 Page Title 맵핑을 BigQuery 파이프라인으로 옮긴 과정입니다. URL 정규화와 이중 조인으로 결측치를
  되살린 방법을 정리했습니다.
...
초기 데이터 분석 환경 구축 시, 구글 스프레드시트(Google Sheets)와 [루커 스튜디오(Looker Studio)](/class/data-analytics-basics/tableau-and-looker-studio)의 조합은 쉽고 유효한 선택지였습니다. 별도의 데이터 웨어하우징 없이도 데이터 소스를 연결하고 시각화할 수 있었기 때문입니다.

외부 플랫폼에서 유입되는 캠페인 트래픽을 분석할 때 일부 플랫폼의 특성상 웹사이트 주소인page_location 값은 존재하지만, [GA4](/class/data-analytics-basics/what-is-ga4) 수집 시점의 이슈로 인해 `Page Title`이 `NULL`로 수집되는 경우가 종종 발생합니다. `Page Title`이 아니더라도 이러한 현상은 전반적 필드에 종종 발생하는데 이를 보완하기 위해 지금까지는 다음과 같은 수동 프로세스를 유지해 왔습니다.
...
따라서 2026년의 시작과 함께, 이러한 수동 의존적인 프로세스를 제거하고 [BigQuery](/class/data-analytics-basics/what-is-bigquery) 기반의 데이터 파이프라인으로 로직을 이관하여 완전 자동화를 구축하는 것을 목표로 삼았습니다.
...
문자열이 일치하지 않아 조인이 실패하는 문제를 해결하기 위해, URL 정규화(Normalization) 전처리를 도입했습니다. BigQuery의 `SPLIT` 함수를 사용하여 `?` 이후의 파라미터를 제거하고, `TRIM` 및 `RTRIM` 함수로 URL 끝의 슬래시(`/`) 유무 때문에 생기는 불일치 케이스를 막았습니다. 그 결과 조인 키(Key)의 일관성을 확보할 수 있었습니다.
...
2.  **Mapping 및 Replace:** 집계된 결과셋에 `REPLACE` 구문을 사용하여 `NULL` 값을 맵핑 테이블의 값으로 치환(`COALESCE`)합니다.

이러한 구조 변경으로 쿼리의 실행 계획(Execution Plan)을 최적화하고 문법적 오류를 해결했습니다.
...
위 정규식으로 URL의 구조가 변경되더라도 핵심이 되는 `{ID}` 값만을 정확하게 추출할 수 있게 되었습니다.

## 4. 로직 고도화: 이중 조인(Dual Join) 전략
정규표현식으로 ID를 추출했는데도, URL 매칭만으로는 해결되지 않는 엣지 케이스들이 존재했습니다. 이를 해결하기 위해 **이중 조인(Dual Join)** 전략을 수립하여 맵핑 성공률을 극대화했습니다.
...
SQL 내에서 `LEFT JOIN`을 두 번 수행하고, `COALESCE(Map_URL.title, Map_ID.title, Original.title)` 함수로 우선순위에 따라 값을 적용하도록 구현했습니다. 이 전략으로 파생 페이지나 예외적인 URL 패턴에서도 정확한 캠페인명을 복구할 수 있게 되었습니다.
...
*   관리자가 수동으로 입력한 데이터 또한 `MERGE` 로직이 보존하거나 갱신합니다.
...
### 결측치 보정은 어느 단계에서 하는 것이 좋나요?

시각화 단계가 아니라 데이터 적재 단계입니다. 루커 스튜디오의 혼합 테이블로 보정하면 조회할 때마다 조인 연산이 붙어 대시보드가 느려지고, 스프레드시트를 수기로 관리하면 담당자가 갱신을 빠뜨리는 순간 데이터가 사라집니다. 무거운 조인을 마트 적재 시점으로 옮기면 조회 성능과 데이터 정합성을 함께 확보할 수 있습니다.

### URL로 조인했는데 매칭률이 낮으면 무엇부터 확인하나요?

URL 뒤에 붙은 쿼리 파라미터부터 봅니다. 같은 페이지라도 utm이나 _gl 값이 붙으면 문자열이 달라져 조인이 실패하기 때문에, SPLIT 함수로 물음표 뒤를 잘라 내고 TRIM과 RTRIM으로 끝 슬래시 유무까지 맞춘 뒤 조인 키를 만듭니다. 경로가 불규칙해서 이 방법으로도 남는 건은 정규표현식으로 ID만 뽑아 냅니다.

### 맵핑 테이블에 값 자체가 없을 때는 어떻게 하나요?

수동으로 주입할 보조 경로를 따로 둡니다. 과거 로그에 한 번도 정상 수집된 적이 없는 콜드 스타트 건은 자동 로직으로 되살릴 수 없어서, 관리자가 직접 넣은 값을 다음 적재가 지우지 않도록 CREATE OR REPLACE 대신 MERGE 문을 씁니다. 이렇게 해 두면 한 번 넣은 값이 다음 스케줄링부터 전체 데이터에 자동으로 적용됩니다.
