...
외부 플랫폼에서 유입되는 캠페인 트래픽을 분석할 때 일부 플랫폼의 특성상 웹사이트 주소인page_location 값은 존재하지만, GA4 수집 시점의 이슈로 인해 `Page Title`이 `NULL`로 수집되는 경우가 종종 발생합니다. `Page Title`이 아니더라도 이러한 현상은 전반적 필드에 종종 발생하는데 이를 보완하기 위해 지금까지는 다음과 같은 수동 프로세스를 유지해 왔습니다.
