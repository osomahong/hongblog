...
### 광고 관리자 숫자와 API로 받은 값이 다른 이유는 무엇인가요?

귀속 기간과 집계 기준이 서로 달라서 생기는 경우가 많습니다. API에서 action_attribution_windows를 지정하지 않으면 광고 관리자에 설정된 기간과 어긋나고, 광고 관리자의 "결과"는 캠페인 목표에 따라 다른 이벤트를 집계하는데 API에는 그 자동 매핑이 없습니다. 원하는 action_type을 직접 골라 꺼내고 귀속 기간도 함께 지정하면 차이가 줄어듭니다.

### 클라이언트 광고 계정 데이터도 바로 가져올 수 있나요?

바로는 어렵습니다. Standard Access는 본인 계정 데이터까지만 열리고, 다른 사업자의 광고 계정을 읽으려면 Advanced Access가 필요합니다. 여기에는 사업자 등록증이나 법인 서류를 내는 비즈니스 인증과 앱 검수가 따라붙고 처리에 수 영업일이 걸립니다.

### 토큰이 자꾸 만료되는데 어떻게 해야 하나요?

시스템 유저 토큰으로 바꾸면 됩니다. 개인 사용자 토큰은 길어도 60일이면 만료되어 자동화 파이프라인이 갑자기 멈추는 원인이 됩니다. Business Manager의 비즈니스 설정에서 시스템 사용자를 추가하고 광고 계정을 자산으로 연결한 뒤 필요한 권한으로 토큰을 만들면 수동 갱신 없이 씁니다.
