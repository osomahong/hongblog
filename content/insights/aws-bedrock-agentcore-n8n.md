...
### n8n 기본 AI Agent 노드만 써도 되는 경우는 언제인가요?

기억이 필요 없는 작업이라면 기본 노드로 충분합니다. 단발성 요약이나 알림처럼 한 번의 실행 안에서 끝나는 일은 AgentCore를 붙여도 얻는 것이 적고 AWS 설정이라는 관문만 늘어납니다. 같은 상대를 이어서 응대하거나 웹 화면을 직접 열어 확인해야 하는 반복 업무일 때 차이가 드러납니다.

### 쓰지 않으면 비용이 들지 않나요?

harness 자체에는 별도 요금이 없고 실행한 리소스만큼만 과금되므로 실험 단계의 부담은 크지 않습니다. 다만 런타임과 게이트웨이, 메모리, 모델 추론의 과금 단위가 서로 달라서 에이전트가 브라우저와 코드 실행기를 얼마나 자주 켜는지에 따라 총액이 달라집니다. 반복 실행되도록 걸어 두면 비용이 조용히 쌓이니 관측 화면에서 호출 수와 토큰 사용량을 초기에 확인해 두는 편이 낫습니다.

### n8n만 써 온 사람이 가장 막히는 곳은 어디인가요?

노드 설치가 아니라 AWS 쪽 설정입니다. AWS 계정과 기능에 맞는 IAM 권한, 노드에 등록할 실행 역할을 먼저 갖춰야 하는데 권한 이름을 하나씩 찾는 일이 만만치 않습니다. 공식 저장소 문서에 기능별로 필요한 권한이 정리되어 있으니 그 목록대로 요청하거나 설정하면 시간을 줄일 수 있습니다.
