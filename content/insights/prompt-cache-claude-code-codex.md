...
title: '클로드 코드(Claude Code)와 코덱스 프롬프트 캐시 정리: 작업을 몰아서 해야 하는 이유'
excerpt: >-
...
metaTitle: '클로드 코드(Claude Code)와 코덱스 프롬프트 캐시 정리: 비용 절감 조건'
metaDescription: >-
...
summary3:
  - 프롬프트 캐시는 이미 보낸 앞부분의 계산 결과를 서버가 보관했다가 다음 요청에서 다시 쓰는 기능입니다.
  - '앤트로픽은 캐시를 쓰면 비용이 최대 90퍼센트, 지연 시간이 최대 85퍼센트 줄어든다고 밝혔습니다.'
  - 앞부분이 1바이트만 달라져도 뒤가 전부 무효가 되므로 모델 변경과 MCP 서버 추가는 세션을 시작하기 전에 몰아서 처리합니다.
---

프롬프트 캐시는 AI에 이미 보낸 앞부분의 계산 결과를 서버가 보관했다가 다음 요청에서 재사용하는 기능입니다. [클로드 코드](/insights/claude-code-practical-guide)나 [코덱스](/insights/codex-cli-mac-easy-setup-guide)로 같은 작업을 해도 어떤 날은 싸고 빠르며 어떤 날은 비싸고 느린 이유가 대부분 여기에 있습니다.
...
### 구독 요금제를 쓰면 캐시가 깨져도 상관없나요?

청구서에 달러가 찍히지 않을 뿐 주간 사용 한도는 그대로 줄어듭니다. 위 계산에서 달러를 한도로 바꿔 읽으면 되는데, 같은 버그 하나를 잡아도 몰아서 한 쪽은 한도 1인분을 쓰고 매번 끊어서 한 쪽은 12인분을 씁니다. 많이 쓰지 않았는데 한도가 찼다면 작업량보다 작업을 끊은 횟수를 먼저 살펴봅니다.

### 캐시를 살리려고 대화를 계속 이어 가는 편이 나은가요?

주제가 바뀌었다면 이어 가지 않는 편이 낫습니다. 캐시 읽기가 원가의 10분의 1이어도 매 턴 읽어야 할 토큰은 계속 늘고, 오픈AI 쪽은 입력이 일정 규모를 넘으면 세션 전체에 긴 컨텍스트 단가가 붙습니다. 앞의 대화를 AI가 참고해야 하는지만 보고 이어 갈지 비울지 정합니다.

### 같은 질문인데 응답이 갑자기 느려지면 무엇을 의심하나요?

대개 캐시가 깨진 상태입니다. 캐시를 읽으면 입력 전체를 모델이 훑는 프리필 단계를 건너뛰는데, 컨텍스트가 큰 세션에서는 이 단계가 대기 시간의 대부분을 차지합니다. 직전에 모델을 바꿨는지, MCP 서버를 넣거나 뺐는지, 자리를 오래 비웠는지를 되짚어 보면 원인이 잡힙니다.
