...
summary3:
  - '콘텐츠 SEO 전략은 검색엔진에 맞춰 글을 다듬는 작업이 아니라 어떤 주제를 몇 편이나 어떤 형식으로 쓸지 미리 정해 두는 일입니다.'
  - '검색으로 들어오는 사람은 형식이 맞으면 읽는 인지층과 다시 찾아오는 관심층, 실제로 문의하는 핵심층으로 나뉩니다.'
  - '글마다 힘을 고르게 쓰지 말고 분기 작업량의 30퍼센트를 앵커 글 한 편에 몰아넣는 편이 사이트 전체를 끌어올립니다.'
---

콘텐츠 SEO 전략은 무엇을 쓸지 정하는 기준입니다. 검색엔진에 맞춰 글을 다듬는 작업이 아니라, 어떤 주제를 몇 편이나 어떤 형식으로 쓸지 미리 정해 두는 운영 방법을 뜻합니다.

글은 꾸준히 쓰는데 검색 유입이 늘지 않는 상황이 있습니다. 이때 대개 [색인](/class/seo-fundamentals/what-is-indexing)이나 사이트맵 같은 기술적인 부분을 먼저 점검하게 됩니다. 그런데 기술 점검을 다 끝내도 유입이 그대로인 경우가 있습니다. 무엇을 쓸지 정하는 기준이 없으면 기술 문제를 다 고쳐도 글이 검색어를 잡지 못하기 때문입니다.
...
같은 논문에서 구글 연구진은 랭킹 모델을 클릭 확률이 아니라 기대 시청 시간을 예측하도록 학습시킨다고 밝혔습니다. 이유도 함께 적혀 있습니다. [클릭률](/class/digital-marketing-terms/what-is-ctr)로 순위를 매기면 사람들이 끝까지 보지 않는 낚시성 영상이 올라가기 때문입니다.
...
- **본문 안에 [내부 링크](/class/seo-fundamentals/what-is-internal-link)를 둡니다.** 글 맨 아래 관련 글 목록만으로는 잘 눌리지 않습니다.
...
> **AI 검색에 인용되기 위한 별도 기법은 없습니다. 하위 질문을 빠짐없이 덮고, 각 문단이 혼자서도 읽히게 쓰는 것이 인용으로 이어집니다.**
...
### 노출은 되는데 순위가 오르지 않으면 키워드를 더 넣어야 하나요?

키워드를 더 넣어도 잘 움직이지 않습니다. 노출이 잡혔다는 것은 색인에서 후보로 꺼내지는 앞 단계를 이미 통과했다는 뜻이고, 순위를 정하는 뒤 단계는 사용자 행동을 봅니다. 이 상태에서 고칠 곳은 제목이 약속한 답이 첫 화면에 있는지입니다.

### 아웃라이어가 나왔을 때 새 글과 갱신 가운데 무엇을 먼저 하나요?

이미 신호가 쌓인 글을 갱신하는 쪽이 먼저입니다. 클릭 데이터가 13개월 단위로 집계된다는 내용이 재판 과정에서 공개됐는데, 백지에서 시작하는 글보다 기존 페이지가 빠르게 움직입니다. 갱신 내용이 실질적이면 수정 시점을 기록해 재크롤링 우선순위를 올립니다.

### AI 검색에 인용되려면 별도 기법이 필요한가요?

필요하지 않습니다. 구글은 AI 검색을 위한 별도 최적화 기법이 없고 기존 SEO면 된다고 밝혔습니다. 대신 한 주제를 하위 질문 8개에서 12개로 나눠 소제목으로 배치하고 각 섹션이 앞 내용을 읽지 않아도 단독으로 읽히게 쓰면, 조각 단위로 잘렸을 때도 뜻이 살아 있습니다.
