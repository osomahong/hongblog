...
ogDescription: >-
  ...
  기록입니다.
summary3:
  - '준이아빠블로그는 Next.js 16과 Neon PostgreSQL, Vercel 조합으로 만들었고 프런트엔드부터 배포까지 Claude Code와의 대화로 구축했습니다.'
  - '콘텐츠 생성과 검수, SEO 최적화, 배포를 서브에이전트 넷과 스킬로 나눠 글 하나를 기획부터 배포까지 30분 안에 처리합니다.'
  - 'Claude Code는 코드베이스를 이해한 상태로 일해서 프로젝트가 커질수록 일관성에 유리하지만 디자인 감각과 사업 판단은 사람 몫으로 남습니다.'
---

## 개발자가 아닌 사람이 직접 블로그를 만든다는 것

준이아빠블로그(digitalmarketer.co.kr)는 디지털 마케터인 제가 [Claude Code](/class/claude-code-for-everyone/what-is-claude-code)를 사용해 직접 만들었습니다. 프론트엔드, 백엔드, 데이터베이스, 배포까지 전부 Claude Code와의 대화로 구축했고, 지금도 같은 방식으로 운영하고 있습니다. 개발을 업으로 삼은 적은 없습니다.
...
2026년 2월 기준으로 Claude Code는 출시 6개월 만에 연간 환산 매출(ARR) 10억 달러를 돌파했고, GitHub Copilot과 Cursor를 제치고 가장 많이 사용되는 AI 코딩 에이전트가 되었습니다. Pragmatic Engineer의 2026년 2월 설문에서는 엔지니어링 팀의 73%가 AI 코딩 도구를 매일 사용한다고 답했습니다. 이 글이 그 흐름 속에서 "개발자가 아닌 사람은 어떻게 활용하는지" 보여주는 하나의 실전 사례가 되었으면 합니다.
...
준이아빠블로그의 CLAUDE.md에는 다음과 같은 내용이 담겨 있습니다.
...
준이아빠블로그의 운영을 실질적으로 자동화하는 것은 **서브에이전트 시스템**입니다. Claude Code는 하나의 에이전트가 모든 것을 처리하는 것이 아니라, 전문 역할별로 서브에이전트를 나눠서 작업할 수 있습니다.

현재 준이아빠블로그에서 운영 중인 에이전트 구성은 다음과 같습니다.
...
블로그를 "감"이 아니라 "데이터"로 운영하기 위해 GA4(Google Analytics 4) 데이터를 적극 활용하고 있습니다. GA4 Data API로 트래픽 데이터를 직접 조회하고, 이를 콘텐츠 전략에 반영합니다.
...
**비즈니스 맥락을 모릅니다.** "마케터가 읽으면 유용한 글"이라는 판단은 제가 내려야 합니다. Claude Code는 기술적으로 완벽한 코드를 작성할 수 있지만, "이 기능이 사용자에게 정말 필요한지"라는 질문에는 답하지 못합니다.
...
### 서브에이전트를 굳이 역할별로 나눠야 하나요?

나눠 두면 부분 실행과 비용 조절이 함께 가능해집니다. 검수만 필요할 때 content-reviewer만 돌리고, SEO 필드만 훑을 때 seo-manager만 돌리는 식으로 쓸 수 있기 때문입니다. 토픽 추천이나 SEO 분석처럼 가벼운 작업에 경량 모델을 배정하고 생성과 검수에만 상위 모델을 붙이면 품질을 유지하면서 비용이 내려갑니다.

### 세션이 길어지면 초반에 말한 내용을 놓치던데 어떻게 하나요?

작업을 3~5단계로 쪼개고 단계마다 커밋해 두는 방식이 기본입니다. 초반에 합의한 조건이 흐려져도 직전 커밋으로 돌아갈 수 있어서 되돌리는 비용이 작습니다. 100만 토큰 컨텍스트 윈도우가 지원되면서 이 문제 자체는 많이 나아졌지만, 한 세션에 모든 작업을 몰아넣지 않는 습관은 여전히 필요합니다.

### CLAUDE.md에는 처음부터 무엇을 적어야 하나요?

프로젝트의 목적과 쓰는 기술, 지켜야 할 규칙 정도면 시작으로 충분합니다. 완성된 설명서를 먼저 만들려 하면 착수가 늦어지므로 짧게 적어 두고 작업하면서 늘리는 편이 낫습니다. 같은 지시를 두 번 하게 됐다면 그 내용이 아직 파일에 없다는 뜻이라 그때 한 줄씩 옮기면 됩니다.
...
- **만드는 것보다 운영하는 것이 핵심입니다.** 에이전트 시스템과 스킬로 반복 업무를 자동화하면, 블로그 운영에 드는 시간을 크게 줄일 수 있습니다.
