...
metaDescription: >-
  Performance Max는 키워드와 잠재고객을 직접 고를 수 없지만 AI의 학습 방향을 유도하는 간접 제어 수단이 있습니다. 잠재고객
  시그널, 제외 키워드와 게재 위치 제외, 에셋 그룹 분리, 피드 온리 캠페인, 채널별 성과 보고서 활용까지 정리했습니다.
...
summary3:
  ...
  - '잠재고객 시그널은 그 대상에게만 노출하라는 지시가 아니라 학습의 출발점이고 실제 구매 고객 목록이 가장 강한 신호입니다.'
  - '2025년 1월 도입된 캠페인 수준 제외 키워드는 3월에 10,000개로 늘었고 8월에는 여러 캠페인에 함께 적용하는 공유 목록이 더해졌습니다.'
---
...
- **자동 생성 에셋을 반드시 꺼야 합니다.** 캠페인 설정에서 "자동으로 생성된 애셋"(Automatically Created Assets) 옵션이 사용 설정돼 있으면, Google이 자동으로 헤드라인과 설명을 생성하여 Shopping 외 채널에도 노출됩니다.
...
### 잠재고객 시그널을 걸면 그 사람들에게만 광고가 나가나요?

아닙니다. 시그널은 AI에게 어디서부터 테스트할지 알려 주는 출발점이고, 더 높은 전환율을 보이는 집단을 찾으면 시그널 밖으로 확장합니다. 그래서 전환이 시그널과 다른 곳에서 나오는 상황은 실패가 아니라, 실제 전환 데이터로 시그널을 갱신할 시점이라는 뜻입니다.

### 브랜드 제외를 켜면 자사 브랜드 검색에 광고가 아예 나가지 않나요?

Shopping 광고는 계속 노출됩니다. 브랜드 제외는 검색 텍스트 광고에서 자사 브랜드 키워드를 빼는 설정이라 쇼핑 노출까지 막지는 않습니다. 브랜드 검색을 별도 검색 캠페인으로 관리하면서 Pmax 성과를 신규 유입 기준으로 보고 싶을 때 쓰는 옵션입니다.

### 피드 온리 캠페인을 만들었는데 디스플레이에 노출되면 무엇을 확인하나요?

캠페인 설정의 자동 생성 에셋 옵션부터 확인합니다. 이 옵션이 켜져 있으면 헤드라인과 설명을 Google이 알아서 만들어 붙이기 때문에, 에셋을 넣지 않았어도 Shopping 밖 채널에 광고가 생성됩니다. 피드 온리는 Google이 공식으로 권장하는 방식이 아니라 우회 전략이라서 정책이 바뀌면 동작이 달라질 수 있다는 점도 함께 봅니다.
