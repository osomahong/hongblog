...
### Claude Code는 어떤 요금제부터 쓸 수 있나요?

Pro 플랜부터 포함됩니다. 무료 플랜에서는 열리지 않습니다. API 키를 쓰는 방법도 있는데, 이 경우에는 구독료 대신 쓴 만큼 토큰 요금이 청구됩니다.

### Cursor와 무엇이 다른가요?

일하는 곳이 다릅니다. Cursor는 편집기 안에서 코드를 고치는 쪽이고, Claude Code는 터미널에서 프로젝트 전체를 놓고 파일을 읽고 만들고 명령을 실행합니다. 편집기에서 쓰고 싶으면 VS Code 확장을 따로 설치하는 경로도 있습니다.

### 코드를 모르는 사람도 쓸 수 있나요?

쓸 수 있지만 결과를 확인할 수 있는 범위 안에서 쓰는 편이 안전합니다. 명령은 한국어로 내려도 되고, 무엇을 고쳤는지 설명도 받습니다. 다만 실행 결과가 맞는지는 사람이 봐야 하므로, 처음에는 되돌릴 수 있는 연습용 폴더에서 시작하는 것을 권합니다.
