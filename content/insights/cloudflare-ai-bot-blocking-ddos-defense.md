...
highlights:
  - robots.txt만 두고 막았다고 판단하지 않습니다. 지키지 않는 크롤러가 있으므로 서버 앞단 규칙을 함께 사용 설정합니다.
...
quiz:
  - question: robots.txt에 크롤러 차단을 적어 두었는데도 해당 봇의 접속이 계속 잡힌다면 무엇을 먼저 확인할까요?
    options:
      - 서버 앞단에서 접속 자체를 막는 규칙이 사용 설정돼 있는지 확인한다
...
summary3:
  - 'AI 크롤러 차단은 robots.txt에 적어 두는 부탁을 넘어 사이트 앞단에서 접속 자체를 끊는 설정입니다.'
  - '클라우드플레어 레이더 기준 2026년 8월 20일 직전 7일 동안 전 세계 HTTP 요청의 35.3퍼센트가 봇이었습니다.'
  - 'IP만 막으면 VPN으로 우회되므로 신원과 행동을 함께 보는 규칙으로 넘어가야 합니다.'
---

AI 크롤러 차단은 [robots.txt](/class/seo-fundamentals/what-is-robots-txt)에 적어 두는 부탁을 넘어, 사이트 앞단에서 접속 자체를 끊는 설정입니다. 사이트를 운영해 본 분이라면 사람이 아닌 봇과 AI 트래픽이 최근 크게 늘고 있다는 것을 눈치채셨을 겁니다. [검색 엔진 크롤러](/class/seo-fundamentals/what-is-search-crawler)가 페이지를 가져가고, 문의 폼에 광고 문구가 들어오고, 로그에 낯선 사용자 에이전트가 찍힙니다.
...
마지막 항목이 사이트 운영자에게 가장 직접적인 손해로 돌아옵니다. 검색 엔진 크롤러는 [색인](/class/seo-fundamentals/what-is-indexing)을 만들고 검색 결과에 링크를 걸어 방문을 돌려보냈는데, 학습이나 답변 생성을 목적으로 하는 크롤러에서는 그런 대가가 없거나 아주 적습니다.
...
![클라우드플레어의 AI 크롤러 제어 설정. robots.txt로 AI 봇 트래픽을 관리하는 토글이 사용 설정돼 있고, 파일을 만들거나 갱신하며 검색 최적화에는 영향이 없다는 설명이 붙어 있다](/images/insights/cloudflare-ai-bot-blocking-ddos-defense/managed-robots-txt.png)

**2. AI 크롤러 분류별 차단.** 앞에서 본 세 가지 분류 설정입니다. 안내문을 무시하는 크롤러도 여기서는 접속 단계에서 걸립니다.

**3. Bot Fight Mode.** 무료 플랜에서 쓰는 기본 봇 대응 기능입니다. 알려진 봇 패턴에 걸리는 요청을 자동으로 처리하며, 사용 여부를 정하는 토글 하나로 되어 있습니다. 봇 종류별로 대응을 나눠 정하려면 유료 플랜의 Super Bot Fight Mode를 써야 하고, 요청마다 점수를 매기는 Bot Management는 엔터프라이즈 플랜에 들어갑니다.
...
여기까지 사용 설정해 두어도 계속 들어오는 접속이 있는데, 차단당하면 다른 IP로 갈아타는 봇입니다. 문 앞 경비가 옷차림만 보고 사람을 가린다면 옷을 갈아입고 다시 오면 그만이듯, IP만 보고 막는 방어는 상대가 IP를 바꾸는 순간 풀립니다.
...
**4. 그래도 서버가 버티지 못하면 Under Attack Mode를 사용 설정합니다.** 클라우드플레어의 7계층 DDoS 완화 기능으로, 사용 설정하면 방문자마다 짧은 확인 화면을 거칩니다. 공식 문서는 이 기능을 마지막 수단으로 설명하면서 실제로 공격받는 동안에만 쓰라고 안내합니다. 자바스크립트를 실행해야 통과하는 구조라서 API 호출과 외부 연동, 모니터링 도구가 함께 막히고, 외부 분석 도구의 수치도 영향을 받습니다. 상시로 사용하는 설정이 아닙니다.
...
1. **원본 서버 주소를 숨깁니다.** CDN을 앞에 두어도 서버의 실제 IP가 노출되어 있으면 공격이 그리로 곧장 들어옵니다. 방화벽에서 CDN 대역의 접속만 받도록 설정합니다. 앞문에 경비를 세우고 뒷문을 열어 두면 의미가 없습니다.
