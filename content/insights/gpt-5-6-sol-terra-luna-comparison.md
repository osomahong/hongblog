---
slug: gpt-5-6-sol-terra-luna-comparison
title: 'GPT-5.6 Sol, Terra, Luna 차이와 가격 비교'
excerpt: >-
  GPT-5.6은 OpenAI가 2026년 7월 출시한 3단계 모델 제품군으로, 최상위 Sol, 균형형 Terra, 경량 Luna로
  구성됩니다. 세 모델의 차이와 두 차례 가격 인하 이력, 요금제별 사용 범위, Claude와 Gemini 경쟁 모델과의 가격과 벤치마크
  비교를 정리했습니다.
category: AI_TECH
tags:
  - AI
  - API
  - 자동화
publishedAt: '2026-07-31T00:00:00.000Z'
updatedAt: '2026-09-14T00:00:00.000Z'
highlights:
  - '기본값은 Terra로 잡고, 대량 반복 작업은 Luna로, 복잡한 추론과 에이전트 작업만 Sol로 올리는 방식이 비용 효율적입니다.'
  - Sol 인하는 2026년 11월 21일까지의 한시 가격이므로 장기 예산은 정가 기준으로 잡아 두세요.
metaTitle: 'GPT-5.6 Sol, Terra, Luna 차이와 가격 비교'
metaDescription: >-
  GPT-5.6은 OpenAI의 3단계 모델 제품군으로 최상위 Sol, 균형형 Terra, 경량 Luna로 나뉩니다. 세 모델의 차이,
  2026년 8월 Sol 가격 인하 내용, ChatGPT 요금제별 사용 범위, Claude와 Gemini 대비 가격과 벤치마크를 정리했습니다.
ogDescription: 'GPT-5.6 세 모델의 차이와 두 차례 가격 인하 이력, Claude와 Gemini 경쟁 모델과의 가격, 벤치마크 비교를 정리했습니다.'
ogImage: /og/gpt-5-6-sol-terra-luna-comparison.png
quiz:
  - question: GPT-5.6 세 모델 가운데 대량 텍스트 분류나 반복 자동화처럼 호출 횟수가 많은 작업에 가장 적합한 모델은 무엇일까요?
    options:
      - 'Luna: 세 모델 중 가장 저렴하고 빨라서 대량 호출 작업에 적합하다'
      - 'Terra: 성능과 비용의 균형이 좋아 모든 작업의 기본값이다'
      - 'Sol: 성능이 가장 높아 어떤 작업이든 가장 효율적이다'
    correctIndex: 0
    explanation: >-
      Luna는 입력 100만 토큰당 0.20달러로 세 모델 중 가장 저렴하고 응답 속도도 가장 빠릅니다. 분류, 추출, 요약처럼
      단순하지만 호출 횟수가 많은 작업일수록 Luna의 비용 우위가 커집니다. Sol은 성능이 가장 높지만 입력 단가가 20배라 대량
      호출에는 비효율적입니다.
summary3:
  - 'GPT-5.6은 최상위 Sol과 균형형 Terra, 경량 Luna 세 모델로 이뤄지고 컨텍스트는 셋 다 100만 토큰입니다.'
  - '2026년 8월 25일 기준 100만 토큰당 입력은 Sol 4달러와 Terra 2달러, Luna 0.20달러이고 출력은 20달러와 12달러, 1.20달러입니다.'
  - 'Sol 인하가는 2026년 11월 21일까지의 한시 가격이라 장기 예산은 입력 5달러와 출력 30달러인 정가로 잡습니다.'
---

GPT-5.6은 OpenAI가 2026년 7월 9일 정식 출시한 3단계 AI 모델 제품군입니다. 최고 성능의 Sol(솔), 균형형 Terra(테라), 경량 Luna(루나) 세 모델로 구성됩니다. 태양, 지구, 달이라는 이름 그대로 크기와 역할이 층을 이루는 구조입니다.

출시 이후 가격은 두 번 바뀌었습니다. 7월 30일에 Luna가 80%, Terra가 20% 내려갔고, 8월 21일에는 Sol이 입력 20%, 출력 33% 인하됐습니다. 7월 인하의 배경은 [GPT-5.6 Luna 가격 인하 글](/insights/gpt-5-6-luna-price-cut)에서 따로 다뤘습니다. 이 글에서는 세 모델의 차이와 가격 변동 이력, ChatGPT 요금제별 사용 범위, 그리고 Claude와 Gemini의 경쟁 모델과 견줬을 때의 위치를 정리했습니다.

## GPT-5.6 Sol, Terra, Luna 세 모델의 차이

세 모델은 같은 기반에서 크기와 단가를 달리한 형제 모델입니다. 공통 사양부터 확인하면 다음과 같습니다.

- **컨텍스트 윈도**: 세 모델 모두 100만 토큰입니다. 컨텍스트 윈도는 모델이 한 번에 읽고 기억할 수 있는 분량으로, 100만 토큰이면 장편 소설 7~8권을 한 번에 넣을 수 있는 크기입니다.
- **최대 출력**: 12만 8천 토큰입니다.
- **지식 기준일**: 2026년 2월 16일까지의 정보를 학습했습니다.

성능과 단가에서 차이가 납니다. 2026년 8월 25일 기준으로 실제 청구되는 단가입니다.

| 구분 | Sol | Terra | Luna |
|------|-----|-------|------|
| 위치 | 최상위 | 균형형 | 경량 |
| 입력 (100만 토큰당) | $4.00 | $2.00 | $0.20 |
| 출력 (100만 토큰당) | $20.00 | $12.00 | $1.20 |
| 어울리는 일 | 복잡한 추론, 에이전트 작업, 까다로운 코드 수정 | 문서 요약, 초안 작성, 일반 질의응답 | 대량 분류, 데이터 추출, 단순 변환 |

표의 Sol 단가는 기간이 정해진 한시 가격입니다. 정가는 입력 $5, 출력 $30이고, 인하가는 최소 2026년 11월 21일까지 적용됩니다. 자세한 내용은 바로 다음 섹션에 정리했습니다.

OpenAI는 Terra가 이전 세대 GPT-5.5와 비슷한 성능을 절반 가격에 낸다고 설명합니다. 실제로 뒤에서 볼 벤치마크에서도 Terra는 GPT-5.5와 같은 점수대에 있습니다. 그래서 실무 기본값은 Terra로 잡고, 단순 반복 작업은 Luna로 내리고, 어려운 작업만 Sol로 올리는 구조가 자연스럽습니다.

## GPT-5.6 가격 변동 이력과 Sol 한시 인하

출시 이후 두 달 사이에 단가가 두 번 조정됐습니다. 순서대로 정리하면 다음과 같습니다. 모두 100만 토큰당 미국 달러 기준입니다.

| 시점 | 조정 내용 | Sol (입력/출력) | Terra (입력/출력) | Luna (입력/출력) |
|------|----------|----------------|------------------|-----------------|
| 2026년 7월 9일 출시 | 최초 가격 | $5 / $30 | $2.50 / $15 | $1 / $6 |
| 2026년 7월 30일 | Luna 80%, Terra 20% 인하 | $5 / $30 | $2 / $12 | $0.20 / $1.20 |
| 2026년 8월 21일 | Sol 입력 20%, 출력 33% 인하 | $4 / $20 | $2 / $12 | $0.20 / $1.20 |

> GPT-5.6 Sol의 API 가격은 2026년 8월 21일부터 입력 100만 토큰당 4달러, 출력 100만 토큰당 20달러입니다. 최소 2026년 11월 21일까지 적용되는 한시 가격이며, 같은 날 Terra와 Luna 단가는 바뀌지 않았습니다.

캐시 관련 단가에도 같은 비율의 인하가 적용됐습니다. 시스템 프롬프트가 길어 캐시 적중률이 높은 서비스라면 이 부분이 실제 청구액에 더 크게 반영됩니다.

| 항목 | 인하 전 | 인하 후 |
|------|--------|--------|
| 입력 | $5.00 | $4.00 |
| 출력 | $30.00 | $20.00 |
| 캐시 적중 입력 | $0.50 | $0.40 |
| 캐시 쓰기 | $6.25 | $5.00 |

요청을 보내는 방식에 따라 붙는 파생 단가에도 인하가 반영됐습니다. 모두 인하 후 Sol 기준입니다.

- **고속 모드(Fast mode)**: 표준 단가의 2배입니다. 입력 $8, 출력 $40이 됩니다. 응답 속도가 병목인 기능에 쓰는 옵션입니다.
- **장기 컨텍스트**: 입력이 27만 2천 토큰을 넘으면 별도 단가가 적용됩니다. 입력 $8, 출력 $30입니다.
- **배치, 플렉스 처리**: 표준 단가의 절반입니다. 입력 $2, 출력 $10으로 내려갑니다. 응답이 몇 시간 늦어도 되는 대량 작업이라면 가장 저렴한 선택입니다.

인하가 적용되는 범위도 확인해 둘 필요가 있습니다. 쓴 만큼 청구되는 사용량에는 적용되고, 구독료에 이미 포함된 사용량에는 해당하지 않습니다.

- **적용됩니다**: 종량제 API, ChatGPT Work와 Codex에서 따로 구매하는 크레딧, Business와 Edu, Enterprise 워크스페이스의 유연 요금제
- **적용되지 않습니다**: Plus, Pro, Business 구독에 기본으로 포함된 사용량. 이 부분은 인하 전과 같습니다.

정리하면 API로 직접 호출하거나 크레딧을 사서 쓰는 쪽은 청구액이 내려가고, 월 구독료만 내고 포함된 만큼만 쓰는 쪽은 달라지는 것이 없습니다.

## ChatGPT 요금제별 GPT-5.6 사용 범위

API가 아니라 ChatGPT 앱에서 쓰는 경우, 요금제에 따라 접근 범위가 달라집니다. 한 가지 특이한 점은 채팅 화면에 Sol, Terra, Luna를 직접 고르는 메뉴가 없다는 것입니다. 모델 선택기에서 추론 수준을 고르면 그에 맞는 모델이 배정되는 방식으로 알려져 있습니다.

- **무료, Go 요금제**: 일반 대화의 기본 모델은 GPT-5.5 Instant이고, 대화에서 GPT-5.6 계열을 직접 선택할 수 없는 것으로 전해집니다.
- **Plus 요금제 (월 20달러)**: 추론 수준 미디엄과 하이를 선택할 수 있고, 이 구간부터 Sol이 열립니다.
- **Pro, Business, Enterprise 요금제**: 엑스트라 하이와 프로 수준까지 제공됩니다.

요금제별 세부 기준은 자주 바뀌므로, 결제 전에는 OpenAI 공식 도움말에서 현재 기준을 확인하는 편이 안전합니다.

## GPT-5.6과 Claude, Gemini 성능 및 가격 비교

모델 비교에는 독립 벤치마크 기관 Artificial Analysis의 집계를 기준으로 사용했습니다. 아래 차트는 여러 시험을 종합한 지능 지수(Intelligence Index)와, 코딩 도구에 물려 실제 개발 작업을 시킨 코딩 에이전트 지수(Coding Agent Index)입니다.

![Artificial Analysis Intelligence Index와 Coding Agent Index 차트. 지능 지수는 Claude Fable 5가 60으로 1위, GPT-5.6 Sol이 59로 2위이고, 코딩 에이전트 지수는 Codex와 GPT-5.6 Sol 조합이 80으로 1위다](/images/insights/gpt-5-6-sol-terra-luna-comparison/aa-intelligence-vs-cost.png)

차트의 핵심 숫자를 정리하면 다음과 같습니다.

- **지능 지수**: Claude Fable 5가 60으로 1위, GPT-5.6 Sol이 59로 2위입니다. 이어서 Claude Opus 4.8이 56, GPT-5.6 Terra가 55, Claude Sonnet 5가 53, GPT-5.6 Luna가 51입니다.
- **코딩 에이전트 지수**: Codex와 GPT-5.6 Sol 조합이 80으로 1위입니다. Terra가 77로 Claude Code와 Fable 5 조합(77)과 같은 점수이고, Luna도 75로 Claude Code와 Opus 4.8 조합(73)보다 높습니다.
- **작업당 비용**: 지능 지수 측정 기준으로 Sol은 작업당 1.04달러입니다. 비슷한 점수대의 Claude Fable 5 대비 약 3분의 1 비용입니다. Terra는 0.55달러, Luna는 0.21달러입니다.

다만 순수 코드 생성 품질을 보는 SWE-bench Pro에서는 Claude Fable 5가 80%로 Sol의 64.6%를 크게 앞선다는 집계도 있습니다. 저장소 수준의 코드 품질이 중요한 작업과, 터미널에서 여러 단계를 수행하는 에이전트형 작업의 승자가 서로 다른 셈입니다.

가격 축에서 각 체급의 경쟁 모델과 나란히 놓으면 다음과 같습니다. 2026년 8월 25일 기준 입력/출력 가격입니다.

| 체급 | GPT-5.6 | Claude | Gemini |
|------|---------|--------|--------|
| 최상위 | Sol $4 / $20 (정가 $5 / $30) | Opus 5 $5 / $25, Fable 5 $10 / $50 | Gemini 3.1 Pro $2 / $12 |
| 중간 | Terra $2 / $12 | Sonnet 5 $2 / $10 | Gemini 3.1 Pro $2 / $12 |
| 경량 | Luna $0.20 / $1.20 | Haiku 4.5 $1 / $5 | Gemini 3.6 Flash $1.50 / $7.50 |

체급별로 구도가 다릅니다.

- **최상위 구간**: 한시 인하가 적용되는 동안에는 Sol이 입력과 출력 모두 Opus 5보다 저렴합니다. 정가 기준으로는 입력이 같고 출력이 더 비쌌으니, 이번 인하로 두 모델의 가격 순서가 뒤집혔습니다. 다만 11월에 정가로 돌아가면 원래대로 복귀합니다. 그래서 이 구간의 선택 기준은 가격보다 작업 유형에 두는 편이 안전합니다. 지표상 에이전트형 작업은 Sol, 순수 코드 품질은 Claude 계열이 앞섭니다.
- **중간 구간**: Terra와 Sonnet 5, Gemini 3.1 Pro가 모두 입력 2달러 안팎에 몰려 있어 가장 치열한 구간입니다. Sonnet 5는 9월부터 $3 / $15로 오를 예정이었으나, Anthropic이 2026년 8월 10일 인상을 철회하고 $2 / $10을 표준 가격으로 확정했습니다([관련 글](/insights/claude-sonnet-5-price-permanent)). 이 구간의 가격 구도는 당분간 지금 상태로 유지됩니다.
- **경량 구간**: Luna가 Haiku 4.5의 5분의 1, Gemini 3.6 Flash의 7분의 1 수준 입력 가격으로 가장 저렴합니다. 다만 Haiku 4.5는 컨텍스트 윈도가 20만 토큰으로 작고, Gemini 3.6 Flash는 이미지, 영상, 음성 입력을 함께 처리하는 멀티모달이 강점이라 단순 가격만으로 비교하기 어려운 면이 있습니다.

## 작업 유형별 GPT-5.6 모델 선택 기준

기준을 작업 유형으로 잡으면 선택이 단순해집니다. 마케팅 팀에서 AI 자동화를 운영하는 상황을 예로 들어 정리하면 다음과 같습니다.

- **리뷰 수천 건 감성 분류, 키워드 추출**: Luna가 적합합니다. 입력 100만 토큰당 0.20달러라 호출 횟수가 많을수록 유리하고, 분류 정확도는 이 급 모델로 충분한 경우가 많습니다.
- **보고서 초안, 광고 문구 작성, 회의록 요약**: Terra가 기본값입니다. GPT-5.5급 성능을 절반 가격에 쓰는 위치라 일상 업무 대부분을 감당합니다.
- **여러 데이터를 종합한 캠페인 전략 분석, 멀티스텝 에이전트**: Sol이 어울립니다. 단가가 높으므로 전체 작업이 아니라 이 단계에만 배정합니다.
- **저장소 수준의 코드 작성과 리팩터링**: 벤치마크 지표를 참고해 Claude 계열과 교차 비교 후 선택하는 편이 안전합니다.
- **영상, 음성, 이미지가 섞인 입력 처리**: Gemini 계열이 강점을 가진 영역입니다.

한 가지 유의할 점이 있습니다. 이 글의 가격은 2026년 8월 25일 기준이고, 벤치마크는 7월 집계입니다. 두 달 사이에 Gemini 3.6 Flash 출시(7월 21일), GPT-5.6 Luna와 Terra 인하(7월 30일), Sonnet 5 인상 철회(8월 10일), Sol 한시 인하(8월 21일)가 이어졌습니다. 모델 선택을 확정하기 전에는 각 사의 공식 가격 페이지에서 최신 숫자를 확인하는 과정이 필요합니다.

## 자주 묻는 질문

### Sol, Terra, Luna 중에 무엇부터 쓰면 되나요?

Terra를 기본값으로 두는 편이 무난합니다. 이전 세대 GPT-5.5와 비슷한 성능을 절반 가격에 내는 자리라 일상 업무 대부분을 감당합니다. 대량 분류나 데이터 추출처럼 단순 반복은 Luna로 내리고, 여러 단계를 거치는 추론이나 까다로운 코드 수정만 Sol로 올리는 순서가 비용 대비 효율이 좋습니다.

### Luna는 성능이 많이 떨어지나요?

작업 종류를 가려서 쓰면 떨어진다고 느끼기 어렵습니다. Luna는 입력 100만 토큰당 0.20달러로 Terra의 10분의 1 수준이라, 리뷰 감성 분류나 키워드 추출처럼 판단 폭이 좁은 작업에 맞춥니다. 반대로 근거를 이어 붙여 결론을 내야 하는 작업에서는 차이가 드러납니다.

### Sol 가격이 글마다 다르게 적혀 있는 이유가 뭔가요?

한시 인하가 적용 중이기 때문입니다. 정가는 입력 100만 토큰당 $5, 출력 $30이고 인하가는 $4와 $20입니다. 인하는 최소 2026년 11월 21일까지로 안내돼 있어서, 그 이후를 포함한 예산은 정가로 잡는 편이 안전합니다.

### ChatGPT 앱에서도 세 모델을 골라 쓸 수 있나요?

직접 고르는 메뉴는 없습니다. 모델 선택기에서 추론 수준을 고르면 그에 맞는 모델이 배정되는 방식으로 알려져 있습니다. Plus 요금제부터 미디엄과 하이를 고를 수 있고 이 구간부터 Sol이 열립니다. 요금제별 기준은 자주 바뀌므로 결제 전에 공식 도움말을 확인하는 편이 안전합니다.

### GPT-5.5를 쓰던 작업을 그대로 옮겨도 되나요?

Terra로 옮기는 경우라면 성능 손실 없이 비용을 줄이는 방향이 됩니다. OpenAI가 Terra를 GPT-5.5와 비슷한 성능이라고 설명하고 있고 독립 벤치마크에서도 같은 점수대에 있습니다. 다만 프롬프트를 그대로 옮긴 뒤에는 표본을 정해 결과를 비교해 보는 과정이 필요합니다.

## Sources

- [OpenAI, GPT-5.6 소개 페이지](https://openai.com/index/gpt-5-6/)
- [OpenAI API 가격 페이지](https://developers.openai.com/api/docs/pricing)
- [OpenAI 도움말, GPT-5.6 in ChatGPT](https://help.openai.com/en/articles/20001325-a-preview-of-gpt-56-sol-terra-and-luna)
- [Artificial Analysis, GPT-5.6 벤치마크 분석](https://artificialanalysis.ai/articles/gpt-5-6-has-landed)
- [Anthropic, Claude 모델 가격](https://claude.com/pricing)
- [Google, Gemini API 가격](https://ai.google.dev/gemini-api/docs/pricing)
