...
MCP는 이 섬들을 AI 하나로 연결합니다. AI가 여러 서비스를 동시에 볼 수 있기 때문에, 서비스 간 정보 이동을 자연어 한 마디로 처리할 수 있습니다.
...
만들지 않아도 됩니다. 서버를 개발하는 일은 개발자의 몫이지만 이미 공개된 노션이나 슬랙 서버를 연결해 쓰는 일은 설정 파일에 몇 줄을 추가하는 수준입니다. 이 연결 과정은 계속 쉬워지고 있어서 앞으로는 버튼을 누르는 정도로 줄어들 여지가 큽니다.

### MCP를 연결하면 AI가 내용을 읽기만 하나요?

읽기에서 끝나지 않습니다. 노션 MCP를 연결하면 데이터베이스를 조회하고 페이지 본문을 읽는 것뿐 아니라 상태 같은 속성을 바꾸고 새 페이지를 만드는 일까지 합니다. 슬랙 MCP는 채널에 쌓인 메시지를 읽고 요약하는 쪽으로 주로 쓰입니다.

### 지금 꼭 도입해야 하나요?

서두를 이유는 없습니다. MCP는 아직 발전 초기라서 설정이 번거롭고 모든 서비스에 서버가 있는 것도 아닙니다. AI가 업무 도구에 직접 접근하는 방향이라는 것과 그 연결을 맡는 표준 규격이 MCP라는 것만 알아 두어도 지금은 충분합니다.
