...
title: '클로드 코워크(Claude Cowork) 내장 브라우저 출시, Aside와 같은 작업 실행 비교'
excerpt: >-
...
metaTitle: 클로드 코워크(Claude Cowork) 내장 브라우저 출시 정리와 Aside 실행 비교
metaDescription: >-
  클로드 코워크 내장 브라우저는 개인 브라우저와 분리된 채 클로드가 직접 웹사이트를 열고 읽고 클릭하는 데스크톱 앱 기능입니다. 2026년
...
> **2026년 9월 17일 안내:** 9월 16일 발표로 코워크가 채팅과 합쳐집니다. 공식 발표문은 기존 채팅과 프로젝트, 아티팩트, 커넥터, 스킬이 그대로 남는다고만 밝혔고 내장 브라우저가 어디로 옮겨 가는지는 적지 않았습니다. 적용된 뒤에 직접 확인해 이 글을 갱신하겠습니다. 바뀌는 내용과 적용 시점은 [클로드 코워크와 채팅 통합 정리](/insights/claude-cowork-chat-merge)에 따로 정리했습니다.

**클로드 코워크 내장 브라우저는 개인 브라우저와 분리된 채 클로드가 직접 웹사이트를 열고 읽고 클릭하는 데스크톱 앱 기능**입니다. 앤트로픽이 2026년 8월 26일 공개했습니다.
...
앤트로픽이 택한 답은 반대 방향입니다. 브라우저를 갈아타는 대신, 이미 쓰고 있는 클로드 데스크톱 앱 안에 브라우저를 넣었습니다.
...
이미 Claude in Chrome 확장을 쓰고 있으면 그 설정이 유지되고, 설정의 Cowork 메뉴에 있는 선호 브라우저 항목에서 언제든 둘 중 하나로 바꿀 수 있습니다. 8월 중순에 크롬 사이드바에서 코워크 세션을 여는 통합이 먼저 공개됐는데, 이번 내장 브라우저는 크롬조차 필요 없게 만드는 다음 단계입니다.
...
**제품의 위치가 다릅니다.** Aside는 브라우저를 통째로 바꾸는 결정이 필요하고, 그 대신 모델을 골라 끼우는 자유를 줍니다. 코워크 내장 브라우저는 이미 클로드를 쓰는 사람에게 추가 설치 없이 따라오는 기능이고, 그 대신 클로드에 묶입니다. 클로드 유료 플랜에 이 기능이 기본으로 들어가면, 전용 에이전트 브라우저를 따로 설치할 이유가 무엇인지부터 다시 따져 보게 됩니다.
