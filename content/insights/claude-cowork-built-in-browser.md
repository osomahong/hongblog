...
- 클로드 코워크 내장 브라우저는 개인 브라우저와 분리된 채 클로드가 직접 웹사이트를 열고 읽고 클릭하는 데스크톱 앱 기능입니다. 2026년 8월 26일부터 Pro, Max, Team 플랜에 한 주에 걸쳐 단계적으로 적용됩니다.
- 개인 브라우저의 탭과 비밀번호는 보이지 않고, 로그인은 Chrome과 Edge와 Firefox에서 사이트를 골라 쿠키를 넘기는 방식만 열려 있습니다. 금융과 이메일과 SSO 사이트는 기본 제외이고, 공식 도움말도 민감한 정보 처리에는 쓰지 말라고 안내합니다.
- Aside 후기 때와 같은 명령 2건을 실행하니 두 작업 모두 정답을 가져왔고, 사이트마다 접근 허용을 먼저 묻는 점이 달랐습니다. 승인 대기를 포함해 두 작업 모두 1분 안팎이 걸렸습니다.
