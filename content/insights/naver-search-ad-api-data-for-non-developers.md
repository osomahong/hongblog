...
title: '비개발자를 위한 네이버 검색광고 API 가이드: 필드 매핑, 통합 리포트'
excerpt: >-
...
quiz:
  - options:
    ...
    question: >-
      네이버 검색광고 API의 ROAS 필드(ror)가 350을 반환했을 때, 구글, 메타의 ROAS와 동일한 기준으로 환산하면
      얼마일까요?
    explanation: >-
...
### 네이버 검색광고 API는 신청하면 바로 쓸 수 있나요?

바로 쓸 수 있습니다. 구글과 메타처럼 앱을 만들고 권한을 신청하고 사업자 인증을 받는 단계가 없습니다. searchad.naver.com에 로그인해 도구 메뉴의 API 사용 관리에서 약관에 동의하면 바로 발급됩니다. 심사나 승인을 기다리지 않아도 됩니다.

### 캠페인 이름이 성과 데이터에 나오지 않는데 왜 그런가요?

네이버는 성과 데이터와 이름 정보를 따로 줍니다. 통계 엔드포인트는 ID와 수치만 돌려주고, 캠페인 이름은 마스터 데이터를 따로 불러 ID로 붙여야 나옵니다. 구글이나 메타처럼 한 번에 같이 오지 않는다는 점이 이 API에서 가장 많이 걸리는 부분입니다.

### 다른 매체와 리포트를 합칠 때 무엇을 조심해야 하나요?

단위가 서로 다릅니다. 비용은 네이버가 원화 정수인데 구글은 100만분의 1 단위라 나눠 줘야 하고, ROAS는 네이버가 퍼센트인데 구글은 배수입니다. 클릭 기준도 매체마다 달라서, 합치기 전에 세 가지를 맞추지 않으면 합계가 조용히 어긋납니다.

### 대행사가 고객사 계정의 데이터를 가져올 수 있나요?

가져올 수 있습니다. 고객사 계정에서 권한을 위임해 주면 됩니다. 광고주 본인이 직접 신청하는 경로와 별개로 열려 있어서, 대행 구조에서도 계정마다 키를 따로 받을 필요가 없습니다.
