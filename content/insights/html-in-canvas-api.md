...
웹에서 화면을 만드는 방법은 크게 둘로 나뉩니다. 하나는 [HTML](/class/digital-basic/what-is-html)과 [CSS](/class/digital-basic/what-is-css)로 만드는 [DOM](/class/digital-basic/what-is-dom)이고, 다른 하나는 픽셀을 직접 칠하는 캔버스입니다.

둘의 차이는 문서와 그림판의 차이로 보면 정확합니다. DOM은 문서입니다. 브라우저가 "여기는 제목, 여기는 버튼, 여기는 입력창"이라는 구조를 알고 있습니다. 그래서 스크린 리더가 읽어 주고, Ctrl+F로 검색이 되고, 드래그해서 복사할 수 있고, 브라우저 번역과 다크 모드가 그대로 적용됩니다. 검색 엔진이 내용을 읽어 가는 것도 문서라서 가능합니다. 대신 픽셀 단위의 자유로운 그리기와 3D 표현에는 약합니다.
...
- 크로스 오리진 iframe은 보안상 그릴 수 없고, 스크롤이 [JavaScript](/class/digital-basic/what-is-javascript)에 묶이는 성능 우려와 크기 조정 처리의 미성숙도 지적됩니다
- 실험 단계의 API는 형태가 바뀔 수 있습니다. 실제로 초기 제안의 placeElement 중심 설계가 지금의 drawElementImage 중심으로 이미 한 차례 바뀌었습니다

## 자주 묻는 질문

### HTML in Canvas는 지금 쓸 수 있나요?

브라우저 지원 상태를 먼저 확인해야 합니다. 표준 논의와 구현이 진행 중인 단계라, 실제 서비스에 넣기 전에 대상 브라우저에서 동작하는지와 지원하지 않을 때의 대체 화면을 함께 준비하는 편이 안전합니다.

### 기존 캔버스 작업과 무엇이 다른가요?

캔버스에 그리던 것을 HTML로 쓸 수 있게 된다는 점입니다. 지금까지는 캔버스 안에 글자와 도형을 좌표로 그려야 했는데, 이 방식은 이미 있는 HTML과 CSS를 그대로 캔버스 위에 올립니다. 접근성과 텍스트 선택처럼 HTML이 기본으로 주던 것을 포기하지 않아도 됩니다.
