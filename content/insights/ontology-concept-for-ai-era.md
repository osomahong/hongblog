...
이 단어를 처음 본격적으로 사용한 사람이 아리스토텔레스입니다. 그는 "무엇이 존재하는지" 를 알기 위해 세상에 있는 모든 사물을 10가지 카테고리로 정리했습니다. 실체, 양, 질, 관계, 위치, 시간, 장소, 능동, 수동, 소유. 어떤 대상이든 이 10가지 범주 안에서 설명할 수 있다고 본 것입니다.
...
> Taxonomy 는 "무엇이 같은 칸에 있는지" 를, Ontology 는 "무엇이 어떤 속성과 관계로 연결되어 있는지" 를 본다. 이 차이가 추천 품질과 설명 가능성을 가른다.

AI 시대에 온톨로지가 부각되는 이유가 여기에 있습니다. 단순한 분류로는 "왜 이 결과가 나왔는지" 를 설명할 수 없지만, 속성과 관계를 명시한 온톨로지 기반 시스템은 모든 답에 근거를 제시할 수 있습니다.
...
**Property(속성)** 는 클래스나 인스턴스의 특성을 설명하거나, 다른 개체와 연결 고리를 만드는 정의입니다. 속성은 두 가지로 나뉩니다. 객체 속성(Object Property) 은 인스턴스와 인스턴스를 잇고, 데이터 속성(Data Property) 은 인스턴스와 구체적인 값(literal) 을 잇습니다.
...
> 추론이 없는 데이터는 단순한 기록일 뿐입니다. 추론이 더해질 때 그제야 데이터가 "이해" 가 됩니다.
...
> 데이터만으로는 질문이 분석이 되지 못합니다. 데이터 위에 온톨로지가 올라가야 그때부터 질문이 답이 됩니다.

## 자주 묻는 질문

### 온톨로지와 분류 체계는 같은 말인가요?

다릅니다. 분류 체계는 무엇이 같은 칸에 들어가는지를 보고, 온톨로지는 각 대상이 어떤 속성과 관계로 이어지는지를 봅니다. 그래서 분류 체계로 추천하면 같은 카테고리라는 것 말고는 근거를 댈 수 없지만, 온톨로지는 어떤 속성이 비슷해서 나온 결과인지 설명할 수 있습니다.

### 온톨로지는 일반 데이터베이스와 무엇이 다른가요?

입력하지 않은 사실을 끌어내는 추론이 붙는다는 점이 다릅니다. 강남구가 서울의 일부이고 서울이 한국의 일부라는 두 사실과 전이성 규칙만 넣어 두면, 강남구가 한국의 일부라는 결론은 따로 적지 않아도 시스템이 만들어 냅니다. 관계를 사람이 하나하나 채워 넣지 않아도 되는 것이 이 구조의 이점입니다.

### 온톨로지를 만들려면 무엇부터 정의해야 하나요?

클래스와 인스턴스, 속성 세 가지입니다. 피자처럼 개념의 뼈대가 되는 것이 클래스이고, 마르게리따처럼 그 클래스에 속하는 실제 개체가 인스턴스이며, 가격이나 토핑처럼 특성을 적거나 다른 개체와 이어 주는 정의가 속성입니다. 이 세 가지가 정리되고 나서야 추론 단계로 넘어갈 수 있습니다.
...
구글이 검색 결과의 의미를 바꾼 것도, 넷플릭스가 한 번도 보지 않은 작품을 추천하는 것도, 팔란티어가 흩어진 데이터에서 인과를 끌어내는 것도 모두 같은 곳에서 출발합니다. 자기 도메인을 어떻게 정의해 두었느냐입니다.
