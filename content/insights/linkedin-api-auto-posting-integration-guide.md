...
### 토큰을 환경 변수에 저장하면 안 되나요?

환경 변수보다는 DB가 맞습니다. 액세스 토큰은 갱신될 때마다 값이 바뀌는데 환경 변수는 값을 바꿀 때마다 배포를 다시 해야 해서 자동 갱신과 어울리지 않습니다. 이 프로젝트는 1인 관리자용이라 행 하나만 유지하는 작은 테이블로 충분했습니다.

### 액세스 토큰이 만료되면 매번 다시 로그인해야 하나요?

대부분은 그럴 필요가 없습니다. 게시 요청이 들어올 때마다 만료 시점을 확인해서 7일 이상 남았으면 그대로 쓰고, 그보다 가까워지면 리프레시 토큰으로 새 토큰을 받아 둡니다. 갱신에 실패해도 아직 만료 전이면 기존 토큰을 쓰고, 리프레시 토큰까지 만료된 경우에만 재인증 안내가 나옵니다.

### 개인 프로필에 글을 올리는데도 Company Page를 연결해야 하나요?

앱 등록 단계에서는 연결해야 합니다. LinkedIn Developer App이 Company Page를 요구하는 것은 앱을 식별하기 위해서이고, 게시 대상을 그 페이지로 제한하지는 않습니다. 개인 개발자라면 Individual Developer용 기본 Company Page를 고르면 됩니다.
