...
### 오피스 프로그램을 따로 설치해야 하나요?

대부분의 작업에는 필요 없습니다. 실행 파일 하나에 문서를 그려 내는 엔진까지 들어 있어서 화면이 없는 서버나 도커 안에서도 만들고 확인하는 순서가 그대로 돌아갑니다. 다만 목차 쪽 번호를 다시 계산하는 refresh 명령은 윈도우에서 워드를 뒷단으로 쓰고, 그 밖의 환경에서는 HTML 방식으로 대신 처리합니다.

### 같은 양식의 문서를 여러 건 만들 때도 매번 새로 생성해야 하나요?

merge로 값만 채우는 쪽이 낫습니다. 레이아웃은 한 번만 설계해 두고 `{{key}}` 자리에 데이터를 넣으면 보고서 100건을 만들 때도 생성 과정을 100번 거치지 않습니다. 결과물의 모양이 건마다 달라지지 않는 것도 이 방식의 이점입니다.

### 고친 파일을 바로 다른 프로그램에서 열어도 되나요?

저장을 먼저 해야 합니다. 상주 모드에서는 변경 사항이 메모리에 있다가 일정 시간 뒤에 디스크로 기록되기 때문에 다른 도구로 읽기 전에 save나 close를 실행합니다. 업무 문서라면 그 뒤에 실제 오피스에서 열어 확인하는 절차까지 붙이는 편이 안전합니다.
