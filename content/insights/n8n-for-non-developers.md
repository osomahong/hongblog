...
quiz:
  - options:
    ...
    explanation: 글에서도 다뤘듯이, n8n의 노드 연결 자체는 직관적입니다. 진짜 장벽은 JSON 구조 이해, HTTP 상태 코드, API 인증 같은 '도구 바깥의 기반 지식'입니다. 물론 3번도 중요한 요소이고 의견이 갈릴 수 있지만, 로직 설계는 어느 정도 AI의 도움을 받을 수 있는 반면, 에러 메시지를 읽고 디버깅하려면 기반 지식이 필수적입니다.
    correctIndex: 1
metaTitle: n8n 자동화, 비개발자 마케터가 직접 써보고 느낀 현실적인 후기
metaDescription: 비개발자 마케터가 n8n으로 업무 자동화를 직접 시도한 경험담입니다. GA4 리포트 자동 발송, 리드 정리 등 실제 활용 사례부터 API 인증, JSON, Expression 등 비개발자가 마주하는 현실적인 어려움과 필요한 학습량까지 솔직하게 공유합니다.
ogTitle: 코딩 모르는 마케터의 n8n 자동화 도전기 - 과연 쉽다는 말이 맞을까?
ogDescription: 매주 반복하던 GA4 리포트, 리드 정리를 n8n으로 자동화한 비개발자 마케터의 솔직 후기. 간단한 건 쉽지만 결국 공부가 필요하더라는 현실적인 이야기입니다.
---
