...
title: '클로드 코드(Claude Code) CLAUDE.md 세팅, 알려진 노하우들과 공식 문서 업데이트 정리'
excerpt: >-
...
summary3:
  ...
  - >-
    소네트 5의 9월 1일 가격 인상은 취소되어 100만 토큰당 2달러와 10달러가 표준가가 되었고, 공식 문서는 모델 교체보다 effort
    조절을 먼저 권합니다.
highlights:
  - CLAUDE.md에 적은 금지 규칙이 지켜지지 않는다면 문장을 고치기 전에 훅이나 권한 설정으로 옮깁니다.
  - 매번 필요하지 않은 절차는 스킬로 내려 둡니다. 클로드 코드는 스킬 본문을 부를 때만 읽습니다.
metaTitle: '클로드 코드(Claude Code) CLAUDE.md 세팅, 공식 문서와 다른 대목 정리'
metaDescription: >-
...
CLAUDE.md는 [클로드 코드](/class/claude-code-for-everyone/what-is-claude-code)가 세션을 시작할 때마다 읽는 지시 파일입니다. 프로젝트 규칙을 여기 적어 두면 매번 설명하지 않아도 된다는 것까지는 쓰는 사람 대부분이 알고 있습니다.
...
문제는 그 섹션에 기대하는 효과입니다. [훅](/class/claude-code-for-everyone/what-are-hooks)이라는 말은 CLAUDE.md보다 상대적으로 낯설 수 있는데, 쉽게 말하면 정해진 시점에 클로드의 판단과 상관없이 실행되는 검사 명령입니다. 위 화면의 첫 문단이 이 대목을 분명하게 적고 있습니다. 클로드는 CLAUDE.md와 자동 메모리를 강제 설정이 아니라 참고 자료로 다루며, 어떤 동작을 클로드의 판단과 무관하게 막으려면 PreToolUse 훅을 쓰라는 내용입니다.
...
> 클로드 코드는 스킬 본문을 부를 때만 읽습니다. 매번 필요하지 않은 절차를 CLAUDE.md에서 스킬로 옮기면 실제로 컨텍스트가 줄어듭니다.
...
여러 정리 글이 [소네트 5](/insights/claude-sonnet-5-price-permanent)의 100만 토큰당 2달러와 10달러를 출시 기념 가격으로 소개하면서 9월 1일부터 3달러와 15달러로 오른다고 적어 두었습니다. 위 화면의 안내 상자가 그 대목을 뒤집습니다. 2달러와 10달러가 이제 표준 가격이 되었고 예정되어 있던 인상은 일어나지 않는다는 내용입니다.
...
클로드 코드가 파일을 여러 곳에서 읽는다는 점도 함께 봐야 합니다. 클로드 코드는 현재 폴더와 그 위 폴더들의 CLAUDE.md를 모두 읽어 이어 붙이고, 실행한 위치에 가까운 파일을 나중에 읽습니다. 큰 저장소에서 다른 팀의 파일까지 딸려 온다면 `claudeMdExcludes` 설정으로 뺄 수 있습니다. 어떤 파일이 실제로 읽혔는지는 세션에서 `/context`를 실행해 메모리 파일 목록으로 확인합니다.
