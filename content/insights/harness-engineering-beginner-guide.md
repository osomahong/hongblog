...
metaTitle: '하네스 엔지니어링 뜻: AI 에이전트 시대 입문 가이드'
metaDescription: >-
  하네스 엔지니어링은 AI 에이전트가 반복해서 안정적으로 일하도록 작업 환경과 절차를 설계하는 일입니다. 용어가 나온 배경과 프롬프트
  엔지니어링과의 차이, 다섯 가지 원칙, Claude Code에서 CLAUDE.md와 스킬이 맡는 역할을 정리했습니다.
...
summary3:
  - '하네스 엔지니어링은 AI 에이전트가 반복해서 안정적으로 일하도록 작업 환경과 절차를 설계하는 일입니다.'
  - '프롬프트 엔지니어링에서 바이브코딩과 컨텍스트 엔지니어링을 거쳐 매번 설명하지 않아도 되는 체계를 만드는 단계로 넘어온 것입니다.'
  - 'Claude Code에서는 CLAUDE.md와 서브에이전트, 스킬이 그 체계의 실체이고 코딩보다 무엇을 만들지 설계하는 능력이 중요해집니다.'
---
...
준이아빠블로그를 예로 들겠습니다. 저는 Claude Code로 이곳을 만들고 운영하고 있습니다. 처음에는 "이거 만들어줘"라고 말하면 됐는데, 프로젝트가 커지면서 AI의 결과물이 일관되지 않는 문제가 생겼습니다. 그래서 이런 구조를 만들었습니다.
...
Gartner는 2026년 말까지 기업 애플리케이션의 40%가 AI 에이전트를 포함할 것으로 전망했습니다(2025년 5% 미만에서 급증). 이제 문제는 "AI를 쓰느냐"가 아닙니다. **"AI 팀원들이 잘 일하도록 어떻게 식당을 운영하느냐"**입니다.

## 실무에서 본 하네스 엔지니어링의 모습

구체적인 예시로, 준이아빠블로그 프로젝트의 하네스 구조를 보여드리겠습니다.
...
하네스 엔지니어링은 코드를 직접 짜는 게 아닙니다. **함께 일하는 환경과 규칙을 설계하는 겁니다.** 준이아빠블로그를 Claude Code로 운영하면서 제가 하는 일의 대부분은 문서 작성, 구조 설계, 결과 확인입니다. 코딩이 아닙니다.
...
AI 에이전트를 감싸는 환경과 규칙, 확인과 수정 체계 전부를 설계하는 일입니다. 하네스는 원래 말에 씌우는 마구를 뜻하는 말이고, 동사로는 강력한 것을 길들여 쓸모 있게 쓴다는 뜻입니다. 모델 자체를 바꾸는 대신 모델이 일하는 틀을 잡는 쪽이라고 보면 됩니다.

### 프롬프트 엔지니어링과 무엇이 다른가요?

다루는 범위가 다릅니다. 프롬프트는 한 번의 요청을 어떻게 쓸지의 문제이고, 하네스는 에이전트가 어떤 재료를 볼 수 있고 어디까지 손댈 수 있으며 결과를 누가 어떻게 확인하는지까지를 정합니다. 요청 문장이 아니라 작업 환경을 설계하는 일에 가깝습니다.

### 개발자가 아니어도 할 수 있나요?

할 수 있습니다. 범위를 정해 주고 확인 단계를 두는 일은 코드를 짜는 능력보다 업무를 쪼개는 능력에 가깝습니다. 어떤 자료만 쓰게 할지, 어디서 사람이 한 번 확인할지를 정하는 것부터가 하네스 설계입니다.
