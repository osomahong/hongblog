...
title: 연동 없는 서비스에 AI로 일 시키는 방법과 이동 중 원격 업무
excerpt: >-
...
metaTitle: '연동 없는 서비스에 AI로 일 시키기: 브라우저 조작과 원격 업무'
metaDescription: >-
...
summary3:
  - 브라우저 조작 AI는 전용 연동이 없는 서비스라도 화면이 있으면 사람처럼 눌러 작업을 맡길 수 있는 방식입니다.
  - 집에 둔 컴퓨터가 60초마다 메일함을 훑어 나에게 온 요청인지 판단하고 텔레그램으로 맥락과 답장 초안을 함께 보냅니다.
  - '자동화가 늘수록 사람이 쓰는 시간은 실행에서 권한과 기준, 발송 같은 판단 쪽으로 옮겨 갔습니다.'
---
AI에게 일을 시키는 방식은 상당히 다양합니다. 프롬프트를 넣으면 문서가 나오고 프로덕트가 뚝딱 나옵니다. 그 결과물을 어디에 쓸지도 각자 정리가 되어 갑니다.

다만 그 프롬프트가 내가 실제로 쓰는 업무 도구까지 직접 닿는지, 아니면 여전히 내가 받아서 써야하는지는 다른 문제입니다. 분석 화면, 데이터 스튜디오 대시보드처럼 AI에 직접 연결할 [MCP](/class/claude-code-for-everyone/what-is-mcp)나 CLI가 없는 도구들은 대개 자동화 이야기에서 빠져 있습니다. 결과물을 만드는 일은 맡길 수 있어도, 그 결과물이 나오는 화면을 고치는 일은 여전히 사람이 해야 하는 영역으로 남아 있습니다.
...
직접 확인이 필요한 내용이 있을 때 스마트폰으로 집에 있는 컴퓨터로 접속합니다. 서로 다른 두 기기를 [VPN으로 같은 망에 묶어](/insights/tailscale-usage-ai-remote-work) 두어서 지하철이든 카페든 위치는 상관이 없습니다. 그리고 컴퓨터에 요청을 합니다. 
