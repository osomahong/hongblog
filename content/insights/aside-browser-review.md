...
title: 'Aside(어사이드) 브라우저 사용 후기: 한국인 창업팀이 만든 AI 에이전트 브라우저'
excerpt: >-
...
metaTitle: 'Aside(어사이드) 브라우저 사용 후기와 기능, 가격, 경쟁 제품 비교'
metaDescription: >-
...
Zapier나 [MCP](/class/claude-code-for-everyone/what-is-mcp) 같은 연동 방식의 에이전트는 서비스마다 통합을 연결해야 움직입니다. 지원하지 않는 서비스가 나오면 거기서 막힙니다. Aside의 매니페스토는 이 지점을 겨냥합니다. 지금의 AI 에이전트는 수백 개의 통합을 연결하라 하고, 매번 사용자 맥락을 다시 묻고, 단계마다 승인을 요구하는데, 이것이 거꾸로 됐다는 이야기입니다.

Aside가 내놓은 답은 단순합니다. 어차피 요즘 업무 대부분은 브라우저 탭 안에서 일어나니, 에이전트를 브라우저 안에 넣고 [사람이 하듯 화면을 보고 클릭하고 입력하게](/insights/aside-vs-cdp-playwright-browser-agent) 하자는 것입니다. 로그인이 이미 되어 있는 브라우저를 쓰기 때문에 별도 연동이 필요 없고, 이론상 웹에서 사람이 하는 일은 전부 시킬 수 있습니다.
...
**브라우저가 에이전트의 실행 환경이 되는 방향.** 태스크 화면에는 "Connect Aside to Codex"라는 배너가 떠 있습니다. 코딩 에이전트가 브라우징이 필요할 때 Aside를 손발로 쓰게 하는 연결입니다. Pro 플랜의 Channels는 Slack이나 Telegram에서 원격으로 작업을 지시하는 기능이고, MCP 서버 연결과 스킬 설정도 이미 들어와 있습니다. 브라우저를 사람의 도구가 아니라 에이전트들이 공유하는 작업 공간으로 만들겠다는 그림이 제품 곳곳에 보입니다.
...
### Aside 브라우저는 무료로 쓸 수 있나요?

무료 플랜이 있습니다. 월 500 크레딧과 루틴 3개, 패스워드 매니저, 메모리, 구독 연결이 포함됩니다. 내 AI 구독이나 API 키를 연결하면 모델 비용을 따로 내지 않고 쓰는 구조라, 가격표보다 이미 내고 있는 구독료를 다시 쓰는 틀로 보는 쪽이 실제 비용 감각에 가깝습니다.

### 유료 플랜은 무엇이 달라지나요?

Pro는 월 20달러로 사용량이 3배가 되고 심층 조사와 무제한 루틴, 슬랙과 텔레그램에서 원격으로 작업을 지시하는 기능이 열립니다. Max는 월 200달러로 사용량이 40배이고 얼리 액세스가 붙습니다. 팀 단위 배포는 별도 문의입니다.

### 루틴 자동화는 무엇을 할 수 있나요?

매일 아침 업계 뉴스 정리나 매주 경쟁사 가격 확인처럼 반복 작업을 등록해 두면 알아서 돌아갑니다. 에이전트 작업 단가가 크레딧 단위로 낮아진 상태라, 루틴이 쌓이면 브라우저가 백그라운드에서 일하는 시간이 사람이 쓰는 시간을 넘어설 수 있습니다.
## 참고 자료
