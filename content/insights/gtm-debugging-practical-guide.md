...
### 미리보기에서는 태그가 발동하는데 실제 사이트에서는 왜 작동하지 않나요?

게시를 하지 않았을 가능성이 가장 큽니다. 워크스페이스 상단에 변경사항 배지가 남아 있으면 설정이 미리보기에서만 적용되고 실제 사이트에는 반영되지 않습니다. 게시한 뒤에도 같다면 Network 탭에서 요청이 실제로 나가는지, 광고 차단 확장이나 CSP가 막고 있지는 않은지 확인합니다.

### SPA에서 페이지를 옮겨도 페이지뷰가 한 번만 잡히는데 어떻게 하나요?

기본 Page View 트리거가 최초 로딩에서만 작동하기 때문입니다. React나 Vue 같은 SPA는 URL만 바뀌고 문서를 새로 불러오지 않으므로, History Change 트리거로 URL 변경을 감지하거나 라우터에서 직접 dataLayer로 값을 보내야 합니다. 이때 GA4 구성 태그의 send_page_view 설정도 함께 확인합니다.

### 이벤트는 들어오는데 파라미터 값이 비어 있는 이유는 무엇인가요?

변수 이름이 어긋났거나, 값이 준비되기 전에 태그가 발동한 경우가 대부분입니다. dataLayer 변수는 대소문자를 구분하므로 GTM 변수명과 개발팀이 넣은 키를 글자 그대로 대조하고, 데이터 영역 변수의 버전 설정도 함께 봅니다. 미리보기의 Variables 탭에서 그 시점의 값을 확인하면 둘 가운데 어느 쪽인지 구분됩니다.
