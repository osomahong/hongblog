...
### 윈도우에서 클로드 코드를 쓰려면 WSL이 꼭 필요한가요?

필요하지 않습니다. 윈도우용 설치 경로가 따로 있어서 PowerShell에서 바로 설치하면 됩니다. WSL은 리눅스 환경에서 쓰고 싶은 경우의 선택지이고, 처음 설치하는 단계에서는 고려하지 않아도 됩니다.

### 설치가 끝났는지 어떻게 확인하나요?

터미널을 새로 열고 `claude` 명령을 실행해 보면 됩니다. 안내 화면이 뜨면 설치가 끝난 것이고, 명령을 못 찾는다고 나오면 설치 경로가 PATH에 잡히지 않은 경우입니다. 터미널을 완전히 껐다 켠 뒤 다시 확인합니다.

### 터미널이 어려운데 다른 방법이 있나요?

데스크톱 앱이 있습니다. 그래픽 화면에서 같은 기능을 쓸 수 있어서 터미널 명령이 부담스러우면 이쪽으로 시작해도 됩니다. 터미널에서만 되는 기능이 필요해질 때 CLI를 추가로 설치하면 됩니다.

### 어느 폴더에서 시작해야 하나요?

작업할 프로젝트 폴더에서 시작합니다. 클로드 코드는 실행한 폴더를 기준으로 파일을 읽고 고치기 때문에, 아무 폴더에서나 열면 원하는 파일을 못 찾습니다. 처음이라면 연습용 폴더를 하나 새로 만들어 두는 편이 안전합니다.
## Sources
