...
- 전환 건수가 1건으로 잡히는지 (2건 이상이면 중복 집계)
- 전환 값(매출)이 실제 결제 금액과 일치하는지
- 전환이 잡히기까지 시간이 얼마나 걸리는지 (보통 수 분 ~ 수 시간 이내)
...
전환 캠페인을 시작할 때 주의할 점이 하나 있습니다. 알고리즘이 "어떤 사람이 전환할 가능성이 높은지"를 학습하는 데 보통 **3~7일**이 걸립니다. 이 기간에 예산, 타겟, 소재를 자주 바꾸면 학습이 초기화됩니다. 초보자들이 가장 많이 하는 실수가 "첫날 성과가 안 좋아서 바로 세팅을 바꾸는 것"입니다. 알고리즘에게 학습할 시간을 주지 않으면 영원히 초기 상태를 반복하게 됩니다.

## 전환 캠페인 시작 전 체크리스트

| 순서 | 체크 항목 |
|------|--------|
| 1 | 우리 비즈니스의 "전환"을 명확히 정의했는지 |
| 2 | 전환 추적 코드(픽셀, 태그)가 정상 작동하는지 |
| 3 | 테스트 전환으로 데이터가 정확히 잡히는 것을 확인했는지 |
| 4 | 주당 50건 이상의 전환 데이터를 확보할 수 있는지 |
| 5 | 부족하다면 마이크로 전환 목표를 대안으로 준비했는지 |
| 6 | 랜딩페이지가 광고 메시지와 일치하는지 |
| 7 | 모바일에서 랜딩페이지가 정상 작동하는지 |
