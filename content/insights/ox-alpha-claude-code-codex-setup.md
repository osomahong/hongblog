...
**3줄 요약:**
- Ox Alpha는 Z.AI의 GLM-5.3-Flash입니다. 클로드 코드와 코덱스 CLI는 모두 Z.AI가 공식 지원하는 열다섯 개 도구 목록에 들어 있어서, 도구를 새로 설치하지 않고 요청을 보내는 주소만 바꾸면 됩니다.
- 클로드 코드는 `~/.claude/settings.json`에 Anthropic Messages 주소를, 코덱스 CLI는 `~/.codex/config.toml`에 OpenAI Responses 주소를 넣습니다. 두 값은 서로 바꿔 쓸 수 없습니다.
- 무료가 아니라 GLM Coding Plan 구독이나 종량 결제가 필요하고 Pro는 월 18달러부터입니다. 앤트로픽 구독으로 클로드 코드를 쓰던 환경이라면 같은 설정 파일을 덮어쓰므로 되돌릴 방법을 미리 준비해 둡니다.
