...
metaDescription: >-
  요소(Element)는 버튼, 이미지, 텍스트처럼 웹페이지를 구성하는 가장 작은 부품입니다. 여는 태그와 내용, 닫는 태그로 이뤄진 구조, 구조와 콘텐츠와 기능으로 나뉘는 세 가지
  종류, GTM 클릭 추적에서 이 단위가 쓰이는 방식을 정리했습니다.
...
summary3:
  ...
  - >-
    개발자가 이 Element를 고쳐 달라고 할 때 가리키는 단위가 이것이고 id와 class 같은 속성이 붙어야 그중 하나를 정확히 집을
    수 있습니다.
---
