...
**교육 종사자**
- "강의 자료 만들기" 프로젝트: 학생 수준 정의 + 강의 톤 가이드 + 과거 자료. 새 주제를 넣으면 같은 결을 가진 자료가 나옵니다.
...
1. **Claude Projects는 같은 자료, 지시, 대화 기록을 묶어 두는 claude.ai의 작업 공간**입니다. 매번 컨텍스트를 다시 깔 필요 없이 정해진 맥락 위에서 일관된 결과를 받습니다.

2. **반복 작업, 같은 형식의 결과, 단계적 작업, 컨텍스트 분리가 필요한 자리에 적합**합니다. 마케터의 주간 리포트, PM의 PRD 작성, 교육자의 강의 자료 같은 시나리오가 표준입니다.

3. **Projects 위에 [Skills](/class/claude-in-practice/what-are-claude-skills)와 [Extended Thinking](/class/claude-in-practice/what-is-extended-thinking)을 얹으면 자동화 깊이가 더 커집니다.** 자료를 너무 많이 넣지 말고, 사용자 지정 지시를 잘 작성해 두는 게 가장 큰 차이를 만듭니다.
