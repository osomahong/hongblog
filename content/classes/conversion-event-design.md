...
term: 앱 전환 이벤트 설계 (GA4 이벤트와 앱 캠페인 연결)
definition: >-
...
summary3:
  - >-
    전환 이벤트 설계는 앱 안 행동 가운데 캠페인 목표로 삼을 것을 골라 이벤트로 수집하고 광고 시스템이 그 행동을 늘리도록 연결하는
    작업입니다.
  - 'GA4 앱 이벤트는 SDK가 알아서 잡는 자동 수집과 구글이 이름을 정해 둔 추천, 우리가 이름을 붙이는 맞춤 세 층으로 나뉩니다.'
  - >-
    수집된 이벤트를 GA4에서 주요 이벤트로 지정한 다음 구글 광고에서 전환으로 가져와야 앱 캠페인의 tCPA 입찰이 그 행동을 목표로
    삼습니다.
---
