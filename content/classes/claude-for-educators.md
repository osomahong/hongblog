...
term: '교사, 강사를 위한 클로드(Claude) 활용하기'
definition: >-
  교사, 강사, 교육 기획자가 수업 자료 제작, 학생 평가, 커리큘럼 설계, 학습 도우미 운영에 Claude의 핵심 도구를 묶어 쓰는 방법을
  정리한 가이드입니다.
...
difficulty: BEGINNER
quiz:
  - question: 수업 자료를 만들 때 Claude를 효과적으로 쓰려면 가장 먼저 무엇을 정해야 할까요?
    options:
      - '학생 수준, 수업 톤, 자료 형식의 기준을 명시한 수업 가이드'
      - 가장 비싼 모델 결제
      - 영어 자료부터 사용
      - 손글씨 자료부터 입력
    correctIndex: 0
    explanation: >-
      교육 자료의 품질은 형식보다 기준에서 결정됩니다. 학생 수준(초, 중급, 고급), 수업 톤(존댓말, 친근체), 자료 형식(슬라이드,
      워크북, 퀴즈) 의 기준을 먼저 명시한 수업 가이드를 만들어두면 Claude가 매번 같은 결의 자료를 생성합니다. 모델, 언어, 입력
      방식은 그 다음 단계의 변수입니다.
metaTitle: '교사, 강사를 위한 클로드 활용법: 수업 자료와 평가'
metaDescription: >-
  교사와 강사의 클로드 활용은 수업 자료 제작과 시험 출제, 커리큘럼 설계, 학습 도우미 운영에 클로드의 도구를 묶어 쓰는 일입니다. 새 단원
  자료 만들기와 출제, 외국어 자료 변환 시나리오, 검수와 기관 도입에서 점검할 것을 정리했습니다.
ogImage: /og/claude-for-educators.png
summary3:
  - '교육 종사자가 Claude로 하는 일은 수업 자료 제작과 시험 출제, 커리큘럼 설계, 학습 도우미 운영, 학사 문서 작성입니다.'
  - 과정별로 Projects를 나눠 교과서와 출제 기준을 올려 두면 새 단원에서도 같은 형식과 난이도로 자료가 나옵니다.
  - 학생에게 나가는 자료는 환각이 섞이지 않았는지 표본으로 검수하고 AI로 만든 자료임을 공개할지 기관 차원에서 먼저 정합니다.
---
...
새 수업이 잡힐 때마다 수업안, 슬라이드, 워크북, 퀴즈를 처음부터 만듭니다. 학생 수준에 맞춰 어조를 다듬고, 같은 주제라도 매번 형식을 다시 잡습니다.

이 반복 작업의 상당 부분이 자동화 가능합니다. 다만 학생을 다루는 일이라 책임 영역이 마케팅과 다릅니다. 정확성, 공정성, 접근성 기준을 함께 묶어 쓰는 흐름을 만드는 게 핵심입니다.

## 📚 교육자가 자주 마주치는 5가지 작업

- **수업 자료 제작**: 수업안, 슬라이드, 워크북, 핸드아웃
...
## 📅 시나리오 1. 새 단원 수업 자료 만들기

**1단계: 수업 가이드 프로젝트 생성**
- 학생 수준(초, 중급, 고급), 수업 톤(존댓말, 친근체), 자료 형식(슬라이드, 워크북, 퀴즈) 기준을 한 번 정해 업로드.

**2단계: 새 단원 주제 던지기**
- "분수의 덧셈" → Claude가 수업 가이드에 맞춰 수업안 초안 생성.
...
- **Projects**: 수업 자료, 교과서, 자주 묻는 질문 모음을 업로드
...
- **학생에게의 공개 정책**: AI 보조로 만든 자료인 점을 학생에게 공개할지
