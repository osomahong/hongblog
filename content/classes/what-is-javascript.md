...
metaDescription: >-
  JavaScript(자바스크립트)는 웹사이트에 생동감을 불어넣는 주인공입니다. 장바구니, 스크롤, 비밀번호 검증 같은 웹 동작이 어떻게
  이뤄지는지 비개발자도 한눈에 이해할 수 있도록 풀어 드리고, GA4, GTM 등 마케팅 분석과의 연결 고리까지 정리했습니다.
ogImage: /og/what-is-javascript.png
summary3:
  - 자바스크립트는 웹페이지에서 동작을 맡는 언어이고 장바구니 숫자가 바뀌거나 입력값을 검사하는 일이 모두 여기서 일어납니다.
  - 어떤 요소를 지켜보다가 클릭 같은 사건이 생기면 정해 둔 동작을 실행하는 구조로 움직입니다.
  - GTM의 스크롤과 클릭 트리거도 이 구조 위에서 돌아가므로 원리를 알면 이벤트가 안 잡히는 이유를 짚을 수 있습니다.
---
...
1. 자바스크립트는 웹페이지에서 동작을 맡는 언어이고 장바구니 숫자가 바뀌거나 입력값을 검사하는 일이 모두 여기서 일어납니다.
2. 어떤 요소를 지켜보다가 클릭 같은 사건이 생기면 정해 둔 동작을 실행하는 구조로 움직입니다.
3. GTM의 스크롤과 클릭 트리거도 이 구조 위에서 돌아가므로 원리를 알면 이벤트가 안 잡히는 이유를 짚을 수 있습니다.
