...
quiz:
  - question: 'AI에게 회의록 정리를 시킬 때, 같은 결과 품질을 매번 안정적으로 받기 위해 가장 효과적인 프롬프트 작성법은 무엇일까요?'
    options:
      - 역할, 맥락, 작업, 제약, 예시 다섯 가지를 갖춘 프롬프트로 작성한다
...
metaDescription: >-
  프롬프트 엔지니어링은 AI에게 일을 시킬 때 결과 품질을 정하는 지시 작성법입니다. 역할과 맥락, 작업, 제약, 예시 다섯 요소와 지시를
  고쳐 쓴 전후 예시, 결과가 부족할 때 다듬는 방법, 자주 하는 실수를 정리했습니다.
...
**1. 역할 (Role)**
- "한국 디지털 마케팅 전문가로서 답해 주세요." 같은 한 문장.
...
**3. 작업 (Task)**
- "다음 회의록을 결정 사항, 할 일, 미해결 이슈 세 가지로 정리해 달라."
...
**교육자**
- "다음 수업 자료(첨부)를 학생 눈높이의 한국어 요약 10줄로 만들어 달라. 영어 용어는 한국어 풀이를 괄호로 함께."

각 시나리오 모두 역할, 맥락, 작업, 제약을 모두 갖춘 형태입니다.

## 📋 3줄 요약

1. 프롬프트 엔지니어링은 AI에게 일을 시킬 때 결과 품질을 정하는 지시 작성법이고 모델을 바꾸기 전에 지시를 먼저 다듬는 쪽이 효과가 큽니다.

2. 좋은 지시는 역할과 맥락, 작업, 제약, 예시 다섯 가지를 갖춥니다.

3. 막연한 수식어에 기대거나 한 프롬프트에 일곱 가지가 넘는 작업을 묶으면 일부가 빠지므로 큰 작업은 단계로 나눕니다.
