---
slug: what-is-conversion
term: 전환 (Conversion)
definition: >-
  마케팅에서 사용자가 광고주가 원하는 특정 행동(구매, 가입, 문의 등)을 완료하는 것을 뜻하는 디지털 마케팅의 가장 핵심적인 성과
  지표입니다.
category: MARKETING
tags:
  - 광고
  - 전환
publishedAt: '2026-02-12T07:32:44.819Z'
updatedAt: '2026-07-23T00:00:00.000Z'
courseSlug: digital-marketing-terms
orderInCourse: 4
aliases:
  - 컨버전
  - 전환 행동
  - 목표 달성
relatedTerms:
  - what-is-cvr
  - what-is-cpc
  - what-is-ctr
  - what-is-roas
difficulty: BEGINNER
quiz:
  - options:
      - 고객이 상품을 구매 완료한 것
      - 고객이 상품을 장바구니에 담은 것
      - 고객이 상품 상세 페이지를 조회한 것
      - 고객이 뉴스레터를 구독한 것
    question: 온라인 쇼핑몰에서 마케팅 캠페인을 분석하고 있습니다. 다음 중 '매크로 전환'에 해당하는 행동은 무엇일까요?
    explanation: >-
      매크로 전환은 비즈니스의 핵심 목표에 해당하는 전환입니다. 이커머스에서는 '상품 구매 완료'가 매크로 전환이며, 장바구니 담기, 페이지
      조회, 뉴스레터 구독은 매크로 전환으로 이어지는 과정인 '마이크로 전환'에 해당합니다.
    correctIndex: 0
metaTitle: '전환(Conversion) 뜻: 마케팅에서 전환을 정하는 기준'
metaDescription: '전환은 사용자가 구매, 가입, 문의처럼 광고주가 원하는 행동을 완료하는 것입니다. 전환을 무엇으로 잡을지 정하는 기준과 전환 추적의 기본을 정리했습니다.'
ogImage: /og/what-is-conversion.png
summary3:
  - '전환은 사용자가 광고주가 원하는 행동을 끝까지 마치는 것이고 무엇을 전환으로 볼지는 업종마다 다릅니다.'
  - '구매나 가입 같은 매크로 전환과 장바구니 담기 같은 마이크로 전환을 함께 보면 어느 단계에서 빠지는지 드러납니다.'
  - 'CPM과 CPC, CTR은 과정을 보는 지표이고 사업의 결과를 보여주는 것은 전환입니다.'
---

## 🤔 혹시 이런 생각 해본 적 없나요?

- "광고 클릭은 많은데, 왜 매출은 안 오르지?"
- "우리 사이트에 방문자는 많은데, 실제로 구매하는 사람은 얼마나 될까?"
- "마케팅 보고서에서 '전환'이라는 단어가 계속 나오는데, 정확히 뭘 의미하는 거지?"

이 모든 질문의 핵심에 **전환(Conversion)**이 있습니다.

## 🔑 전환, 한마디로 뭘까요?

**전환(Conversion)**이란 사용자가 **광고주가 원하는 특정 행동을 완료**하는 것을 말합니다.

여기서 중요한 점은, 전환의 정의가 **비즈니스마다 다르다**는 것입니다.

- **이커머스**: 상품 구매 완료
- **SaaS 서비스**: 회원가입, 무료 체험 시작
- **교육 플랫폼**: 수강 신청, 상담 문의
- **앱 서비스**: 앱 설치, 첫 로그인
- **B2B**: 견적 요청, 데모 신청

즉, **"우리 비즈니스에서 가장 중요한 사용자 행동이 무엇인지"**를 먼저 정의해야 전환을 측정할 수 있습니다.

## 🎯 낚시와 같습니다

<div style="overflow-x:auto; margin:1.5rem 0;">
  <div style="max-width:100%; border:3px solid #000; background:#fff;">
    <div style="background:#FFD700; border-bottom:3px solid #000; padding:10px 16px; font-weight:700;">낚시로 보는 전환 3단계</div>
    <div style="display:flex; flex-wrap:wrap;">
      <div style="flex:1; min-width:140px; padding:14px 16px; border-right:3px solid #000;">
        <div style="font-weight:700;">1. 미끼를 던진다</div>
        <div style="font-size:0.9em; color:#555; margin-top:4px;">= 광고 노출</div>
      </div>
      <div style="flex:1; min-width:140px; padding:14px 16px; border-right:3px solid #000;">
        <div style="font-weight:700;">2. 물고기가 다가온다</div>
        <div style="font-size:0.9em; color:#555; margin-top:4px;">= 클릭</div>
      </div>
      <div style="flex:1; min-width:140px; padding:14px 16px; background:#FFD700;">
        <div style="font-weight:700; color:#FF0000;">3. 미끼를 문다</div>
        <div style="font-size:0.9em; margin-top:4px;">= 전환 (구매, 가입, 문의)</div>
      </div>
    </div>
  </div>
</div>

낚시하는 상황에 빗대면 이해가 빠릅니다.

- **미끼를 던지는 것**(= 광고 노출): 물고기에게 보여주는 단계
- **물고기가 다가오는 것**(= 클릭): 관심을 보이는 단계
- **물고기가 미끼를 무는 것**(= **전환**): 실제로 원하는 결과가 나오는 단계!

아무리 많은 물고기가 다가와도, **미끼를 물지 않으면 의미가 없습니다.** 마케팅도 마찬가지입니다. 노출과 클릭이 아무리 많아도, **전환이 없으면 비즈니스 성과로 이어지지 않습니다.**

## 📊 전환의 종류

**1. 매크로 전환 (Macro Conversion)**
비즈니스의 핵심 목표에 해당하는 전환입니다.
- 상품 구매, 유료 구독, 계약 체결
- 이것이 곧 매출과 직결됩니다.

**2. 마이크로 전환 (Micro Conversion)**
매크로 전환으로 가는 과정에서 발생하는 작은 전환입니다.
- 장바구니 담기, 뉴스레터 구독, 위시리스트 추가, 콘텐츠 다운로드
- 마이크로 전환을 추적하면 **어디서 이탈하는지** 파악할 수 있습니다.

**3. 직접 전환 vs 간접 전환**
- **직접 전환**: 광고 클릭 후 바로 전환이 발생
- **간접 전환**: 광고를 본 후 며칠 뒤에 직접 검색해서 전환 (기여 전환)

## 💼 실무에서 어떻게 활용할까요?

<div style="overflow-x:auto; margin:1.5rem 0;">
  <div style="max-width:100%; border:3px solid #000; background:#fff;">
    <div style="background:#FFD700; border-bottom:3px solid #000; padding:10px 16px; font-weight:700;">이커머스 전환 퍼널 예시</div>
    <div style="padding:16px;">
      <div style="border:3px solid #000; background:#F3F3F3; padding:8px 12px; max-width:100%;">사이트 방문 <span style="font-family:monospace; font-weight:700;">1,000명</span></div>
      <div style="font-size:0.85em; color:#555; padding:4px 0 4px 24px;">↓ 700명 이탈</div>
      <div style="border:3px solid #000; background:#F3F3F3; padding:8px 12px; width:70%;">장바구니 담기 <span style="font-family:monospace; font-weight:700;">300명</span> <span style="font-size:0.85em;">(마이크로 전환)</span></div>
      <div style="font-size:0.85em; color:#555; padding:4px 0 4px 24px;">↓ 250명 이탈</div>
      <div style="border:3px solid #000; background:#FFD700; padding:8px 12px; width:45%;">구매 완료 <span style="font-family:monospace; font-weight:700; color:#FF0000;">50명</span> <span style="font-size:0.85em;">(매크로 전환)</span></div>
    </div>
  </div>
</div>

*단계별 인원을 추적하면 사용자가 어디서 이탈하는지 바로 확인할 수 있습니다.*

**1. 전환 목표를 명확히 설정해야 합니다**

- 캠페인을 시작하기 전에 "이 캠페인의 전환은 무엇인지"를 먼저 정의합니다.

- 전환 정의가 불명확하면 성과 측정 자체가 불가능합니다.

**2. 전환 추적 도구를 설정해야 합니다**
- GA4 이벤트 설정, Meta 픽셀, Google Ads 전환 추적 태그 등
- 전환을 정의했다면, 실제로 측정할 수 있도록 기술적 세팅이 필요합니다.

**3. [CVR](/class/digital-marketing-terms/what-is-cvr), ROAS와 함께 종합적으로 분석합니다**
- 전환 수만 보면 안 됩니다. CVR(전환율)로 효율을, ROAS로 수익성을 함께 봐야 합니다.
- 전환 수가 많아도 광고비 대비 수익이 낮으면 개선이 필요합니다.

## 📋 3줄 요약

1. 전환은 사용자가 광고주가 원하는 행동을 끝까지 마치는 것이고 무엇을 전환으로 볼지는 업종마다 다릅니다.

2. 구매나 가입 같은 매크로 전환과 장바구니 담기 같은 마이크로 전환을 함께 보면 어느 단계에서 빠지는지 드러납니다.

3. CPM과 CPC, CTR은 과정을 보는 지표이고 사업의 결과를 보여주는 것은 전환입니다.

## 참고 자료

- [전환 정의, Google Ads 고객센터](https://support.google.com/google-ads/answer/6365?hl=ko)
