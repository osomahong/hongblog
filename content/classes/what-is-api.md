...
metaDescription: >-
  API는 서로 다른 프로그램이 소통하고 데이터를 주고받도록 만들어진 약속된 연결 통로입니다. 레스토랑 비유로 본 요청과 응답 구조, 공개 API와 파트너 API를 비롯한 네 가지
  종류, 마케터가 자동화에 쓰는 방법을 정리했습니다.
ogImage: /og/what-is-api.png
summary3:
  - 'API는 서로 다른 프로그램이 정해진 형식으로 데이터를 주고받도록 만든 약속된 연결 통로입니다.'
  - 'REST API는 엔드포인트라는 URL 주소로 요청을 보내고 응답을 받는 방식이라 구글 지도와 메타 광고 같은 웹 서비스 대부분이 이 방식을 씁니다.'
  - 'Zapier와 Make, n8n 같은 자동화 도구는 모두 API로 움직이므로 GA4 데이터를 시트에 저장하고 슬랙으로 알리는 흐름을 코드 없이 만들 수 있습니다.'
---
...
쉽게 말해, 여러분이 직접 프로그램 내부를 건드리지 않아도 **정해진 방식으로 요청하면 원하는 기능이나 데이터를 받을 수 있게 해주는 중간 다리** 역할을 합니다.
...
- Meta Ads API와 Google Ads API로 광고 성과 데이터를 자동으로 수집할 수 있습니다.
...
1. API는 서로 다른 프로그램이 정해진 형식으로 데이터를 주고받도록 만든 약속된 연결 통로입니다.

2. REST API는 엔드포인트라는 URL 주소로 요청을 보내고 응답을 받는 방식이라 구글 지도와 메타 광고 같은 웹 서비스 대부분이 이 방식을 씁니다.

3. Zapier와 Make, n8n 같은 자동화 도구는 모두 API로 움직이므로 GA4 데이터를 시트에 저장하고 슬랙으로 알리는 흐름을 코드 없이 만들 수 있습니다.
