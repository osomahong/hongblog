...
difficulty: BEGINNER
quiz:
  - question: 새로 발행한 블로그 글이 구글 검색에 전혀 나오지 않습니다. 가장 먼저 확인할 것은 무엇일까요?
    options:
      - 이 페이지가 색인되었는지 확인한다
      - 글에 노리는 키워드를 더 많이 넣는다
      - 글의 분량을 두 배로 늘린다
      - 광고를 집행해 방문자를 먼저 늘린다
    correctIndex: 0
    explanation: >-
      검색엔진은 발견, 저장, 표시 순서로 페이지를 처리합니다. 저장 단계인 색인을 통과하지 못한 페이지는 어떤 검색어에서도 나오지
      않습니다. 글을 고치거나 늘리는 일은 색인이 된 다음에야 의미가 생깁니다. 서치 콘솔의 URL 검사 도구로 색인 여부를 먼저 확인하는
      편이 빠릅니다.
...
여기서 중요한 것은 세 단계가 순서대로 일어난다는 점입니다.
...
SEO는 한 덩어리가 아니라 성격이 다른 세 갈래로 나뉩니다.
...
같은 단어를 반복해서 넣는 방식은 오래전에 효과가 사라졌습니다. 지금 검색엔진은 문서 전체가 무엇을 다루는지를 봅니다. 다만 사람들이 실제로 쓰는 표기를 한 번도 쓰지 않으면 그 검색어에서는 잡히기 어렵습니다. 반복이 아니라 누락이 문제입니다.
...
이 상황은 상품 수가 많은 사이트에서 자주 나옵니다. 검색엔진이 사이트 하나에 쓰는 시간에는 한도가 있어서, 안쪽 깊이 들어가야 나오는 페이지는 발견이 늦어집니다. 사이트맵에 상품 주소를 모아 알려 주거나, 카테고리 페이지에서 상품으로 이어지는 링크를 정리하면 나아집니다.
