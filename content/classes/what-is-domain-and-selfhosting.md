...
1. 도메인은 숫자로 된 IP 주소 대신 사람이 읽을 수 있게 붙인 서비스 주소이고 DNS가 그 이름을 IP로 바꿔 줍니다.

2. 도메인은 등록 업체에서 사며 클라우드플레어는 원가에 가까운 연 10달러 안팎이고 가비아는 한국어 지원과 .kr 도메인이 강점입니다.

3. 셀프호스팅은 남의 플랫폼 대신 직접 서버를 운영하는 방식이라 플랫폼 제한은 없어지지만 보안과 백업, 장애 대응까지 직접 맡아야 합니다.
