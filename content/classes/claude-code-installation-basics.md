...
term: '클로드 코드(Claude Code) 설치 따라하기: 터미널 입문 포함'
definition: >-
...
metaDescription: >-
  클로드 코드 설치는 맥과 리눅스는 터미널에, 윈도우는 파워셸에 명령 한 줄을 넣으면 끝납니다. 설치 전에 필요한 준비물과 첫 로그인 순서,
  IDE에 연결하는 방법, 비개발자가 처음 해 볼 작업, 자주 마주치는 에러 세 가지를 정리했습니다.
ogImage: /og/claude-code-installation-basics.png
summary3:
  - >-
    Claude Code 설치는 맥과 리눅스에서 curl -fsSL https://claude.ai/install.sh | bash 한 줄,
    윈도우 파워셸에서 irm https://claude.ai/install.ps1 | iex 한 줄이면 끝납니다.
...
1. Claude Code 설치는 맥과 리눅스에서 curl -fsSL https://claude.ai/install.sh | bash 한 줄, 윈도우 파워셸에서 irm https://claude.ai/install.ps1 | iex 한 줄이면 끝납니다.

2. 2026년 4월 기준 Claude Code를 쓰려면 Pro 이상 구독이나 Anthropic Console 계정이 있어야 합니다.

3. 설치한 뒤 새 터미널에서 claude를 실행해 브라우저로 로그인하고 작업할 폴더에 들어가 다시 실행하면 시작됩니다.
