...
이 세 가지 경로는 서로를 대신하지 못합니다. 사이트맵에 주소를 올려 두어도 어디서도 링크되지 않은 페이지는 발견이 늦어질 수 있습니다. **링크가 우선이고 사이트맵은 그것을 돕는 수단입니다.**
...
### 사이트맵만 제출하면 링크는 없어도 되나요?

두 경로는 서로를 대신하지 못합니다. 사이트맵에 주소를 올려 두어도 어디서도 링크되지 않은 페이지는 발견이 늦어질 수 있습니다. 링크가 먼저이고, 사이트맵은 링크를 적게 받는 페이지의 발견을 돕는 수단이라고 보는 편이 맞습니다.

### 회원만 볼 수 있는 페이지도 검색되나요?

검색 결과에 나올 수 없습니다. 크롤러는 로그인을 하지 않아서 회원 전용 페이지는 크롤러에게 보이지 않습니다. 이 상태는 고장이 아니라 의도한 대로 움직이는 것이라, 노출이 필요하면 일부 내용을 로그인 없이 볼 수 있게 열어야 합니다.

### 페이지가 적은 사이트도 크롤링 예산을 신경 써야 하나요?

스무 개 남짓인 사이트에서는 신경 쓸 일이 아닙니다. 상품이 수만 개인 쇼핑몰처럼 주소가 많은 곳에서 문제가 됩니다. 정렬이나 필터가 붙은 주소가 끝없이 만들어지거나 끊어진 링크가 쌓이면 정작 봐야 할 페이지에 크롤러가 늦게 도달합니다.

## 📋 3줄 요약

1. 검색엔진 크롤러는 인터넷의 링크를 따라다니며 페이지를 찾아 읽어 오는 자동 프로그램이고 구글은 구글봇, 네이버는 예티를 씁니다.

2. 크롤러가 페이지에 닿는 길은 다른 페이지에 걸린 링크와 사이트맵, 서치 콘솔의 URL 검사 요청 세 가지입니다.

3. 검색엔진이 사이트 한 곳에 쓰는 시간과 횟수에는 크롤링 예산이라는 한도가 있어서 쓸데없는 주소가 많으면 정작 볼 페이지가 늦게 발견됩니다.
