...
difficulty: BEGINNER
quiz:
  - question: 새로 만든 페이지를 검색엔진이 발견하게 하려면 무엇이 있어야 할까요?
    options:
      - 그 페이지로 이어지는 링크나 사이트맵 등록 같은 도달 경로
      - 페이지 본문에 반복해서 넣은 핵심 키워드
      - 다른 사이트보다 빠른 서버 응답 속도
      - 이미지와 영상이 충분히 들어간 풍부한 구성
    correctIndex: 0
    explanation: >-
      크롤러는 링크를 따라 이동하거나 사이트맵에 적힌 주소를 읽어서 페이지에 도착합니다. 어디서도 연결되지 않고 사이트맵에도 없는 페이지는
      주소를 아는 방법이 없어서 발견 자체가 일어나지 않습니다. 본문 내용이나 속도, 구성은 크롤러가 페이지에 도착한 다음에 평가되는
      항목입니다.
