...
quiz:
  - options:
    ...
    question: DOM을 바르게 설명한 것은?
    explanation: >-
      DOM은 브라우저가 HTML 문서를 읽고, 각 요소를 부모-자식 관계의 트리 구조(객체)로 변환한 것입니다. JavaScript나
      GTM은 이 DOM에서 특정 요소를 찾고 수정할 수 있습니다. HTML 원본 파일 그 자체가 아닌, 브라우저가 해석한
      결과물입니다.
    correctIndex: 2
...
summary3:
  ...
  - '화면에 똑같이 생긴 버튼이 여럿이어도 DOM의 계층 구조와 속성으로 어느 것을 눌렀는지 구분합니다.'
  - 'GTM 트리거의 CSS 선택자가 바로 이 주소여서 클릭이 안 잡히는 문제는 대개 선택자가 잘못됐거나 모호해서 생깁니다.'
---

## 🤔 혹시 이런 생각 해본 적 없나요?

- "[JavaScript](/class/digital-basic/what-is-javascript)는 어떻게 수많은 버튼 중에서 '구매하기' 버튼만 콕 집어서 색깔을 바꿀까?"
...
쉽게 말해, [HTML](/class/digital-basic/what-is-html)이 그냥 글자로 적힌 텍스트 문서라면, DOM은 그 문서를 분석해서 "이건 제목, 이건 본문, 이건 버튼"이라고 컴퓨터가 이해할 수 있는 객체(물건)로 바꿔놓은 상태를 말합니다.
...
1. 우리가 보는 웹사이트 화면은 도시의 겉모습입니다. 건물이 있고, 간판이 보이죠.
2. 하지만 택배 기사(JavaScript/GTM)는 겉모습만 보고는 집을 찾을 수 없습니다. "파란 지붕 집으로 가주세요"라고 하면 헷갈리니까요.
...
- [MDN Document Object Model 문서](https://developer.mozilla.org/ko/docs/Web/API/Document_Object_Model)
