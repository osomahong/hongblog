...
이름의 두 토막과 그것을 담는 상자 하나가 구조를 그대로 말해 줍니다.
...
GTM의 작업은 세 가지 이름으로 설명됩니다. **무엇을(태그) 언제(트리거) 어떤 값으로(변수)** 보낼지를 정하는 일이 GTM 작업의 대부분을 차지합니다.
...
세 가지가 맞물리는 모습은 결제 기록으로 따라가면 잡힙니다.
...
가장 자주 헷갈리는 지점입니다. 둘은 같은 일을 하는 다른 제품이 아닙니다.
...
적어 넣는 코드는 개발 담당자가 넣고, GTM을 쓰는 사람이 알 범위는 여기까지입니다. 적는 형식과 화면이 바뀌어도 값이 유지되게 하는 방법은 [React SPA에서 GTM 설치하기](/insights/react-spa-gtm-setup-guide)에 코드와 함께 적어 두었습니다.
...
- **미리보기를 건너뛰고 게시하기**: 트리거 조건을 잘못 적어도 게시 전에는 드러나지 않습니다. 게시한 뒤부터 데이터가 어긋나고, 그 기간의 기록은 다시 만들어 내지 못합니다
- **같은 이벤트를 양쪽에서 보내기**: GA4 코드를 직접 심어 둔 상태에서 GTM으로 같은 이벤트를 또 만들면 [전환](/class/digital-marketing-terms/what-is-conversion)이 두 번 집계됩니다. 전환 수가 커지면 그 숫자로 잡은 [KPI](/class/marketing-strategy-frameworks/what-is-kpi)와 광고 성과 판단이 함께 어긋납니다
...
컨테이너 코드를 처음 심는 작업과 데이터 영역에 값을 적어 넣는 작업은 코드를 고쳐야 해서 개발 담당자가 필요합니다. 그 두 가지가 끝난 뒤의 태그 추가와 조건 변경과 삭제는 관리 화면에서 처리합니다. 개발 담당자를 부르는 횟수를 줄이는 도구입니다.

### 태그를 몇 개까지 넣어도 되나요?

개수보다 무엇이 왜 들어 있는지 적혀 있는지가 중요합니다. 담당자가 바뀌면 이름만 보고는 용도를 알기 어려운 태그가 남고, 지워도 되는지 판단하지 못해 그대로 쌓입니다. 태그 이름에 어떤 도구의 어떤 이벤트인지 적어 두는 규칙이 있으면 정리가 쉬워집니다.
