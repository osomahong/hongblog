...
quiz:
  - options:
    ...
    question: 바이브코딩으로 회원가입 기능이 있는 웹 앱을 만들려고 합니다. AI가 Supabase를 추천했는데, Supabase에서 '사용자 정보를 어떤 구조로 저장할지 미리 정의한 것'을 무엇이라고 할까요?
    explanation: 스키마(Schema)는 데이터베이스에 어떤 데이터를 어떤 형태로 저장할지 미리 정의한 설계도입니다. 예를 들어 users 테이블에 name은 문자열, email은 고유한 문자열 같은 규칙을 정하는 것이 스키마 설계입니다. 바이브코딩에서도 AI에게 스키마를 먼저 설계해달라고 요청하면 더 안정적인 앱을 만들 수 있습니다.
    correctIndex: 1
---
