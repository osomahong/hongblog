...
1. 데이터베이스는 데이터를 정해진 구조로 저장하고 필요할 때 빠르게 찾아 쓰게 만든 저장소입니다.

2. 표 형태로 저장하고 자료 사이의 관계를 정의하는 SQL과 JSON에 가까운 문서 형태로 자유롭게 담는 NoSQL로 나뉩니다.

3. 스키마는 어떤 값이 어떤 형태로 들어갈지 미리 정해 두는 설계도이고 회원 관리가 필요하면 Supabase, 실시간 앱이면 Firebase가 자주 쓰입니다.
