...
흰 바탕에 검은 글씨로만 내용을 잔뜩 적은 PPT 슬라이드를 상상해 보세요. 그게 바로 HTML입니다. 내용은 있지만 보기에 예쁘지는 않죠.
...
브라우저는 이 주문서를 읽고, 그냥 텍스트였던 '구매하기'를 우리가 아는 '클릭하고 싶은 파란 버튼'으로 변신시켜 줍니다.
...
이것도 CSS가 하는 일입니다. "화면이 작아지면 배치를 바꿔라"라는 CSS 규칙을 이해하면, 모바일 최적화 마케팅에 대해 더 구체적인 아이디어를 낼 수 있습니다.

## 📋 30초 요약

1. CSS는 웹사이트의 인테리어이자 PPT 디자인 테마입니다.
2. HTML(내용)을 건드리지 않고도 디자인과 분위기를 바꿀 수 있게 해줍니다.
