...
1. 터미널은 마우스 대신 텍스트 명령으로 컴퓨터에 일을 시키는 도구이고 AI 코딩 도구가 명령을 실행하라고 할 때 쓰는 화면입니다.

2. 명령은 지금 있는 폴더를 기준으로 돌아가므로 명령이 듣지 않을 때는 pwd로 현재 위치부터 확인합니다.

3. npm과 git 같은 개발 도구 대부분이 터미널에서 돌아가고 sudo는 시스템 설정을 바꿀 수 있으므로 이유를 모르면 쓰지 않습니다.
