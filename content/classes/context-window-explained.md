...
metaTitle: '컨텍스트 윈도우 뜻과 모델별 크기 비교 (2026)'
metaDescription: >-
  컨텍스트 윈도우는 AI가 한 번의 대화에서 기억할 수 있는 텍스트 양입니다. 토큰의 의미, 대화가 길어지면 AI 답변이 나빠지는 이유, 현행 모델별 크기 비교까지
   쉽게 정리했습니다.
...
1. 컨텍스트 윈도우는 모델이 한 번의 대화에서 동시에 처리할 수 있는 입력과 출력 토큰의 총량입니다.

2. 2026년 7월 기준 Fable 5와 Opus 5, Sonnet 5는 1M 토큰이고 Haiku 4.5는 200K 토큰이라 한국어로는 단행본 한두 권과 절반에서 한 권 분량에 해당합니다.

3. 질문과 답이 모두 같은 창에 쌓여서 대화가 길어지면 남는 자리가 줄어들고 Opus 4.7 세대부터는 토크나이저가 바뀌어 같은 글이 약 30퍼센트 많은 토큰으로 계산됩니다.
