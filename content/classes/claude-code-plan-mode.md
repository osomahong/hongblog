...
1. **Plan Mode는 Claude Code가 실제 파일을 수정하기 전에 계획을 먼저 보여주는 모드**입니다. 사용자가 검토, 승인한 뒤에야 변경이 진행됩니다.

2. **`/plan` 슬래시 명령, Shift+Tab 단축키, IDE 토글로 진입**합니다. 여러 파일 수정, 되돌리기 어려운 변경, 비개발자의 영향 범위 확인에 가장 큰 효과가 납니다.

3. **계획을 동료와 공유 가능한 검토 자료로도 쓸 수 있습니다.** 계획서 형식만 보지 말고 경로, 의도까지 점검하고, 과도하게 큰 계획은 단계로 나눠 진행하세요.
