...
1. 데이터 구조는 정보를 담는 그릇의 모양이고 하나의 값을 담는 변수와 순서대로 나열하는 배열, 이름을 붙여 담는 객체 셋이 기본입니다.

2. JSON은 이 객체와 배열을 텍스트로 적은 데이터 교환 형식이라 프로그램끼리 값을 주고받을 때 표준으로 쓰입니다.

3. JSON은 자바스크립트 객체와 달리 키를 반드시 큰따옴표로 감싸야 해서 Unexpected token 오류는 대개 이 규칙을 어긴 데서 납니다.

JSON을 주고받는 정식 통로는 [REST API](/class/vibe-coding-basics/what-is-rest-api) 클래스에서 이어집니다.

## 참고 자료

- [json.org 한국어 소개](https://www.json.org/json-ko.html)
