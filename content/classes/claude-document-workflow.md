...
metaDescription: >-
  클로드 문서 워크플로는 PDF와 이미지, 엑셀, 워드 자료를 그대로 올려 분석하고 요약과 표로 바꾸는 작업 방식입니다. 자료 첨부와 지시,
  요약, 분석, 결과물로 이어지는 네 단계와 회의록, 보고서, 영수증, 외국어 자료 시나리오를 정리했습니다.
...
### 1M 컨텍스트 모델이면 100쪽 보고서를 한 번에 맡겨도 되나요?

파일은 통째로 올리되 요청은 나누는 편이 안전합니다. 한 번에 전부 시키면 자료 앞부분의 정확도가 떨어지기 때문에 요약과 키 인사이트, 다음 행동을 각각 따로 요청하는 순서를 씁니다. 결과를 받은 뒤에는 인용한 대목이 보고서 안에 실제로 있는지 몇 군데 골라 확인합니다.

### 스캔한 PDF나 사진은 OCR을 먼저 돌려야 하나요?

따로 돌리지 않아도 됩니다. claude.ai와 Claude API 모두 PDF와 PNG, JPG 같은 이미지를 그대로 받기 때문에 별도 전처리 없이 올려서 분석을 시킬 수 있습니다. 영수증 사진처럼 여러 장을 한꺼번에 올려 표로 정리하는 작업도 같은 방식으로 진행합니다.

### 회사 내부 자료도 그대로 올려도 되나요?

가명화나 마스킹을 거친 뒤 올립니다. 고객 개인정보와 미공개 자료는 식별할 수 있는 부분을 지우고 넣는 것이 기본입니다. 자료를 올리기 전에 조직의 보안 기준을 확인하는 단계를 워크플로에 넣어 두면 급할 때도 빠뜨리지 않습니다.
