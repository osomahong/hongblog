...
metaDescription: >-
  쿠키는 웹사이트가 방문자의 브라우저에 저장하는 작은 텍스트 파일입니다. 퍼스트파티와 서드파티, 세션과 영구 쿠키의 구분, GA4와 메타 픽셀이 심는 추적 쿠키, 서드파티 쿠키
  차단이 리타게팅과 전환 추적에 미친 영향을 정리했습니다.
ogImage: /og/what-is-cookie.png
summary3:
  - '쿠키는 웹사이트가 방문자의 브라우저에 남기는 작은 텍스트 파일이고 로그인 상태와 장바구니, 방문 기록을 담습니다.'
  - 내가 연 사이트가 직접 만드는 퍼스트파티 쿠키와 그 안에 실린 다른 도메인이 만드는 서드파티 쿠키는 쓰임이 다릅니다.
  - 2024년부터 크롬이 서드파티 쿠키를 단계적으로 막으면서 서드파티 쿠키에 기대던 리타겟 광고의 조건이 달라졌습니다.
---
...
여러분이 백화점 회원카드를 만들면, 그 카드에 고객번호가 적혀있죠? 쿠키도 똑같습니다. 웹사이트가 여러분의 브라우저에 "이 브라우저는 user12345번 고객입니다"라는 작은 파일을 저장해둡니다. 다음에 그 사이트를 다시 방문하면, 브라우저가 자동으로 그 파일을 웹사이트에 보여주면서 "저 왔어요!"라고 알려주는 겁니다.
...
1. 쿠키는 웹사이트가 방문자의 브라우저에 남기는 작은 텍스트 파일이고 로그인 상태와 장바구니, 방문 기록을 담습니다.

2. 내가 연 사이트가 직접 만드는 퍼스트파티 쿠키와 그 안에 실린 다른 도메인이 만드는 서드파티 쿠키는 쓰임이 다릅니다.

3. 2024년부터 크롬이 서드파티 쿠키를 단계적으로 막으면서 서드파티 쿠키에 기대던 리타겟 광고의 조건이 달라졌습니다.
