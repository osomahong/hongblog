...
Fable과 같은 사양의 **Claude Mythos**도 함께 발표되었지만, 이 모델은 일반 사용자용이 아닙니다. 앤트로픽의 Project Glasswing에 참여하는 승인된 조직만 초대 방식으로 쓸 수 있습니다. 아래에서 다시 설명합니다.

## 📊 현행 라인업 한눈에 비교 (2026년 9월 14일 기준)

| 항목 | Claude Fable 5.1 | Claude Opus 5 | Claude Sonnet 5 | Claude Haiku 4.5 |
|---|---|---|---|---|
| **API 모델 ID** | claude-fable-5-1 | claude-opus-5 | claude-sonnet-5 | claude-haiku-4-5 |
| **가격(입력)** | $10 / 1M 토큰 | $5 / 1M 토큰 | $2 / 1M 토큰 | $1 / 1M 토큰 |
| **가격(출력)** | $50 / 1M 토큰 | $25 / 1M 토큰 | $10 / 1M 토큰 | $5 / 1M 토큰 |
| **컨텍스트 윈도우** | 1M 토큰 | 1M 토큰 | 1M 토큰 | 200K 토큰 |
| **최대 출력** | 128K 토큰 | 128K 토큰 | 128K 토큰 | 64K 토큰 |
| **상대적 속도** | 느림 | 보통 | 빠름 | 가장 빠름 |
| **신뢰 가능한 지식 기준일** | 2026년 6월 | 2026년 5월 | 2026년 1월 | 2025년 2월 |
| **기본 effort** | high | high | high | 미지원 |
| **Adaptive thinking** | 지원(항상 켜짐) | 지원 | 지원 | 미지원 |
| **Extended thinking** | 미지원 | 미지원 | 미지원 | 지원 |

> **참고**: 위 수치는 platform.claude.com의 공식 모델 개요 문서를 2026년 9월 14일에 옮긴 것입니다. Sonnet 5의 입력 $2, 출력 $10은 도입 가격이 아니라 확정 가격입니다. 출시 당시에는 9월 1일부터 $3과 $15로 오른다고 안내됐다가 인상이 철회됐습니다. 자세한 경위는 [소네트 5 가격 확정 정리](/insights/claude-sonnet-5-price-permanent)에 있습니다. 이전 세대인 Opus 4.8, Opus 4.7, Sonnet 4.6 등은 레거시 모델로 여전히 사용할 수 있습니다.

## 🚀 Fable 5와 Mythos 5, 새로 더해진 최상위 라인업

Fable은 앤트로픽이 일반 공급하는 라인업 가운데 **가장 성능이 높은 자리**입니다. 2026년 6월 9일 Fable 5로 시작했고, 2026년 9월 1일 **Fable 5.1**이 나오면서 지금 쓰이는 번호가 5.1로 올라갔습니다. 달라진 점은 [페이블 5.1과 미토스 5.1 업데이트 정리](/insights/claude-fable-5-1-mythos-5-1-update)에 있습니다. 공식 문서는 이 모델을 **"Next-generation intelligence for long-running agents"**(장시간 실행되는 에이전트를 위한 차세대 지능)로 소개합니다. 발표문에는 통상 두 달 걸릴 것으로 예상된 코드베이스 이전 작업을 Stripe가 하루 만에 마쳤다는 사례가 실려 있습니다.
...
### Claude 5가 나왔는데 Opus는 이제 못 쓰나요?

그대로 쓸 수 있습니다. Claude는 하나의 모델이 아니라 Fable, Opus, Sonnet, Haiku 네 라인업이 함께 돌아가는 패밀리입니다. Fable이 최상위 자리에 더해졌을 뿐 아래 세 라인업은 그대로 운영됩니다. 앤트로픽 공식 문서도 대부분의 작업을 Opus 5로 시작하라고 안내합니다.

### 어떤 모델부터 써 보면 되나요?

Opus 5가 기본값입니다. 공식 문서가 대부분의 업무를 Opus 5로 시작하라고 적어 두었고, 까다로운 추론이나 오래 걸리는 에이전트 작업에서 Opus 5로 부족할 때 Fable 5.1로 올리는 순서를 권합니다. 반대로 분류나 1차 응대처럼 정확도가 일정 수준만 되면 되는 작업은 Haiku로 내립니다.

### Sonnet 5 가격이 오른다고 들었는데 맞나요?

오르지 않았습니다. 출시 당시에는 도입가 입력 $2와 출력 $10이 2026년 8월 31일까지라고 안내됐지만 인상이 철회되어 확정 가격이 됐습니다. 9월부터 $3과 $15로 오른다고 적힌 글은 그 이전 기준입니다.

### Fable 5.1과 Mythos 5.1은 무엇이 다른가요?

사양과 가격은 같고 안전장치가 다릅니다. Fable은 모든 사용자에게 열리고 영역별 안전 분류기가 함께 돌아갑니다. Mythos는 일반 판매용이 아니라 Project Glasswing에 참여하는 승인된 조직처럼 검증을 거친 곳에만 일부 안전장치를 푼 상태로 제공됩니다.

### 모델 ID는 어디에 쓰나요?

API로 호출할 때 어떤 모델을 쓸지 지정하는 값입니다. 화면에서 클로드를 쓰는 경우에는 몰라도 되고, 직접 코드를 짜서 부르거나 도구에 모델을 지정할 때 필요합니다. 위 표의 API 모델 ID 행에 라인업별 값이 있습니다.
