...
보장되지 않습니다. 사이트맵은 주소를 알려 주는 데까지 관여하고, 저장할지는 검색엔진이 색인 단계에서 따로 판단합니다. 넣었는데 색인되지 않는다면 중복 내용과 얇은 본문, noindex 표시 가운데 무엇에 걸렸는지 확인하는 것이 순서입니다.

### lastmod 날짜는 자주 바꿀수록 좋은가요?

실제로 글을 고쳤을 때만 바꿉니다. 내용을 고친 뒤 이 날짜를 갱신하면 크롤러가 다시 와서 확인할 이유가 생기지만, 고치지도 않은 글의 날짜를 매일 바꾸면 신뢰를 잃습니다. 네 항목 가운데 실제로 신경 쓸 값이기도 합니다.

### noindex를 걸어 둔 페이지도 사이트맵에 넣나요?

빼는 쪽이 맞습니다. 넣어 두면 검색엔진에 반대되는 신호를 동시에 보내는 셈이 됩니다. 색인에 넣지 말라고 하면서 여기 이런 페이지가 있다고 알려 주는 상태가 되기 때문입니다.
