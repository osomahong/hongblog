...
사라지지 않습니다. 크롤링을 막는 파일이라 크롤러가 페이지 안에 적어 둔 noindex 표시까지 읽지 못하고, 그래서 이미 저장된 색인이 그대로 남습니다. 검색에서 빼는 것이 목적이라면 크롤링은 열어 둔 채 noindex를 넣고, 색인이 빠진 뒤에 필요하면 그때 막습니다.

### 차단 목록에 적으면 그 경로가 감춰지나요?

오히려 알려집니다. 이 파일은 주소 뒤에 robots.txt를 붙이면 누구나 열어 볼 수 있어서, 막아 둔 경로가 그대로 공개됩니다. 외부에 알려지면 곤란한 정보는 로그인을 걸거나 서버에서 접근을 막는 방식으로 다뤄야 합니다.

### 스타일 파일이나 스크립트 폴더도 막아도 되나요?

막지 않는 편이 좋습니다. 크롤러는 사람이 보는 화면과 같은 모습으로 페이지를 확인하는데, 이 파일들을 받지 못하면 화면을 그리지 못해 본문이 있는지도 판단하기 어려워집니다. 모바일 화면 평가에도 영향을 주므로 용량을 아끼려는 차단은 다른 곳에서 합니다.
