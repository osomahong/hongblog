...
summary3:
  - 'robots.txt는 사이트 최상위에 두는 텍스트 파일로 크롤러에게 어느 경로를 방문하지 말라고 알리는 규칙 목록입니다.'
  - '강제력이 있는 장치가 아니라 요청이라서 지키지 않는 크롤러도 있고 막아 둔 경로가 있다는 사실 자체는 드러납니다.'
  - '검색 결과에서 빼려면 robots.txt로 막지 말고 페이지에 noindex를 넣어야 하며 막아 버리면 그 표시를 읽지 못해 색인이 남습니다.'
---
...
1. robots.txt는 사이트 최상위에 두는 텍스트 파일로 크롤러에게 어느 경로를 방문하지 말라고 알리는 규칙 목록입니다.

2. 강제력이 있는 장치가 아니라 요청이라서 지키지 않는 크롤러도 있고 막아 둔 경로가 있다는 사실 자체는 드러납니다.

3. 검색 결과에서 빼려면 robots.txt로 막지 말고 페이지에 noindex를 넣어야 하며 막아 버리면 그 표시를 읽지 못해 색인이 남습니다.
