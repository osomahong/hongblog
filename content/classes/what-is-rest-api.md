...
1. REST API는 서로 다른 프로그램이 인터넷으로 데이터를 주고받도록 정해 둔 요청과 응답의 규칙입니다.

2. 요청에는 가져오는 GET과 새로 보내는 POST, 고치는 PUT, 지우는 DELETE 네 가지 메서드를 쓰고 데이터는 대개 JSON으로 오갑니다.

3. API 키는 누가 얼마나 썼는지 확인하는 값이라 코드에 직접 적지 않고 응답 코드가 200이면 성공이고 401이면 인증 실패입니다.
