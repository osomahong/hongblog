...
term: REST API
definition: 서로 다른 프로그램이 인터넷으로 데이터를 주고받을 수 있도록 정해진 규칙으로 소통하는 방식입니다.
category: AI_TECH
tags:
  - 바이브코딩
  - API
...
손님은 주방에 직접 들어갈 수 없습니다. 대신 **웨이터(API)에게 정해진 방식으로 요청**하면, 원하는 결과를 받을 수 있습니다. API도 마찬가지입니다. 여러분의 프로그램이 다른 서비스에 직접 접근할 수 없지만, **정해진 규칙(API)**으로 데이터를 요청하고 응답받을 수 있습니다.
