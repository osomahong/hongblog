...
**인증(Authentication)**은 "접속한 사람이 누구인지" 확인하는 과정입니다. 현실에서 비유하면 이렇습니다.
...
1. 인증은 접속한 사람이 누구인지 확인하는 절차이고 보안은 허가된 사람만 데이터에 닿게 하는 장치입니다.

2. 로그인 상태를 유지하는 방법은 서버가 정보를 들고 브라우저에 세션 ID만 주는 세션 방식과 정보를 암호화한 토큰을 브라우저에 넘기는 JWT 방식으로 나뉩니다.

3. AI에게 코드를 맡길 때 자주 나는 사고는 API 키를 코드에 그대로 적는 것과 삭제 같은 위험한 API 경로에 인증 검사를 빠뜨리는 것입니다.
