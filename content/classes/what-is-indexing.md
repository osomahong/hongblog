...
1. 색인은 검색엔진이 크롤러가 받아 온 페이지를 분석해 자기 데이터베이스에 저장하는 단계이고 색인되지 않으면 어떤 검색어로도 나오지 않습니다.

2. 크롤링은 주소를 알면 방문하는 단계라 판단이 거의 없지만 색인은 저장할 가치가 있는지 검색엔진이 판단하는 단계입니다.

3. 서치 콘솔에 크롤링됨으로 나오면 받아 갔지만 저장하지 않은 상태이므로 경로가 아니라 페이지 내용 자체를 고쳐야 합니다.
