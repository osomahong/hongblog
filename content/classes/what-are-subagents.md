...
quiz:
  - question: 서브에이전트가 가장 큰 효과를 내는 작업 유형은 무엇일까요?
    ...
    explanation: >-
      ...
      서브에이전트가 과합니다.
metaTitle: '서브에이전트(Subagent) 뜻과 만드는 법'
metaDescription: >-
  서브에이전트(Subagents)는 한 작업의 부분을 나눠 처리하는 보조 에이전트입니다. 주력 에이전트가 작업을 분배하고 결과를 합치는 흐름, .claude/agents 폴더에
  정의하는 방법, 자주 쓰는 유형과 개수를 늘릴 때 생기는 문제를 정리했습니다.
ogImage: /og/what-are-subagents.png
summary3:
  - '서브에이전트는 큰 작업의 부분을 나눠 동시에 처리하고 주력 에이전트가 그 결과를 모아 합치는 보조 에이전트입니다.'
  - '프로젝트의 .claude/agents 폴더에 마크다운으로 정의하며 코드 검토와 테스트 작성, 자료 조사처럼 성격이 분명한 작업에 맞습니다.'
  - '자주 쓰지 않는 서브에이전트가 많으면 어느 것을 부를지 흐려지므로 다섯에서 일곱 개 정도로 두고 역할이 겹치지 않게 나눕니다.'
---
...
1. 서브에이전트는 큰 작업의 부분을 나눠 동시에 처리하고 주력 에이전트가 그 결과를 모아 합치는 보조 에이전트입니다.

2. 프로젝트의 .claude/agents 폴더에 마크다운으로 정의하며 코드 검토와 테스트 작성, 자료 조사처럼 성격이 분명한 작업에 맞습니다.

3. 자주 쓰지 않는 서브에이전트가 많으면 어느 것을 부를지 흐려지므로 다섯에서 일곱 개 정도로 두고 역할이 겹치지 않게 나눕니다.
