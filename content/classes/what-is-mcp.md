...
1. **MCP는 AI 애플리케이션을 외부 시스템과 연결하는 오픈소스 표준 규격**입니다. "AI를 위한 USB-C 포트"라는 비유로 modelcontextprotocol.io 공식 페이지에 설명되어 있습니다.

2. **앤트로픽이 제안했고 Claude·ChatGPT·VS Code 등이 모두 채택**하고 있습니다. 한 번 MCP 서버를 만들면 여러 AI 애플리케이션에서 같이 씁니다.

3. **노션·슬랙·캘린더·깃허브·파일 시스템 같은 도구를 표준 방식으로 Claude에 연결**할 수 있습니다. [Skills](/class/claude-in-practice/what-are-claude-skills)와 묶이면 자동화 깊이가 커지고, 권한 범위·인증 분리·신뢰할 만한 서버 사용이 표준 보안 원칙입니다.
