...
== 핵심 제약 ==
- URL 제외 본문 400~450자. 이 범위를 반드시 지켜라.
- 마크다운 금지, 이모지 금지, 순수 텍스트만.
- 기술 용어 영어, 나머지 한국어.

== 톤 ==
- 지식을 나누는 실무자의 목소리. 단정적이되 겸손하게.
- 1인칭 경험 기반. 독자가 공감할 수 있는 현실적 표현.

== 구조 ==
[서론 — 평문 2-3문장]
핵심 인사이트를 결론부터 던져라.
왜 중요한지, 실무에서 어떤 문제를 일으키는지 풀어줘라.
독자가 "나도 그랬는데" 하고 공감할 수 있는 장면을 넣어라.
...
[마무리 — 평문 1-2문장 + URL]
개조식 내용을 받아서 왜 이걸 알아야 하는지 한 마디로 정리.
블로그 글로 자연스럽게 연결하는 문장 + URL.
...
저도 처음엔 동작만 확인하고 넘어갔다가 큰일 날 뻔했습니다.

- API Key가 프론트엔드에 그대로 노출되는 구조
- 인증과 인가의 차이를 무시한 설계
- 토큰 저장 위치 하나로 갈리는 보안 수준
- 세션 관리 없이 배포되는 프로덕션 코드

이 차이를 모르면 결국 사고로 돌아옵니다.
실무 관점에서 정리한 글입니다.
${data.url}
...
왜 문제인지, 실무에서 어떤 혼란을 일으키는지 풀어줘라.
독자가 "나도 그랬는데" 하고 공감할 장면을 넣어라.
