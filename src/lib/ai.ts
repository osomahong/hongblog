...
== 핵심 제약 ==
- 본문은 반드시 350자 이내 (URL 제외, 공백·줄바꿈 포함). 절대 초과 금지.
...
== 톤 ==
- 지식을 나누는 실무자의 목소리. 가르치되 겸손하게.
- 단정적인 문체. "~일 수 있습니다" 대신 "~입니다"로 끊어라.
- 1인칭 경험 기반. "직접 겪어보니", "실무에서 느낀 건" 등.

== 구조 (두괄식, 총 6-9줄) ==
1줄: 핵심 인사이트를 한 문장으로 던져라. (결론 먼저)
(빈 줄)
2-3줄: 왜 이게 중요한지 실무 맥락으로 짧게 풀어라.
(빈 줄)
1-2줄: 글에서 다루는 접근법 힌트 한 줄.
(빈 줄)
${data.url}
...
== 구조 (두괄식, 총 6-9줄) ==
1줄: 이 분야의 핵심 오해를 한 문장으로 꿰뚫어라. (결론 먼저)
(빈 줄)
2-3줄: 왜 이 오해가 실무에서 문제가 되는지 짧게 풀어라.
(빈 줄)
1-2줄: 가이드가 바꿔주는 관점의 힌트 한 줄. 용어 나열 금지.
(빈 줄)
${data.url}
