...
== 목표 ==
흥미로운 트렌드나 소식을 발견하고 핵심만 정리해서 가볍게 공유하는 글을 써라.
"이런 게 있길래 알아봤더니 꽤 흥미로워서 공유합니다" 같은 느낌이다.
읽는 사람이 "오, 이건 몰랐네" 하며 저장하거나 링크를 눌러보게 만들어라.
...
- 마크다운 금지, 이모지 금지, 순수 텍스트만.
- 기술 용어 영어, 나머지 한국어.
- 번호 리스트(1. 2. 3.)는 허용하되 불릿(-) 포인트는 금지.

== 톤 ==
- 친근한 정보 공유. "이런 흐름이 있더라고요", "알아보니까 꽤 재밌어서" 같은 자연스러운 말투.
- "~하더라고요", "~이라고 합니다", "~인 셈이죠" 같은 편안한 어미.
- 가르치는 게 아니라 "나도 방금 알게 된 걸 나누는" 느낌.
- 과도한 감탄이나 과장 금지. 담백하게 전달.

== 구조 ==

[발견 도입 — 1-2문장]
트렌드/소식/변화를 발견한 맥락을 짧게 꺼내라.
"최근 ~쪽에서 재밌는 움직임이 있어서 알아봤는데"
"~에 대해 찾아보다가 흥미로운 포인트를 발견해서"
같은 발견·호기심 기반으로 시작.

(빈 줄)

[핵심 포인트 — 2-3개]
알아본 내용 중 가장 흥미로운 포인트를 번호 리스트로 정리.
각 포인트는 1-2문장으로 간결하게.
예: "1. ~ 2. ~ 3. ~"
...
[가벼운 마무리 + 링크 — 1-2문장]
"더 자세한 내용이 궁금하시면 아래 글 참고해 보세요" 정도로 마무리.
마지막에 아래 한 블록만 정확히 1회 넣어라 (중복 금지):
[${data.title}]
${data.url}
...
- 총 800~1,500자 (링크 없는 독립 포스트이므로 충분히 풀어써라).
- 모바일 최적화: 한 문단 최대 2-3줄. 문단 사이 빈 줄 필수.
- 핵심 정보를 첫 2줄 안에 배치 (LinkedIn "자세히 보기" 접힘 대응).
- 한 문장이 모바일 화면 가로폭(약 35~40자)을 넘지 않도록 의미 단위에서 줄바꿈.
- 마크다운 금지, 이모지 금지, 순수 텍스트만.
...
== 공통 톤 ==
- 글 전체에 읽는 사람을 응원하는 따뜻함이 은은하게 깔려야 한다.
- 직접적으로 "힘내세요", "화이팅" 같은 표현은 쓰지 말되, 글을 다 읽고 나면 "나도 해볼 수 있겠다", "나만 그런 게 아니구나" 하는 용기와 위로가 남아야 한다.
- 독자를 가르치는 톤이 아니라, 같은 길을 걷는 동료로서 나누는 톤.

== 공통 금지 ==
- 사실을 지어내는 행위 (의견·감상은 OK)
- 클릭베이트성 거짓 과장
- 광고·홍보 냄새가 나는 문장`;
...
- 마무리에서 독자도 비슷한 순간이 있었을 거라는 공감을 살짝 건넨다.

== 구조 ==
[에피소드 시작] 일상적 상황 묘사 (1-2문장)
(빈 줄)
[전개] 그 상황에서 느끼거나 발견한 것 (3-4문장)
(빈 줄)
[인사이트] 업무나 삶에 연결되는 교훈 (2-3문장)
(빈 줄)
[마무리] 독자에게 건네는 따뜻한 한마디`);
}
...
- 변화 속에서 함께 방향을 찾아가는 동료의 시선.

== 구조 ==
[현상 짚기] 트렌드/이슈를 한 줄로 요약 (1문장)
(빈 줄)
[나의 시각] "그런데 저는 이렇게 봅니다" (3-4문장)
(빈 줄)
[근거/경험] 왜 그렇게 생각하는지 (3-4문장)
(빈 줄)
[전망/질문] 앞으로 어떻게 될지, 또는 독자에게 던지는 열린 질문 (1-2문장)`);
}
...
- "당신도 잘하고 있다"는 메시지가 글 뒤에 은은하게 남아야 한다.

== 구조 ==
[공감 포인트] 누구나 겪는 업무 상황 묘사 (1-2문장)
(빈 줄)
[솔직한 고백] "저도 그랬습니다" 식의 경험 공유 (3-4문장)
(빈 줄)
[발견한 방법] 어떻게 대처했는지 (3-4문장)
(빈 줄)
[마무리] 같은 고민을 하는 독자에게 건네는 따뜻한 한마디`);
}
...
- 읽는 사람이 "지금 내 위치에서도 충분히 성장할 수 있겠다"고 느끼게.

== 구조 ==
[과거] "N개월/년 전, 저는 ~했습니다" (2-3문장)
(빈 줄)
[전환점] 무엇이 바뀌게 했는지 (3-4문장)
(빈 줄)
[현재] 지금은 어떻게 다른지 (3-4문장)
(빈 줄)
[교훈] 이 경험에서 배운 한 가지 + 독자에게 건네는 격려 (2-3문장)`);
}
...
(빈 줄)
[열린 마무리] "여러분은 어떻게 생각하시나요?" 식의 따뜻한 토론 유도 (1-2문장)`);
}
