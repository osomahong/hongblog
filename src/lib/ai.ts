...
규칙:
- 한 문장당 한 줄. 절대 한 줄에 두 문장 넣지 마라.
- 문장은 짧게. 최대 30자 이내로.
- 줄과 줄 사이에 빈 줄을 넣어서 숨 쉬는 공간을 만들어라.
- 전체 15줄 이내 (빈 줄 포함).
- 글의 결론이나 해결책은 절대 공개하지 마라. 궁금하게 만들어라.
- 이모지 사용 금지.
- 마크다운 문법 금지. 순수 텍스트만.
- 기술 용어는 영어, 나머지는 한국어.
...
1줄: 스크롤을 멈추게 하는 한 줄. 도발적 질문이나 반직관적 사실.

(빈 줄)

2-4줄: 공감 또는 문제 상황. 짧은 문장 2-3개.

(빈 줄)

5-7줄: 힌트만 던져라. 답은 숨기고 방향만 제시.
...
나쁜 예 (장황함):
"GA4에서 Direct / (none) 트래픽이 갑자기 늘어나는 현상을 경험해보셨나요? UTM 세팅 문제일 수도 있지만, 실제로는 다양한 원인이 복합적으로 작용합니다."

좋은 예 (SNS 가독성):
"GA4 Direct 트래픽이 갑자기 늘었다면.

UTM 문제라고 생각하기 쉽습니다.
근데 진짜 원인은 다른 데 있을 수 있습니다.
...
원문 정보:
제목: ${data.title}
내용: ${data.content.substring(0, 1500)}
URL: ${data.url}
...
- 줄과 줄 사이에 빈 줄을 넣어서 숨 쉬는 공간을 만들어라.
- 전체 15줄 이내 (빈 줄 포함).
- 용어 정의나 목록을 절대 나열하지 마라. 궁금하게 만들어라.
- 이모지 사용 금지.
- 마크다운 문법 금지. 순수 텍스트만.
- 기술 용어는 영어, 나머지는 한국어.
...
1줄: 이 분야에서 흔히 겪는 혼란이나 오해를 짧게 던져라.

(빈 줄)

2-4줄: 공감 포인트. "이거 헷갈리지 않나요?" 느낌으로 짧게.

(빈 줄)

5-7줄: 가이드의 존재만 언급. 내용은 숨겨라.
