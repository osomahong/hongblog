...
아래 규칙은 AEO 답변 엔진 인용 가치와 초보자 친화성을 동시에 확보하기 위한 필수 항목이다. 위반 시 content-reviewer가 REVISE 판정을 내린다.

- **"독자"라는 단어 금지.** 대체: "이 글을 읽는 분들", "여러분".
- **추상 영어 용어 금지.** `레이어`, `페어`, `스택` 등 → `기능`, `Q&A`, `쌓인 구조` 등 구체 한국어로 대체. (기술 식별자인 `<head>`, `JSON-LD`, `canonical` 등 표준 용어는 예외)
- **군더더기 표현 제거.** "Q&A 쌍" → "Q&A", "~예시로 씁니다" → "~예시로 들어 설명합니다", "~연결해 보여드리겠습니다" → "~보여드리겠습니다".
- **병렬 설명 3개 이상은 불릿 리스트.** 한 문단에 "A는 ~, B는 ~, C는 ~" 형태 3개 이상 나열 시 `- **용어**: 설명` 형식으로 분리.
- **전문 개념 섹션 초입에 초보자 친화 브리지 문장 1~2개 필수.** 구조: "①낯섦 인정 → ②(선택) 완결형 재정의 → ③실제 예시 예고". 예시: "JSON-LD라는 단어 자체가 처음 듣는 개념일 수 있습니다. 쉽게 말해 페이지가 AI에게 자기 정체를 알려주는 표준 자기소개 양식입니다. 지금 이 글이 실제로 내보내는 자기소개를 그대로 보여드리겠습니다."

이 브리지 문장은 **완결된 자연어 정의형 문장**으로 작성한다. 음성 검색·AI Overviews가 그대로 답변으로 가져갈 수 있는 형태여야 한다.

## H2 헤딩 종결 톤 규칙 (필수)

`src/app/insights/[slug]/page.tsx:83-128`의 `extractFaqPairs()`가 `[?？]\s*$`로 끝나는 H2만 FAQPage Q&A로 추출한다. 물음표 누락은 자동 CRITICAL이다.

- **질문형 H2는 반드시 `?`로 끝냄** (전각 `？`도 허용)
- **종결 톤은 존댓말 의문형 통일**: `~을까요?`, `~할까요?`, `~인가요?`, `~나요?`, `~걸까요?`
- **반말 의문형 금지**: `~는가`, `~하는가`, `~되는가`, `~할까`, `~인지`, `~뭘까`
- **"용어란? 부연 의문구" 중간 물음표 패턴 금지**: `## CTR이란? 광고 소재가 관심을 끌고 있는가` → `## CTR(클릭률)은 광고 소재가 관심을 끌고 있는지 어떻게 알려줄까요?`로 합침
- 비질문형(요약·정리·Sources)은 예외로 명사구 유지 가능

## 시간 민감 정보 시점 명시 규칙

가격·플랜·버전·시장 수치 등 **시간이 지나면 사실이 달라질 수 있는 정보**는 해당 블록 앞·내부에 "YYYY년 M월 기준" 시점을 명시한다.

**대상**:
- 구독 플랜·가격 (`$20`, `$100`, `Pro/Max/Team/Enterprise`)
- SDK·도구 버전 (`v2.1.83`, `Opus 4.7`)
- 베타·연구 미리보기 단계
- 벤더 정책 결정 (A/B 테스트, 차단 규칙)
- 시장 통계·점유율

**권장 문구**: "아래 표는 **2026년 4월 기준** 공식 요구 사항이며, 시점이 지나면 달라질 수 있습니다"

자세한 원칙과 기준은 `.claude/skills/write-insight/SKILL.md`와 `.claude/skills/content-ops/references/aeo-checklist.md`를 참조한다.
