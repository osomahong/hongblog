...
> **2026-09-14 예외.** 사용자가 이 건에 한해 규칙을 열었다("크론으로 자동 실행해도 돼, 규칙 열어줄게").
> 범위는 셋으로 좁힌다. 카피를 이미 사람이 승인했을 것, 하는 일이 **초안을 예약으로 올리는 것뿐**일 것,
> 새 카피를 만들거나 발행하지 않을 것. 예약 상한 때문에 초안으로 밀린 건을 슬롯이 빈 뒤 올리는
> 경우가 여기 해당한다. **카피 생성과 `--publish`는 여전히 대화형 세션 전용이다.**
