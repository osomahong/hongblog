...
const RULES = [
    ...
    // ── 문어체 동사·부사 ──
    // 해석을 피동으로 숨긴 것. "~라는 신호로 읽힙니다", "방향이 읽힙니다", "사실처럼 읽힙니다"는
    // 누가 그렇게 보는지 없는 논설 어투다. "~로 보입니다", "~라는 뜻입니다", "~를 읽습니다"로 쓴다.
    // 실제로 읽는 행위("스킬 본문은 부를 때만 읽힙니다")도 "읽습니다", "불러옵니다"로 능동으로 쓴다.
    // "잘 읽히는 글", "문서를 통째로 읽히는"(사동)은 일상어라 잡지 않는다. 종결형 읽힙니다와 "~로 읽힐" 꼴만 잡는다.
    { pattern: /읽힙니다|(?:로|으로|처럼|같이|답게)\s?(?:읽힐|읽혔|읽힌다|읽히는|읽힌)(?![가-힣])/g, fix: "~로 보입니다, ~라는 뜻입니다, ~를 읽습니다", severity: "HARD", src: "2026-09-07 사용자 지적 (배포 글에 '읽힙니다' 잔존)" },
    ...
    // '할인가' 같은 명사는 제외한다. 앞 글자가 '할'이면 '인가'가 어미가 아니다.
    { pattern: /(?!할)[가-힣](는가|은가|던가|인가|한가)(?![가-힣])/g, fix: "헤딩은 명사형으로, 본문은 '-는지' 절로 (예: 왜 재는가 → 측정하는 이유, 무엇인가 → 무엇인지)", severity: "HARD", src: "2026-08-06 사용자 지적 (GA4 온보딩 커리큘럼), 2026-08-30 -인가/-한가 보강" },
