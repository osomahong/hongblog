...
const RULES = [
    ...
    // "두 갈래로"처럼 조사가 붙으면 solo()가 놓친다. 조사 결합형까지 잡는다.
    // "갈래로만"처럼 조사가 두 개 겹쳐도 잡히도록 결합 수를 늘렸다 (2026-08-15)
    { pattern: /(?<![가-힣])갈래(?:로|가|를|는|의|와|도|만){0,2}(?![가-힣])/g, fix: "가지, 종류, 구분", severity: "HARD", src: "2026-08 사용자 지적, 2026-08-11 조사 결합형 보강" },
    ...
    { pattern: /갈립니다|갈리는|갈린다|갈렸|갈라집니다|갈라지는|갈라진|갈라졌/g, fix: "달라집니다, 나뉩니다", severity: "HARD", src: "2026-08-07 사용자 지적" },
    // 수 관형사 + 추상명사 직접 결합. "세 지식", "두 재료"처럼 단위명사 없이 눌러 쓴 AI식 압축.
    // "두 값", "두 경로"는 개발 문서에서 자연스러워 셋 이상만 잡는다.
    { pattern: /(?<![가-힣])(?:한|두|세|네|다섯|여섯|일곱|여덟|아홉|열)\s?(?:지식|재료|개념)(?![가-힣])/g, fix: "단위명사를 붙인다 (세 가지 지식, 두 가지 재료)", severity: "HARD", src: "2026-08-12 사용자 지적 (투슬리 온보딩 덱)" },
    { pattern: /(?<![가-힣])(?:세|네|다섯|여섯|일곱|여덟|아홉|열)\s?(?:값|경로)(?![가-힣])/g, fix: "단위명사를 붙인다 (다섯 가지 값)", severity: "HARD", src: "2026-08-12 사용자 지적 (투슬리 온보딩 덱)" },
    ...
    { pattern: /손보[아았습는]|손봐|손볼|손봅니다|손대[는고며]|손댑니다|손댄|손이 가는|손에 익/g, fix: "고칩니다, 바꿉니다, 익숙해집니다", severity: "HARD", src: "2026-08-15 사용자 지적 (GA4 Edu), 2026-08-15 손봐 활용형 보강" },
    ...
    { pattern: /[가-힣]인가(?![가-힣])/g, fix: "의문이면 '-인지'나 명사형으로 ('무언가/누군가' 부정칭과 '~인가 싶다'는 유지)", severity: "WARN", src: "2026-08-06 사용자 지적 (GA4 온보딩 커리큘럼)" },
