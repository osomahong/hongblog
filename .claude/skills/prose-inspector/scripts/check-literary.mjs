...
const RULES = [
    ...
    // 타동사 '가르다'와 활용형. 2026-09-16에 "갈라 말씀드립니다"가 그대로 나갔다.
    // '가르마', '가르치다', '머리를 가르다' 같은 실물 용법은 건드리지 않는다.
    { pattern: /(?<![가-힣])갈라\s(?:말씀|설명|정리|적|쓰|보|놓|드리)|(?<![가-힣])갈라서\s(?:말씀|설명|정리|보)|(?<![가-힣])가르(?:면|자|고)\s(?:말씀|설명|정리)/g, fix: "나누어, 따로. 또는 나눈다는 말 없이 바로 쓴다", severity: "HARD", src: "2026-09-16 사용자 지적 ('갈라 말씀드립니다')" },
    // 추상적인 것을 '재다'로 판단한다고 쓴 은유. 길이, 시간, 온도를 재는 실물 용법은 건드리지 않는다.
    { pattern: /(?:완성도|성과|실력|수준|가치|시점|기준|역량|능력|깊이|퀄리티|품질)(?:으로|로)\s?재(?:지|다|는|고|서|면|야|었|겠)/g, fix: "정하다, 판단하다, 보고 정하다", severity: "HARD", src: "2026-09-16 사용자 지적 ('완성도로 재지 않습니다'는 말로 쓰지 않는 표현)" },
    // 지시 관형사와 함께 쓴 추상 비유 '축'. 좌표축, 회전축 같은 실물은 건드리지 않는다.
    { pattern: /(?<![가-힣])(?:(?:다른|같은|새로운|또\s?다른)\s?|(?:그|이|저)\s)축(?:으로|을|이|은|에|에서|과|와|도|만){0,2}(?![가-힣])/g, fix: "무엇인지 그대로 쓴다 (다른 주제, 관점 둘)", severity: "HARD", src: "2026-09-15 사용자 지적 (인터뷰에서 '축, 자리, 숫자 같은 비문' 지목)" },
    ...
    { pattern: /무릇|일컫|(?<![가-힣])기실|여실히|바야흐로|가히|자못|(?<![가-힣])실로 /g, fix: "일상어로 풀어 쓴다", severity: "HARD", src: "일반 문어체" },
    { pattern: /(?<!헷)(?<!엇)(?:갈립니다|갈리는|갈린다|갈렸|갈라집니다|갈라지는|갈라진|갈라졌)/g, fix: "달라집니다, 나뉩니다", severity: "HARD", src: "2026-08-07 사용자 지적" },
    // 스마트 기기를 일상 사물 이름으로 줄여 쓴 것. 영어 원문 watches and glasses를 그대로 옮기면
    // 손목시계와 안경이 되어 다른 물건을 가리킨다. 두 단어가 나란히 오는 조합만 잡아 오탐을 줄인다.
    { pattern: /시계(?:나|와|랑|,)\s?안경|안경(?:이나|과|이랑|,)\s?시계/g, fix: "스마트워치, 스마트글라스", severity: "HARD", src: "2026-09-07 사용자 지적 (K2 호라이즌 글)" },
    { pattern: /휴대전화|손전화/g, fix: "스마트폰", severity: "WARN", src: "2026-09-07 사용자 지적 (K2 호라이즌 글)" },
    ...
    // 효과가 있었는지를 속어로 대신한 것. 무엇이 통했는지 대상을 밝혀 쓴다.
    { pattern: /(?:안 )?먹히(?:는|지|나|고|기)|먹혔|먹혀요|먹힙니다|얻어걸리|얻어걸린/g, fix: "어떤 전략이 통했는지, 효과가 있었는지", severity: "HARD", src: "2026-09-07 사용자 지적 (그로스해킹 대본)" },
    // 조사가 끼어 붙여 쓰지 않은 손 은유. 기존 배포 글에 쓰인 자리가 있어 WARN으로 둔다.
    { pattern: /손을 (?:안 )?(?:대|댔|댑|댄|보|봤|본|볼)/g, fix: "고칩니다, 바꿉니다, 건드립니다", severity: "WARN", src: "2026-09-07 사용자 지적 (그로스해킹 대본)" },
    ...
    { pattern: /(?:첫|두|세|네|다섯)\s?자리(?!\s?[수값]|에\s?올|를\s?차지)|(?:첫|두|세|네|다섯|[0-9])\s?번째\s?자리|자리(?:별|마다)|(?:만나는|일하는|작업하는|머무는|쓰는|도착하는|도달하는|닿는|묻는|모이는|향하는|보이는|열리는)\s자리/g, fix: "작업 공간, 환경, 화면", severity: "HARD", src: "2026-08-10 사용자 지적 (도구를 '자리'로 은유), 2026-09-15 관형형 동사 일반화 ('도착하는 자리'가 빠져나감)" },
