...
const RULES: Rule[] = [
    ...
    // ── 금지 어휘 ──
    // '독자(獨自)적인'은 읽는 사람을 가리키는 '독자(讀者)'와 동음이의어다.
    // '독자 AI 파운데이션 모델' 같은 고유명사는 규칙 대상이 아니므로 예외로 둔다.
    {
        pattern: /독자/g,
        label: "'독자' 금지: '이 글을 읽는 분들', '여러분'으로",
        severity: "HARD",
        allowContext: /구독자|독자 AI|독자 파운데이션|독자 모델|독자적|독자 개발|독자 기술|독자성/,
    },
    ...
    // 2026-09-14 전수 점검에서 9개가 남아 FAQPage로 발행되고 있던 것을 확인하고 일반화했다.
    // 물음표로 끝나는 헤딩 가운데 해요체(요/죠)로 끝나지 않는 것을 전부 잡는다.
    { pattern: /^#{2,3} .*(?<![요죠])(까|나|지|가)\?\s*$/gm, label: "반말 의문형 헤딩 금지: 해요체로 (예: '어떻게 될까?' -> '어떻게 되나요?')", severity: "HARD" },
    // 물음표를 붙이지 않은 문어체 의문형 헤딩도 같은 위반이다. 2026-09-14에 "구체적으로 뭔가",
    // "왜 어려운가" 같은 헤딩이 위 규칙(물음표 필수)을 통과해 남아 있던 것을 확인하고 추가했다.
    { pattern: /^#{2,3} .*(는가|은가|인가|던가|뭔가|운가)\??\s*$/gm, label: "문어체 의문형 헤딩 금지: 명사형으로 (예: '왜 어려운가' -> '어려운 이유')", severity: "HARD" },
